...
# Configuration
INPUT_DIR = os.path.join("scraped_content", "bigquery_api")
OUTPUT_DIR = os.path.join("processed", "bigquery_api")
...
def clean_text(text):
    ...
    text = re.sub(r'Home\s+BigQuery\s+Documentation', '', text)
    # Trim whitespace
    text = text.strip()
    return text

def process_methods():
    """Process the BigQuery API methods."""
    print("Processing BigQuery API methods...")
    
    # Load the main methods file
    methods_file = os.path.join(INPUT_DIR, "bigquery_api_methods.json")
    ...
    for method in methods:
        ...
        if not name or name in ["Home", "BigQuery", "Documentation"]:
            continue
        
        # Extract resource type if present
        resource_type = None
        if name.startswith("REST Resource:"):
            resource_type = name.replace("REST Resource:", "").strip()
    ...
    output_file = os.path.join(OUTPUT_DIR, "bigquery_api_methods_processed.json")
...
def process_endpoints():
    """Process the BigQuery API endpoints from individual method files."""
    print("Processing BigQuery API endpoints...")
    ...
    for method_file in method_files:
        ...
        if method_file.endswith("bigquery_api_methods.json"):
            continue
        
        # Extract endpoint name from filename
        filename = os.path.basename(method_file)
        endpoint_name = filename.replace("bigquery_api_", "").replace("_methods.json", "")
    ...
    # Save all endpoints
    output_file = os.path.join(OUTPUT_DIR, "bigquery_api_endpoints.json")
    with open(output_file, 'w') as f:
...
def create_reference_markdown():
    ...
    methods_file = os.path.join(OUTPUT_DIR, "bigquery_api_methods_processed.json")
    endpoints_file = os.path.join(OUTPUT_DIR, "bigquery_api_endpoints.json")
    ...
    markdown = "# BigQuery API Reference\n\n"
    ...
    output_file = os.path.join(OUTPUT_DIR, "bigquery_api_reference.md")
    with open(output_file, 'w') as f:
        f.write(markdown)
...
def main():
    print("Starting BigQuery API data processing...")
    setup_directories()
