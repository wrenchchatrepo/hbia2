...
class CredentialServer:
    def __init__(self, host: str = "localhost", port: int = 8000, api_key: str = None):
        ...
        self.host = host
        self.port = port
        self.api_key = api_key or os.environ.get("MCP_CREDENTIALS_API_KEY")
    ...
    def start(self):
        ...
        print(f"Starting MCP Credentials server on http://{self.host}:{self.port}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="MCP Credentials Server")
