...
A secure credential management system for MCP servers. This package provides tools to securely store and retrieve credentials using the macOS Keychain.
...
git clone https://github.com/wrenchchatrepo/mcp-credentials.git
cd mcp-credentials
...
```bash
mcp-credentials store COMPANY_NAME
...
```bash
mcp-credentials store COMPANY_NAME --value "Example Company LLC"
```

### Retrieve a Credential

```bash
mcp-credentials get COMPANY_NAME
```
...
```bash
mcp-credentials store-file credentials.txt
```
...
```bash
mcp-credentials-server --host localhost --port 8000
```

The server generates a temporary API key if none is provided. To specify your own API key:

```bash
mcp-credentials-server --api-key your-api-key
```

Or set it as an environment variable:

```bash
export MCP_CREDENTIALS_API_KEY=your-api-key
...
```python
from mcp_credentials import get_specific_detail, store_in_keychain, SERVICE_NAME
```
...
To add additional credential types, modify the `ORG_KEYS` list in `mcp_credentials/keychain.py`. 