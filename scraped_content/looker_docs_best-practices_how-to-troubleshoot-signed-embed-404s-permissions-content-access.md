# https://cloud.google.com/looker/docs/best-practices/how-to-troubleshoot-signed-embed-404s-permissions-content-access
...
![](/static/looker/docs/images/hc-sso-error-2210.png)
...
Use this method to determine if the path to the content is broken. Depending on the method used to generate the signed embed URL, the location where the [content path](/looker/docs/single-sign-on-embedding#embed_url) is specified can vary. In most scripts, there is a section that defines the path. For example, to embed a dashboard with the name **123** , you would use the path `/embed/dashboards/123`. 

Use the examples in the [Embed URL](/looker/docs/single-sign-on-embedding#embed_url) section of the [Signed embedding](/looker/docs/single-sign-on-embedding) documentation page to confirm that the path is defined correctly. 

If you're using the [`Create Signed Embed Url`](/looker/docs/single-sign-on-embedding#using_the_create_sso_embed_url_api_endpoint) API endpoint to generate the URL, the **Target URL** parameter for the same dashboard would look like this:
...
If the admin can see the content at that path, then it is likely that the correct permissions or content access has not been granted to the embed user. To determine if this is the case, an admin can find the user on the [Users](/looker/docs/admin-panel-users-users) page in the **Admin** panel, and select the inline **Sudo** option. 

![](/static/looker/docs/images/hc-sudo-user-2210.png)

[Sudoing](/looker/docs/admin-panel-users-users#impersonating_\(sudoing\)_users) as another user enables admins to see what the user sees with their given permissions, content access, and user attributes. If, while sudoing as the embed user, the admin can no longer see the content, you will need to take further steps to troubleshoot the user's permissions and content access. 
...
Next, confirm that the specified or inherited roles and permissions are correctly assigned to the user by finding the user on the [Users](/looker/docs/admin-panel-users-users) page in the **Admin** panel and selecting **View**. 

One thing to note is that embed users don't have access to all the permissions a regular Looker user has — see the full list of [embed permissions](/looker/docs/single-sign-on-embedding#permissions) on the [Signed embedding](/looker/docs/single-sign-on-embedding) documentation page. 
...
The quickest way to confirm in which folder the content is saved is to view the content in the regular Looker environment and then find the folder name in the content header. After [navigating](/looker/docs/access-control-and-permission-management#viewing_a_folder_and_lists_of_looks_and_dashboards) to that folder, select the gear icon in the top right corner under **Manage Access**. You can quickly determine if a content-access issue is causing the 404 error by granting the specific embed user `View` access to the folder. 
...
[Closed systems](/looker/docs/access-levels#configuring_a_closed_system) are a special case when it comes to checking content access. A closed system silos saved content for individual groups. Implementing a closed system is often recommended as a best practice for every Powered By Looker deployment (embedded analytics). The main benefit of this system is that it completely removes the **All Users** group, sets all personal folders to private, and prevents users from seeing other users or their content in the instance (unless all the users share a group). This effectively creates a multi-tenant instance for multiple clients so that they cannot access other users' content or information unless they're explicitly granted access. 
...
If you would like to enable a closed system for your instance, [contact a Google Cloud sales specialist](https://cloud.google.com/contact) or open a [support request](https://help.looker.com/hc/en-us).