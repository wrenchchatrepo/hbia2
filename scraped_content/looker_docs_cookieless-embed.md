...
**Important:** Looker has two embed APIs: the Looker Embed SDK and a JavaScript `window.postMessage`-based API. Looker cookieless embed works for both of these APIs. The Looker Embed SDK simplifies how the embedding client interacts with the Looker iframe, but examples that illustrate the use of the `window.postMessage` API are provided in this document. The Looker cookieless embed API is different from the Looker signed embed URL signing API and requires different processing in the embedding application server. The Looker signed embed URL signing API and the Looker cookieless embed API may be used at the same time, although each individual user _must_ use only one method or the other.

To use either API, the embedding application must be able to authenticate into the Looker API with admin privileges. The embed domain must also either be listed in the [**Embed Domain Allowlist**](/looker/docs/admin-panel-platform-embed#embedded_domain_allowlist), or, if using Looker 23.8 or later, the embed domain can be included when the cookieless session is acquired.
...
![A sequence diagram that illustrates the creation of an embed iframe.](/static/looker/docs/images/create-embed-iframe-workflow-2440.png)
  ...
  7. Once the tokens have been returned to the browser, a Looker embed login URL must be constructed. The Looker Embed SDK will construct the embed login URL automatically. To use the the `windows.postMessage` API to construct the embed login URL, see the Using the Looker `windows.postMessage` API section of this document for examples.

The login URL does not contain the signed embed user detail. It contains the target URI, including the navigation token, and the authorization token as a query parameter. The authorization token must be used within 30 seconds and can only be used once. If additional iframes are required, an embed session must be acquired again. However, if the session reference token is provided, the authorization token will be associated with the same session.
...
If valid, the session is restored for the request and the dashboard request runs.
  ...
  13. The Looker UI running in the iframe determines that the dashboard HTML is a cookieless embed response. At that point the Looker UI sends a message to the embedding application requesting the tokens that were retrieved in step 6. The UI then waits until it receives the tokens. If the tokens do not arrive, a message is displayed.
...
![A sequence diagram that illustrates generating new tokens.](/static/looker/docs/images/generating-new-tokens-2220.png)
...
Looker cookieless embed can be implemented by using either the Looker Embed SDK or the `windows.postMessage` API. The method that uses the [Looker Embed SDK](https://github.com/looker-open-source/embed-sdk) is easier, but an example showing how to use the `windows.postMessage` API is also available. Detailed explanations of both implementations can be found in the Looker [Embed SDK README file](https://github.com/looker-open-source/embed-sdk#readme). The [Embed SDK git repository](https://github.com/looker-open-source/embed-sdk) also contains working implementations.

### Configuring the Looker instance

Cookieless embedding has commonality with Looker [signed embedding](/looker/docs/single-sign-on-embedding). Cookieless embedding relies on **Embed SSO Authentication** being enabled. However, unlike Looker signed embedding, cookieless embedding does not use the **Embed Secret** setting. Cookieless embedding uses a JSON Web Token (JWT) in the form of an **Embed JWT Secret** setting, which can be set or reset on the the [**Embed** page](/looker/docs/admin-panel-platform-embed) in the **Platform** section of the **Admin** menu.
...
The following example shows how callbacks are used. Callbacks should only be used when it is necessary for the embedding client application to be aware of the status of the Looker embedding session. You can also use the [`session:status` event](/looker/docs/embedded-javascript-events#session:status), making it unnecessary to use callbacks with the Embed SDK.
...
When the Looker embed session expires, a dialog will be displayed in the embedded iframe. At this point, the user will not be able to do anything in the embedded instance. When this occurs the [`session:status` events](/looker/docs/embedded-javascript-events#session:status) will be generated, allowing the embedding application to detect the current state of the embedded Looker application and take some kind of action.
