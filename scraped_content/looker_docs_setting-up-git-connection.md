...
When you create a new project, it exists only in [Development Mode](/looker/docs/dev-mode-prod-mode#development_mode) to let you develop your model in a safe environment that won't impact other users. Once you're ready to publish your project, the next step is to configure a Git connection.

Configuring a Git connection for your project lets you deploy your LookML to [Production Mode](/looker/docs/dev-mode-prod-mode#production_mode), which lets other users explore your data model, build dashboards, and add LookML to your model. For a quicker setup, use a bare repository. For a more robust Git integration, create your own Git repository and follow the instructions on this page to connect it to your Looker instance.

**Note:** When you create a new project during the process of [generating a model](/looker/docs/generating-a-model), the project is created with a bare repository.
...
To access Git integration options, you must have [Development Mode turned on](/looker/docs/dev-mode-prod-mode#switching_in_and_out_of_development_mode).
...
If your Git account uses two-factor authentication, you can go to your Git provider and create access tokens to use instead of passwords. Go to the [Setting up HTTPS git connection with 2FA Enabled](https://www.googlecloudcommunity.com/gc/Administering-Looker/Setting-up-HTTPS-git-connection-with-2FA-Enabled/td-p/580237) Community post to see instructions for creating personal access tokens for common Git providers.
...
Note the following for GitHub HTTPS authentication:
  ...
  * GitHub doesn't accept account passwords for authenticating Git operations on github.com. See [GitHub's blog post](https://github.blog/2020-12-15-token-authentication-requirements-for-git-operations/) for more information. To connect a Looker project to GitHub by using HTTPS, [use the developer settings in GitHub](https://docs.github.com/en/github/authenticating-to-github/keeping-your-account-and-data-secure/creating-a-personal-access-token) to create a personal access token.
  * Looker doesn't support GitHub's [fine-grained personal access tokens](https://github.blog/2022-10-18-introducing-fine-grained-personal-access-tokens-for-github/). To connect your Looker project to GitHub by using HTTPS, use GitHub's **Tokens (classic)** option when you create a personal access token.

#### Single account HTTPS authentication

If you set up your LookML project with a single Git account, Looker uses that Git account to sign in to your Git provider to commit changes on behalf of the developers. Looker makes these commits by using the developer's Looker username so that you can tell which developer made each commit. You can see your project's commit history on your Git provider's interface or by selecting the [**History**](/looker/docs/git-command-reference#history) option from the Git menu of the Looker IDE. See [Executing Git commands in Looker](/looker/docs/version-control-and-deploying-changes#executing_git_commands_in_looker) for more information on the Git menus in Looker.
...
The following tasks and requirements are needed for Git authentication with user attributes:
  ...
  * Your Looker admin must set up Looker user accounts with [user attributes](/looker/docs/admin-panel-users-user-attributes#creating_user_attributes) that correspond to Git username and Git password (or [access token](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens) if the Git user accounts have two-factor authentication).
  * User attributes for Git account passwords (or access tokens) must be [hidden](/looker/docs/admin-panel-users-user-attributes#creating_user_attributes). When you create the password or access token attribute, select **Yes** under the **Hide Values** option and enter the Git provider URL in the **Domain Allowlist** field.
  * The user attributes for Git name and password (or access token) must be filled in for each Looker developer. User attributes can be configured [by a Looker admin](/looker/docs/admin-panel-users-users#user_attributes) or [by the Looker user](/looker/docs/user-menu#changing_standard_account_settings).

The following example shows the [**User Attributes** Admin page](/looker/docs/admin-panel-users-user-attributes), where a Looker admin has set up user attributes for the Git username and the user access token.

![Table on the User Attributes Admin page that displays the string-type user attributes github_token and github_username.](/static/looker/docs/images/dev-git-https-useratt-admin-2112.png)

The following example shows a Looker user's [**Account**](/looker/docs/user-account) page where the user has entered their Git credentials into the user attribute fields.

![](/static/looker/docs/images/dev-git-https-useratt-account-2112.png)
...
![](/static/looker/docs/images/dev-connect-git-create-repo-existing-https-2114.png)
...
![](/static/looker/docs/images/dev-connect-git-create-repo-https-2114.png)

  2. [Navigate to your LookML project](/looker/docs/manage-projects#accessing-project-files), and select the **Settings** icon from the navigation bar.
...
> Resetting your Git connection will preserve Git history for the main branch. It will also preserve the history of each Looker developer's personal branch once they pull, merge, and deploy. To preserve history for all branches, see the [Migrating to a new Git repository](/looker/docs/best-practices/how-to-migrate-to-a-new-git-repo) Best Practices page.

![](/static/looker/docs/images/dev-connect-git-2112.png)
...
> If your Git account uses two-factor authentication, or if you're using GitHub (which requires a personal access token instead of a password), you can go to your Git provider and create access tokens to use instead of passwords. Go to the [Setting up HTTPS git connection with 2FA Enabled ](https://www.googlecloudcommunity.com/gc/Administering-Looker/Setting-up-HTTPS-git-connection-with-2FA-Enabled/td-p/580237) Community post to see instructions for creating personal access tokens for common Git providers.
  ...
  8. If you selected **Use user attributes for username and password/personal access token** , Looker displays the **Username User Attribute** and **Personal Access Token User Attribute** drop-down menus in the **Development Mode Credentials** section. Use the drop-down menus to select the [user attributes](/looker/docs/admin-panel-users-user-attributes#creating_user_attributes) for an individual developer's Git credentials.

![](/static/looker/docs/images/dev-configure-git-https-user-attr-2114.png)

(Looker users can edit the values for their user attribute fields on their [Account](/looker/docs/user-account) page, or Looker admins can edit the user attribute values for a user on the [Users](/looker/docs/admin-panel-users-users#user_attributes) admin page.)

  9. Select the **Continue Setup** button.

Git is now configured for your LookML project. From here you can [validate your LookML](/looker/docs/lookml-validation#validating_your_changes), and then create your initial commit and deploy to production to make your project available in [Production Mode](/looker/docs/dev-mode-prod-mode#production_mode). You can also:

  * Use the Git menu to access the [Git commands](/looker/docs/version-control-and-deploying-changes#executing_git_commands_in_looker).
  * Use the Git menu to test your Git connection.
  * If you're a Looker admin, go to the [project settings](/looker/docs/git-options#project_settings) to require LookML validation and to configure [Git integration options](/looker/docs/git-options#git_integration_options).
...
To configure a LookML project with SSH Git authentication, follow these steps:
  ...
  1. [Open your project](/looker/docs/manage-projects#accessing-project-files) or [create a new LookML project](/looker/docs/generating-a-model).
...
![Diagram showing the progression from the Project Settings page to the Configure Git page. New projects will display a Configure Git button, and existing projects will display a Reset Git Connect button, but both buttons lead to the Configure Git page.](/static/looker/docs/images/dev-connect-git-2112.png)
  ...
  4. Get the SSH URL for your Git repository. Here are the formats for common Git providers:
     ...
     * [Cloud Source Repositories](https://community.looker.com/technical-tips-tricks-1021/using-cloud-source-repositories-for-version-control-in-looker-30241): `ssh://username@example.com@source.developers.google.com:2022/p/<project-id>/r/<repository-name>`
     * GitLab: `git@gitlab.com:<organization-name>/<repository-name>.git`
     * [Bitbucket](https://community.looker.com/technical-tips-tricks-1021/using-bitbucket-for-version-control-in-looker-30244): `git@bitbucket.org:<organization-name>/<repository-name>.git`
     * [Phabricator](https://community.looker.com/technical-tips-tricks-1021/setting-up-phabricator-and-looker-for-version-control-30243): `ssh://username@example.com/diffusion/MYCALLSIGN/<repository-name>.git`
...
![](/static/looker/docs/images/dev-connect-git-create-repo-718.png)
...
![](/static/looker/docs/images/dev-connect-git-create-repo-existing-718.png)
...
![](/static/looker/docs/images/dev-git-new-deploy-keys-718.png)
...
**Note:** This is an important step, because Looker requires this setting to make and deploy changes to your LookML model.
...
If you haven't yet created a remote Git repository, or if you just want to get set up quickly so others can benefit from your LookML, you can configure a bare Git repository. Configuring a bare Git repository creates a local Git repository on your Looker instance, rather than connecting to a remote repository.

When you select the option to create a bare repository, Looker creates the repository on your Looker server and starts the Git history for the repository.

When you create a new project during the process of [generating a model](/looker/docs/generating-a-model), the project is automatically configured with a bare repository.

**Warning:** Don't use the bare repository option if you intend to subsequently connect your LookML project to a Git repository that has a Git history. If you do so, Looker will be unable to reconcile the two Git histories and you will be unable to perform any Git actions.
...
> Resetting your Git connection will preserve Git history for the main branch. It will also preserve the history of each Looker developer's personal branch once they pull and merge and then deploy. To preserve history for all branches, see the [Migrating to a new Git repository](/looker/docs/best-practices/how-to-migrate-to-a-new-git-repo) Best Practices page.

  4. Select **Set up a bare repository instead** at the bottom of the **Configure Git** page.

![](/static/looker/docs/images/dev-configure-git-bare-repo-718.png)
...
**Warning:** Don't use this procedure if you have configured your project with a bare repository. If you have set up your LookML project with a bare repository, Looker creates a repository on your Looker server and starts the Git history for the repository. If you subsequently connect your LookML project to a Git repository that has a Git history, Looker will be unable to reconcile the two Git histories and you will be unable to perform any Git actions.
...
**Note:** This workflow is only for repositories to which you have write access.
  ...
  3. Once you have configured Git for your project, [pull from production](/looker/docs/git-command-reference#pull_prod) from the **Git Actions** panel.
  4. Once you have pulled from production, [select **Deploy to Production**](/looker/docs/version-control-and-deploying-changes#deploying_to_production).
...
If you want to to use a specific public repository as the basis of a blank LookML project but you don't have write access to that repository, follow the instructions that are outlined in the [Cloning a public Git repository](/looker/docs/create-projects#cloning_a_public_git_repository) section of the **Creating a new LookML project** documentation page.

If you're setting up a Git workflow using one repository across multiple instances, see the Community post on [Git workflow using one repository across multiple instances — development, staging, and production](https://www.googlecloudcommunity.com/gc/Technical-Tips-Tricks/Git-Workflow-Using-One-Repository-Across-Multiple-Instances/ta-p/587693) for more information.
...
If Looker is unable to connect to your Git repository, the [Git button](/looker/docs/version-control-and-deploying-changes#executing_git_commands_in_looker) will display the text **Test Git Connection** to prompt you to troubleshoot your connection.

![](/static/looker/docs/images/dev-git-test-auto-718.png)

You can also access the Git connection test tool by selecting **Test Git Connection** in the **Git actions** panel:

![](/static/looker/docs/images/dev-git-test-manual-718.png)
...
![Git Connection Test dialog displaying a list of steps with green checkmarks next to them to indicate that all steps were successful.](/static/looker/docs/images/dev-git-test-success-2100.png)
...
![Git Connection Test dialog displaying a list of successful and failed steps. An error under the Verify authorization credentials step reads Cannot AUTH to remote \(Looker deploy key\).](/static/looker/docs/images/dev-git-test-error-2100.png)