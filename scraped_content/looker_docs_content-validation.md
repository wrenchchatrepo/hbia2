...
You must have the [`develop`](/looker/docs/admin-panel-users-roles#develop) permission to use the Content Validator. 

**Note:** A Looker developer can use the Content Validator to update references in Looks and dashboards, even if the developer has only the **View** [access level](/looker/docs/organizing-spaces#folder_access_levels) for the Looks and dashboards. This allows a developer to fix content that might become broken when the developer updates the underlying LookML.

The reasoning is that a developer needs to be able to fix content that might be broken when the developer makes changes to the underlying LookML.

To open the Content Validator, select **Content Validator** from the **Develop** menu in the [main navigation panel](/looker/docs/finding-content#using_the_left_navigation_panel). From here, you can use the Content Validator in two ways:
...
Also, consider your timing when using the Content Validator. When you push development changes to production you should fix affected Looks and tiles at approximately the same time, so that they are broken for the minimum amount of time. If you fix Looks and tiles too early, based on your work in [Development Mode](/looker/docs/dev-mode-prod-mode), they will appear broken to users until you push your changes. Similarly, if you delay in fixing Looks and tiles until long after pushing the changes, the Looks and tiles will appear broken to users until you fix them.

**Note:** If you use the Content Validator to change field names that serve as filters in your dashboards, be sure to review the **Filters** section of your dashboard schedules and update the filters as needed to reflect your changes. If a scheduled dashboard delivery includes filters that no longer function (for example, if the referenced field has changed), the scheduled delivery will fail and the schedule owner will receive an error message that a filter is invalid. 

Use caution when using the Content Validator to make a change to an Explore that is used in a [merged results](/looker/docs/merged-results) dashboard tile, as the Content Validator applies any changes to _every_ Explore that is included in the merged results tile. For example, you cannot use the Content Validator to change the model name in a single Explore in a merged results tile without changing the model name in all the Explores that are included in the merged results.

## Using the Content Validator to fix errors

The Content Validator is a useful tool for finding and fixing errors resulting from changes in your LookML. For example, if you change a field name from `customer` to `user`, any Looks or dashboard tiles that included the `customer` field will no longer function (unless you used the [`alias`](/looker/docs/reference/param-field-alias) parameter). The Content Validator will list any references to the `customer` field and will also provide **Replace** or **Remove** buttons to fix the error.

When you run the Content Validator, it validates all references that your Looks and dashboards make to your LookML [models](/looker/docs/model-and-view-files#model_files), [Explores](/looker/docs/reference/param-explore-explore), [views](/looker/docs/reference/param-view-view), and [fields](/looker/docs/reference/param-field), and will show an error for any references your content makes to an unknown LookML object.

The Content Validator does _not_ show errors for the following:

  * **Looks that have been deleted and are in the[Trash](/looker/docs/admin-spaces#trash).** If you want to validate a deleted Look, your Looker admin must restore the Look first.
  * **Content that is based on LookML models for which you don't have`develop` permission.** You can use the Content Validator only on the content that is based on models for which you have `develop` permission (the models that you can see in the Looker IDE). See the [Roles](/looker/docs/admin-panel-users-roles#model_sets) documentation page for information on model permissions. 

**Note:** As long as you have `develop` permission on the underlying model for a Look or dashboard, you can use the Content Validator to update references in the Look or dashboard, even if you have only the **View** [access level](/looker/docs/organizing-spaces#folder_access_levels) for the Look or dashboard. This lets a developer fix content that might be broken when the developer makes changes to the underlying LookML, even when the developer doesn't have the **Manage Access, Edit** access level for the content itself.

### Running the Content Validator

**Important:** Read the Before using the Content Validator section before proceeding. The Content Validator can impact many parts of your Looker content, so it's important to plan and consider beforehand.

To improve the performance of the Content Validator, a Looker admin can enable the [Content Validator Scoping](/looker/docs/admin-panel-general-labs#content-validator-scoping) Labs feature on your instance. When this Labs feature is enabled, you can scope the validation to specific LookML projects and a specific content folder. See the Scoping the Content Validator section for information about how to scope the validation.

If the Content Validator Scoping Labs feature is not enabled on your instance, follow these steps to run content validation:
...
The Content Validator will run validation on all of the content that is based on LookML projects for which you have `develop` permission and then display the results. See the section Viewing the content validation results for information about reviewing the results.

#### Scoping the Content Validator

When the [Content Validator Scoping](/looker/docs/admin-panel-general-labs#content-validator-scoping) Labs feature is enabled, you can scope the validation to specific LookML projects and a specific content folder (including its subfolders, if any). This can improve the performance of the Content Validator.

**Note:** Because this is a Looker Labs feature, it is not supported for [Looker (Google Cloud core)](/looker/docs/looker-core-overview) instances
...
Note the following for Content Validator scoping:
  ...
  * If you specify a LookML project, the Content Validator determines the project's associated models by looking at the source LookML of the specified projects and by looking at the models configured for each project, which you can see on the [**LookML Projects**](/looker/docs/manage-projects) page. If a model file has been deleted from a LookML project and isn't shown as a model in the project on the **LookML Projects** page, the Content Validator won't show errors that are related to content based on that deleted model. 

**Tip:** If you want to find errors related to content based on a deleted model, where the model is no longer showing on the **LookML Projects** page, run the Content Validator without scoping the validation for LookML projects.

To run the Content Validator on a Looker instance with **Content Validator Scoping** enabled, follow these steps:
...
When the Content Validator completes a validation, it displays an error table. If a Looker admin has enabled the [Content Validator Scoping](/looker/docs/admin-panel-general-labs#content-validator-scoping) Labs feature on your instance, the validation results are scoped to the projects and folders that you selected when you ran the Content Validator.
...
**Warning:** When you use the Content Validator to make updates to Looks and tiles that are based on changes that you've made in Development Mode, you might break the Looks and tiles in Production Mode until you push your Development Mode updates to production. If you are in Development Mode, the Content Validator reminds you of this in a warning message that content is validated against the state of the LookML projects in your Development Mode.
...
![Content Validator window showing multiple errors in validation results](/static/looker/docs/images/content-validator-results-2500.png)
...
You can use the **Group by** tabs at the upper right of the page to change the layout of the error table:
  ...
  * **Folder** : List each [folder](/looker/docs/organizing-spaces), grouping together the Looker content that has errors. This is useful if you want to fix only the errors in a particular folder.
...
The icon next to the content's name identifies the content type:
  ...
  * ![Tile on dashboard icon.](/static/looker/docs/images/dev-content-validator-icon-db-tile-2108.png) — Dashboard tile
  * ![Alert on dashboard icon.](/static/looker/docs/images/dev-content-validator-icon-db-alert-2108.png) — Dashboard alert
  * ![Schedule on dashboard or Schedule on Look icon.](/static/looker/docs/images/dev-content-validator-icon-schedule-2108.png) — Schedule for a dashboard or Look
  * ![Filter on dashboard icon.](/static/looker/docs/images/dev-content-validator-icon-db-filter-2108.png) — Filter for a dashboard, or a field on a tile that listens to a dashboard filter (see the [Adding and editing user-defined dashboard filters](/looker/docs/filters-user-defined-dashboards#adding_dashboard_filters) documentation page for information on configuring tiles to listen to dashboard filters)
...
![](/static/looker/docs/images/develop-content-validator-group-error-2308.png)
...
![](/static/looker/docs/images/develop-content-validator-group-content-214.png)
...
The content validation results include a **Replace** button for the following types of elements so that you can rename them:
  ...
  * View names in [custom filters](/looker/docs/filtering-and-limiting#custom-filters) or [table calculations](/looker/docs/table-calculations)
  * View names in Looks and dashboard tiles
  * Field names: 
    * In the [**Data** section of a Look](/looker/docs/viewing-looks#making_sense_of_a_look)
    * In a [query-based dashboard tile](/looker/docs/creating-user-defined-dashboards#query_tiles)
    * Referenced in [table calculations](/looker/docs/table-calculations)
    * Referenced in [custom filters](/looker/docs/filtering-and-limiting#custom-filters)
    * Referenced in [custom fields](/looker/docs/custom-fields#viewing_and_using_custom_fields)
    * Referenced in visualization configurations, such as the [**Customizations**](/looker/docs/column-options#customizations) area of the **Series** tab for column charts
    * Referenced by a dashboard tile to listen to a dashboard filter (this is configured in the **Tiles to Update** tab of the filter configuration window, described on the [Adding and editing user-defined dashboard filters](/looker/docs/filters-user-defined-dashboards#adding_dashboard_filters) documentation page)
...
![](/static/looker/docs/images/develop-content-validator-replace-3-2308.png)
...
You cannot use the Content Validator to remove fields from [custom filters](/looker/docs/filtering-and-limiting#custom-filters), [custom fields](/looker/docs/custom-fields#viewing_and_using_custom_fields), or [table calculations](/looker/docs/table-calculations), because typically you need to make additional changes to keep the expression working. Instead, use the content validation error messages to find places where custom filters, custom fields, and table calculation expressions need to be fixed, and then rework the expression as appropriate.
...
![](/static/looker/docs/images/develop-content-validator-remove-2-2308.png)
...
![](/static/looker/docs/images/develop-content-validator-remove-3-2308.png)
...
**Note:** If you accidentally delete a Look, your Looker admin _might_ be able to recover it for you. Looker stores deleted content in the trash until your Looker admin team empties the [trash](/looker/docs/admin-spaces#trash) as part of their maintenance process.
...
Note the following about the Content Validator:
  ...
  * Looks that have been deleted and are in the [Trash](/looker/docs/admin-spaces#trash) folder won't be validated. If you want to validate a deleted Look, your Looker admin must restore the Look first.
