...
The primary query returns the following results:  Products Category | Products Count  
---|---  
Active | 5  
Jeans | 9  
Formalwear | 3  
  
The secondary query returns the following results:  Products Category | Items in Inventory Count  
---|---  
Active | 11  
Jeans | 16  
Formalwear | 6  
...
You can also reference the [Why are there nulls in my secondary merge results query?](/looker/docs/best-practices/how-to-troubleshoot-nulls-in-secondary-merge-result-query) Best Practices page for more troubleshooting tips for missing or null merged results values.
...
The primary query returns the following results:  Products Category | Products Department | Products Count  
---|---|---  
Active | Kids | 522  
Active | Adults | 545  
Dresses | Adults | 878  
...
The secondary query returns the following results:  Products Category | Products Brand Name | Items in Inventory Count  
---|---|---  
Active | Brand 1 | 223  
Dresses | Brand 2 | 80  
Dresses | Brand 3 | 3  
...
This is all you need to start merging results. However, you can also use some advanced exploring techniques to further refine your query. You can:
  ...
  3. Optionally, add [filters](/looker/docs/filtering-and-limiting) for the data.

  4. Optionally, include [table calculations](/looker/docs/table-calculations) to create ad hoc metrics.
...
You can use the merged results to:
  ...
  * Examine and sort the data, including [drilling into the data](/looker/docs/viewing-and-interacting-with-explores#drilling_down_into_the_data). If you drill into the dimension used to merge the two Explores, however, Looker displays only drill fields from the primary Explore.
  * View [totals](/looker/docs/creating-and-editing-explores#displaying_totals). Looker calculates totals on each of the component queries and uses those totals in the merged results. Therefore, totals may appear too high, because what you are seeing are totals calculated before the [results were merged](/looker/docs/merged-results#what_if_one_query_doesnt_have_a_matching_data_value). One way to avoid this is to align the filters on each query.
  * Create visualizations.
  * [Pivot dimensions](/looker/docs/creating-and-editing-explores#pivoting_dimensions) in the merged results by selecting **Pivot** from the gear menu in the dimension's column of the data table. Note that you can't pivot dimensions in the source queries.
...
To download your merged results query, you can save the query to a dashboard and then [download the dashboard as a PDF or as a collection of CSV files](/looker/docs/downloading#downloading_data_from_a_dashboard).
...
Once you've added your merged results query to a dashboard, you can [add or apply dashboard filters](/looker/docs/filters-user-defined-dashboards) to your merged results tile, [rearrange the tile](/looker/docs/editing-user-defined-dashboards#rearranging_and_resizing_tiles), [edit the tile](/looker/docs/editing-user-defined-dashboards#editing_tiles), or [add new tiles](/looker/docs/creating-user-defined-dashboards#adding_tiles_and_text_to_a_dashboard) to your dashboard. You can't download the data from a tile based on merged results, but you can [download the dashboard as a PDF](/looker/docs/downloading#downloading_a_dashboard_as_a_pdf) or [as a collection of CSV files](/looker/docs/downloading#downloading_a_dashboard_as_csvs).

> Any dashboard filters applied to a merged results tile will be turned off if the merged query is changed in any way. You can reinstate the filters by turning them back on again in the [filter configuration window](/looker/docs/filters-user-defined-dashboards#editing_dashboard_filters).

## Merging queries in embedded Looks, dashboards, and Explores

You can merge queries within [embedded Looks, dashboards, and Explores](/looker/docs/private-embedding), if you have the appropriate permissions. To merge the query of an [embedded Look](/looker/docs/private-embedding#embedding_a_look) with another query:
...
To merge an [embedded dashboard](/looker/docs/private-embedding#embedding_a_dashboard) tile's query:
