...
[Liquid](https://shopify.github.io/liquid/) is a templating language that you can use in Looker to create more dynamic content. For example, you could build URLs to external tools based on the results of a query, or change which database table is queried based on a user's selection.

Liquid statements are built from variables, filters, and tags. Variables contain information that you want to use, and the variables that Looker provides are described on this page. You can further modify those values by using filters and tags, which you can read about in this [Liquid guide](https://github.com/Shopify/liquid/wiki/Liquid-for-Designers).

There are several places in LookML that you can use Liquid:

  * The [`action`](/looker/docs/reference/param-field-action) parameter
  * The [`description`](/looker/docs/reference/param-field-description#using_liquid_variables_in_the_description_parameter) parameter of a field (but not of an Explore)
  * The [`html`](/looker/docs/reference/param-field-html#liquid_variables) parameter
  * Label parameters at the field level, including the [`label`](/looker/docs/reference/param-field-label) parameter, [`view_label`](/looker/docs/reference/param-field-view-label) parameter, [`group_label`](/looker/docs/reference/param-field-group-label) parameter, and [`group_item_label`](/looker/docs/reference/param-field-group-item-label) parameter
  * The [`link`](/looker/docs/reference/param-field-link#liquid_variables) parameter
  * Parameters that begin with `sql` (such as [`sql`](/looker/docs/reference/param-field-sql#liquid-variables) and [`sql_on`](/looker/docs/reference/param-explore-join-sql-on#liquid-variables))
  * The [`default_value`](/looker/docs/reference/param-lookml-dashboard#default_value) dashboard filter parameter
  * The [`filters`](/looker/docs/reference/param-lookml-dashboard-table-chart-legacy#filters) dashboard element parameter
...
>   * Chart downloads — When used in a query that is rendered as a table chart, if the table chart columns have been manually rearranged, some download formats of the chart [display columns in their original orders](/looker/docs/downloading#column_order).
> 
...
A = Works with the [`action`](/looker/docs/reference/param-field-action) parameter.

DV = Works with the [`default_value` (for dashboards)](/looker/docs/reference/param-lookml-dashboard#default_value) parameter.

DE = Works with the [`description`](/looker/docs/reference/param-field-description#using_liquid_variables_in_the_description_parameter) parameter at the field level, but won't work with `description` at the Explore level.

F = Works with the [`filters` (for dashboard elements)](/looker/docs/reference/param-lookml-dashboard-table-chart-legacy#filters) parameter.

H = Works with the [`html`](/looker/docs/reference/param-field-html#liquid_variables) parameter.

LA = Works with the label parameters at the field level, including the [`label`](/looker/docs/reference/param-field-label#liquid_variables_with_label) parameter, [`view_label`](/looker/docs/reference/param-field-view-label) parameter, [`group_label`](/looker/docs/reference/param-field-group-label) parameter, and [`group_item_label`](/looker/docs/reference/param-field-group-item-label) parameter, but won't work with label parameters at the model, Explore, view, or reference line level, or with `label` as a subparameter of `link`.

LI = Works with the [`link`](/looker/docs/reference/param-field-link#liquid_variables) parameter.

S = Works with all LookML parameters that begin with `sql` (e.g. [`sql`](/looker/docs/reference/param-field-sql#liquid-variables), [`sql_on`](/looker/docs/reference/param-explore-join-sql-on#liquid-variables), and [`sql_table_name`](/looker/docs/reference/param-view-sql-table-name)).

Variable |  Definition |  Usage | Example Output  
---|---|---|---  
Field Values  
`value` |  The raw value of the field returned by the database query. Can refer to a pivoted field's value.In addition to the parameters shown in the **Usage** column, `value` is supported in the `label` subparameter of the [`action`](/looker/docs/reference/param-field-action) and [`link`](/looker/docs/reference/param-field-link#liquid_variables) parameters.Note: The `value` variable is HTML-escaped. For example, the character `<` is changed to `&lt;`. As a result, HTML tags in `value` won't render as HTML.|  A H LI | 8521935  
`rendered_value` |  The value of the field with Looker's default formatting.You can reference [date formatting syntax](/looker/docs/time-formatting-for-charts) in `rendered_value`, as shown in the [Easy date formatting with Liquid](/looker/docs/best-practices/how-to-use-liquid-to-format-dates) Best Practices page.In addition to the parameters shown in the **Usage** column, `rendered_value` is supported in the `label` subparameter of the [`action`](/looker/docs/reference/param-field-action) and [`link`](/looker/docs/reference/param-field-link#liquid_variables) parameters.|  A H LI | $8,521,935.00  
`filterable_value` |  The value of the field formatted for use as a filter in a Looker URL. For example, when filtering on a string value that include a comma such as "Altostrat, Inc", the `value` variable returns two different strings, "Altostrat" and "Inc". The `filterable_value` variable corrects this by escaping special characters and returning a single string, in this example, "Altostrat, Inc".|  A H LI | 8521935  
Links  
...
`linked_value` |  The value of the field with the Looker default formatting and default linking. Measures don't have default linking, so measures require configuration of the [`drill_fields`](/looker/docs/reference/param-field-drill-fields) parameter to work with `linked_value`. |  A H LI |  [$8,521,935.00](https://yourcompany.looker.com/explore/thelook/orders?fields=orders.order_amount&limit=500)  
Filters  
`_filters['view_name.field_name']` | The user filters applied to the field you ask for with `view_name.field_name`.`_filters['view_name.field_name']` is also supported in the [`sql`](/looker/docs/reference/param-view-sql-for-derived-table) parameter of a [derived table](/looker/docs/derived-tables), but is not supported in other `sql` parameters. Using `_filters['view_name.field_name']` in a derived table `sql` parameter requires the `sql_quote` Liquid filter.|  A DE H LA LI |  NOT NULL  
...
`{% condition filter_name %}``sql_or_lookml_reference``{% endcondition %}` |  The value of the filter you ask for with `filter_name` applied to the `sql_or_lookml_reference` as SQL. This variable is used with [templated filters](/looker/docs/templated-filters) and [conditional joins](/looker/docs/reference/param-explore-join-sql-on#conditional_joins). |  S |  _See the[Templated filters](/looker/docs/templated-filters) documentation page and the [Conditional joins](/looker/docs/reference/param-explore-join-sql-on#conditional_joins) section of the **`sql_on`** documentation page for examples._  
`{% parameter parameter_name %}` |  The value of the parameter filter you ask for with `parameter_name`. |  DE LA S |  _See the[`parameter`](/looker/docs/reference/param-field-parameter) parameter documentation page for examples._  
`parameter_name._parameter_value` |  Injects the value of the parameter filter you ask for with `parameter_name` into a logical statement. |  DE H LA LI S |  _See the[`parameter`](/looker/docs/reference/param-field-parameter#using_parameter_in_a_logical_statement) parameter documentation page for important details and examples._  
User Attributes  
`_user_attributes['name_of_attribute']` |  The value of the [user attribute](/looker/docs/admin-panel-users-user-attributes) you ask for with `name_of_attribute`, for the particular user running the query, if user attributes are being used. The `_user_attributes['name_of_attribute']` variable can also be used in [advanced filter syntax](/looker/docs/admin-panel-users-user-attributes#advanced_filter_syntax).|  A DE H LA LI S DV F |  northeast _(if, for example, the user attribute was "region")_ See the [Using user attributes for dynamic schema and table name injection](/looker/docs/best-practices/how-to-create-a-dynamic-schema-with-user-attributes) Best Practices page for an additional example.  
`_localization['localization_key']` |  Returns the value associated with a localization key defined in a model's [strings file](/looker/docs/model-localization#creating_locale_strings_files) based on a [user's locale](/looker/docs/admin-panel-users-users#locale_\(beta\)). |  DV F |  _See the[Localizing your LookML model](/looker/docs/model-localization#filter_localization_example) documentation page for an example._  
LookML Objects  
...
Queries  
`_query._query_timezone` |  The [time zone](/looker/docs/admin-panel-general-settings#user_specific_time_zones) in which the query was run. |  A DE H LA LI S |  America/Los_Angeles  
...
`view_name.field_name._is_selected` |  Returns `true` if the field you ask for with `view_name.field_name` is selected and therefore included in the query or is included in the query using the `required_fields` parameter.In totals calculations, `_is_selected` returns `false` in certain cases:
...
This is because, when calculating totals, the query is run with all dimensions removed. Therefore, dimensions are not considered selected in Liquid during totals calculation. |  DE LA LI S |  true  
...
See the [Using date_start and date_end](/looker/docs/best-practices/how-to-use-date_start-and-date_end-with-date-filters) Best Practices page for an in-depth explanation on how to use the `date_start` and `date_end` Liquid variables to deal with date-partitioned tables.

See the Analytic Block [Flexible period-over-period analysis](https://community.looker.com/lookml-5/analytic-block-flexible-period-over-period-analysis-4507) Community post for an example of using `date_start` and `date_end` for flexible period-over-period analysis.
...
You can use Liquid variables in a field's [`label`](/looker/docs/reference/param-field-label) parameter to dynamically change the field's appearance in the field picker and in visualizations. See the table in the Liquid variable definitions section on this page to see which Liquid variables will work with the `label` parameter.
...
The other Liquid variables marked with **LA** in the [Liquid variable definitions table](/looker/docs/liquid-variable-reference#liquid_variable_definitions), such as those that return a value based on a filter (like `_filters`) or require that a query be run before the variable value can be determined (like `in_query`), won't change the name of the field in the field picker. In those cases, the field name will only be changed in the resulting visualization.

When using the [`parameter`](/looker/docs/templated-filters) Liquid variable with `label`, `label` is passed the value of the [`value`](/looker/docs/reference/param-field-parameter#specifying_allowed_values) subparameter.

### Usage of Liquid variables with the `description` parameter

You can use Liquid variables with the [`description`](/looker/docs/reference/param-field-description) parameter to dynamically change the description for a field. This description appears when users hold the pointer over the field's information icon in the [field picker](/looker/docs/changing-explore-menu-and-field-picker#field_picker_default_behavior), the field's column name in the data section of the Explore, or the field's column name in a [table chart](/looker/docs/legacy-table-options). See the table in the Liquid variable definitions section on this page to see which Liquid variables work with the `description` parameter.
...
The following variables can be used with `description` to affect the field picker, the data section of an Explore, and the column header in a [table chart](/looker/docs/legacy-table-options):
...
The other Liquid variables marked with **DE** in the [Liquid variable definitions table](/looker/docs/liquid-variable-reference#liquid_variable_definitions), such as Liquid variables that return a value based on a filter (like `_filters`) or require that a query run before the variable value can be determined (like `in_query`) won't change the description in the field picker or in the data section of an Explore. These Liquid variables will only affect the description shown when a user hovers over the field's column header in a [table chart](/looker/docs/legacy-table-options).

For examples of how to use Liquid in the `description` parameter, see the [`description`](/looker/docs/reference/param-field-description#using_liquid_variables_in_the_description_parameter) parameter documentation page.
...
When you are defining a [SQL-based derived table](/looker/docs/derived-tables#sql-based_derived_tables), if you use the [`_filters['view_name.field_name']`](/looker/docs/liquid-variable-reference#_filters) Liquid variable where the value is rendered in SQL and the filter returns a string value, you need to add single quotation marks around the output. You can do this by including the `sql_quote` Liquid filter.

For example, if you are using either of these Liquid variables in the [`sql`](/looker/docs/reference/param-view-sql-for-derived-table) parameter of a [`derived_table`](/looker/docs/reference/param-view-derived-table) parameter:
...
![Results in a data table for a query with the Created Month and Count Without Liquid fields selected.](/static/looker/docs/images/liquid-example-without-liquid-2318.png)
...
![Results in a data table for a query with the Created Month and Count with Liquid fields selected.](/static/looker/docs/images/liquid-example-with-liquid-2318.png)
