...
**Warning:** The default user attributes `landing_page`, `locale`, and `number_format` can be edited, but they shouldn't be deleted. If you delete one of these user attributes, [create a new user attribute](/looker/docs/admin-panel-users-user-attributes#creating_user_attributes) with the same name.
...
To define a user attribute, click the **Create User Attribute** button on the **User Attributes** page in the **Users** section of the **Admin** menu. Each user attribute has the following settings:
  ...
  * **Data Type** : This setting is used to check that valid values are assigned to users for this user attribute. The data type of the user attribute can be one of the following:
    ...
    * **String** : Select this option to create a user attribute that exactly matches one string value, such as a username. To use multiple string values or a [Looker filter expression](/looker/docs/filter-expressions#string) in the user attribute value, select the **String Filter (advanced)** option instead. If you want your user attribute to be treated as a literal string, be sure to include single quotes `'` in its syntax as in this example: `none '{{ _user_attributes['name_of_attribute'] }}'`
    * **Number** : Select this option to specify a single number, such as employee number. To use a range of numbers or a [Looker filter expression](/looker/docs/filter-expressions#number), use the **Number Filter (advanced)** instead.
    * **Date/Time** : Select this option to specify a single date or time, such as user's birth date. To use a range of dates or a [Looker filter expression](/looker/docs/filter-expressions#date_&_time), use the **Date/Time Filter (advanced)** instead.
    * **Relative URL** : Select this option to specify a relative URL, such as `/browse/boards/2`, that points to specific content like a board, a folder, or a Markdown file (such as a README or document file in a project) on your Looker instance. The `landing_page` user attribute, for example, has a data type of **Relative URL** and can be used to [specify a specific home page for a user or group](/looker/docs/admin-panel-general-homepage#setting_a_homepage_for_a_specific_user_or_group).
    * **String Filter (advanced)** : Select this option to allow multiple string values or a Looker filter expression in the user attribute. See the [Filter expressions](/looker/docs/filter-expressions#string) documentation page for a list of filter expressions you can use for strings.
    * **Number Filter (advanced)** : Select this option to allow a range of numeric values or a Looker filter expression in the user attribute. See the [Filter expressions](/looker/docs/filter-expressions#number) documentation page for a list of filter expressions you can use for numbers.
    * **Date/Time Filter (advanced)** : Select this option to allow a range of dates or a Looker filter expression in the user attribute. See the [Filter expressions](/looker/docs/filter-expressions#date-and-time) documentation page for a list of filter expressions you can use for date and time.

Use the **String Filter (advanced)** , **Number Filter (advanced)** , and **Date/Time Filter (advanced)** data types to enter values using [Looker filter expressions](/looker/docs/filter-expressions), which will return a range of values for a user attribute.
...
After defining a user attribute, you can assign a value for it to an [individual user](/looker/docs/admin-panel-users-users):
...
**Note:** For security purposes, using wildcards or a range of values in a user attribute is not supported with [access grants](/looker/docs/reference/param-model-access-grant). See the [`access_grant`](/looker/docs/reference/param-model-access-grant#values_listed_in_allowed_values_must_match_user_attribute_values_exactly) parameter documentation page for more information.

## Assigning values to user groups

You can assign a value for a user attribute to a [user group](/looker/docs/admin-panel-users-groups). From the **User Attributes** page of the **Admin** panel, select **Edit** to the right of the attribute you want to set. Then follow these steps:
...
The host, port, database, username, password, and schema of a [connection](/looker/docs/admin-panel-database-connections) can each be given the value of a user attribute. (The connection host field won't accept a user attribute that has a **User Access** level set to **Editable**.)
...
**Note:** If you set one or more connection parameters to a user attribute, you must [define separate persistent derived table (PDT) credentials](/looker/docs/connecting-to-your-db#pdt-overrides) to use PDTs in your LookML model. (One exception: The [BigQuery **Max Billing Gigabytes**](/looker/docs/db-config-google-bigquery#max_billing_gigabytes) setting can be set to a user attribute without requiring a separate PDT user.)

#### Configuration

Any connection can be configured to use user attributes from the **Connections** page in the **Admin** section of Looker. (See the [Admin settings - Connections](/looker/docs/admin-panel-database-connections) documentation page for information on the **Connections** page.) To create a new connection, click **Add Connection**. To configure an existing connection, click **Edit** next to the connection.

If an input can be set to a user attribute, Looker displays a **User attribute** button next to the input ![](/static/looker/docs/images/looker-user-attributes-icon-2408.png).
...
To reference a user attribute in the **Additional JDBC parameters** field, you use the same [Liquid templating syntax](/looker/docs/liquid-variable-reference) that is available in LookML. User attributes are made available through the [`_user_attributes`](/looker/docs/liquid-variable-reference#user_attributes) Liquid variable. For example, to reference a user attribute named `my_jdbc_param_attribute`, use the following syntax:
...
**Note:** If you set the database of a connection to a user attribute, you must [define separate persistent derived table (PDT) credentials](/looker/docs/connecting-to-your-db#pdt-overrides) to use PDTs in your LookML model.

### Data actions

[Data actions](/looker/docs/reference/param-field-action) can be configured to include certain user attributes with their JSON payload. Use this to send user-specific information along with the data, such as their credentials to perform an operation against a particular service.
...
You can configure a [custom action](/looker/docs/action-hub#building_a_custom_action) to include user attributes that restrict users from sending or scheduling Looker content to that action destination if they don't have a value defined for that user attribute.

#### Configuration

The `params` parameter in a [custom action](/looker/docs/action-hub#building_a_custom_action) represents the form fields that a Looker admin must configure on the action's enablement page from the **Actions** list in the **Admin** panel. In the `params` parameter of your action file, include:
...
A Looker admin must configure the action's form fields with the user attribute:
  ...
  2. Click the user attribute icon ![](/static/looker/docs/images/looker-user-attributes-icon-2408.png) for the appropriate field, and select the desired user attribute.

See the [Adding user attributes to custom actions](/looker/docs/action-hub#adding_user_attributes_to_custom_actions) section of the [Sharing data through an action hub](/looker/docs/action-hub) documentation page.

### Filters

Filters on [Explores](/looker/docs/filtering-and-limiting#user_attributes_adapt_filters_to_the_user), [Looks](/looker/docs/viewing-looks), and [dashboards](/looker/docs/filters-user-defined-dashboards#matches_a_user_attribute) can be set to a user attribute to customize the query based on the user who is running it.
...
If you'd like to do something more complex than a simple equality check for the filter, select **matches (advanced)** and reference the user attribute using a [Liquid variable](/looker/docs/liquid-variable-reference):
    
    
    {{ _user_attributes['name_of_attribute'] }}
    

For example, suppose you need to apply an `sf_` prefix to the value of the `salesforce_username` user attribute because that is how the values are stored in your database. To add the prefix to the user attribute value, use the [`_user_attributes`](/looker/docs/liquid-variable-reference#user_attributes) Liquid variable syntax:
...
You can use the same pattern to insert user attributes into LookML [dashboard filters](/looker/docs/reference/param-lookml-dashboard#default_value) and [dashboard element filters](/looker/docs/reference/param-lookml-dashboard-table-chart-legacy#filters).

### Scheduled dashboards and Looks

Dashboard and Look filters can be set on a per-schedule basis, including the option to use a user attribute. This lets you customize the data delivery results for each email recipient. You can customize [deliveries for content](/looker/docs/scheduling) that are sent as one-time deliveries and recurring deliveries.
...
Open the Scheduler for the [Look](/looker/docs/delivering-looks-explores#applying_filters_to_the_delivery) or [dashboard](/looker/docs/scheduling-and-sending-dashboards#using_user_attribute_filters):
...
[Access filters](/looker/docs/reference/param-explore-access-filter) provide a secure way to apply user-specific data restrictions. Defining one or more access filters for a LookML Explore enforces that the data returned from an Explore is filtered based on the user running the query. Thus, access filters provide an extra layer of restriction, ensuring the user can only see specific subsets of the data from a database connection.

**Note:** **SQL Note** : Access filters provide security at the _row_ level by inserting conditions in the SQL `WHERE` clause. User attributes can be leveraged in LookML in another way to provide security at the _column_ level, as described in the [Masking Sensitive Fields for Certain Users](https://community.looker.com/technical-tips-tricks-1021/masking-sensitive-fields-for-certain-users-30240) Community post.
...
For LookML projects, you can configure [Git authentication over HTTPS](/looker/docs/setting-up-git-connection#connecting_to_git_using_https). Projects that use HTTPS Git authentication have the option of leveraging user attributes to log in to individual developer's Git accounts when performing Git operations for the developer.

User attributes for Git account passwords must be [hidden](/looker/docs/admin-panel-users-user-attributes#creating_user_attributes). When creating the password attribute, select **Yes** under the **Hide Values** option and enter the Git provider URL in the **Domain Allowlist** field.
...
Access grants work like this:
  ...
  1. You define an access grant using the [`access_grant`](/looker/docs/reference/param-model-access-grant) parameter. As part of the definition, you associate the access grant with a user attribute. You also specify which user attribute values provide access to the access grant.
  2. Next, you use the `required_access_grants` parameter at the [Explore](/looker/docs/reference/param-explore-required-access-grants), [join](/looker/docs/reference/param-explore-join-required-access-grants), [view](/looker/docs/reference/param-view-required-access-grants), or [field](/looker/docs/reference/param-field-required-access-grants) level to restrict that structure to only users who have access to every access grant listed.
...
For more information about how to define access grants, see the [`access_grant`](/looker/docs/reference/param-model-access-grant) parameter documentation page.

**Warning:** For security purposes, using wildcards or a range of values in a user attribute is not supported with [access grants](/looker/docs/reference/param-model-access-grant). See the [`access_grant`](/looker/docs/reference/param-model-access-grant#values_listed_in_allowed_values_must_match_user_attribute_values_exactly) parameter documentation page for more information.

### Liquid variables

LookML enables the use of several different [Liquid variables](/looker/docs/liquid-variable-reference), which can be useful for more complex types of customized output. A user's attribute values can now be included in Liquid. The Liquid expression must use syntax that is appropriate to your database dialect.

You can see examples in the [Connection](/looker/docs/admin-panel-users-user-attributes#configuration) section of this documentation page, and in the [Using user attributes for dynamic schema and table name injection](/looker/docs/best-practices/how-to-create-a-dynamic-schema-with-user-attributes) Best Practices page.

### Google BigQuery data limits

If you use Google BigQuery as your database, Google charges you for each query based on the size of the query. To help prevent users from accidentally running too expensive a query, you can apply a user attribute in the [**Max Billing Gigabytes**](/looker/docs/db-config-google-bigquery#max_billing_gigabytes) setting in your [BigQuery connection](/looker/docs/connecting-to-your-db). The values that you supply in the user attribute should be the number of gigabytes that a user is allowed to pull in a single query.

### Embedded dashboards

You can limit the data that is displayed in [embedded Looks and dashboards](/looker/docs/private-embedding) by basing filter values on user attribute values. For more information, see the [Creating a Proof of Concept Embedded Dashboard (Powered by Looker) ](https://community.looker.com/embedding-looker-powered-by-looker-75/creating-a-proof-of-concept-embedded-dashboard-powered-by-looker-824) Community post.

### Localization

The user attributes `locale` and `number_format` can set the appearance of data, visualizations, and parts of the Looker user interface for specific users or user groups. See the [Localizing Looker](/looker/docs/supported-user-interface-languages) documentation page for more information.

## Testing user attributes and access filters

You can test the effects of your user attributes with Looker's [sudo](/looker/docs/admin-panel-users-users#impersonating-users) function. Admins (or users with both the `see_users` and `sudo` [permissions](/looker/docs/admin-panel-users-roles#permission_sets)) can sudo as another user to see their experience of Looker.

When you are in [Development Mode](/looker/docs/dev-mode-prod-mode), your changes are not visible to other users until you [deploy your changes to production](/looker/docs/version-control-and-deploying-changes#deploy_to_production). If you haven't deployed your changes for other users to see, you won't see your changes when you sudo as a different user.