# https://cloud.google.com/looker/docs/admin-panel-alerts-and-schedules-schedule

Depth: 3

As users navigate through folders, the [listing for each Look and dashboard](/looker/docs/finding-content#navigating_to_dashboards_and_looks_from_folders) indicates whether they have scheduled that content.

Looker administrators and users who have either the `manage_schedules` or `see_schedules` permission can also see the **Scheduler Plans** page by selecting **Schedules** in the **Alerts & Schedules** section of the **Admin** menu, which lists information about any [scheduled content](/looker/docs/scheduling) that is upcoming or ongoing.
...
To view a schedule's history, click the schedule's three-dot **Options** icon more_vert and select **History** to visit the [**Schedule History** page](/looker/docs/admin-panel-alerts-and-schedules-schedule-history), which is filtered for this scheduled plan.
