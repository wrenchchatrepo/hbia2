# https://cloud.google.com/looker/docs/advanced-deploy-mode
...
**Note:** For projects that have advanced deploy mode enabled, if the project has never been deployed to production, the initial deployment to production must be made with the deployment manager. You cannot use the API or the webhook for the initial deployment of a project that has advanced deploy mode enabled. After the initial deployment to production (even if it was initially deployed without advanced deployment mode), you can use the webhook or the API to deploy subsequent changes to production with advanced deploy mode.

## Enabling advanced deploy mode

To enable advanced deploy mode:

![](/static/looker/docs/images/enable-advanced-deploy-mode-718.png)

  1. In the Looker IDE, navigate to the [**Project Configuration**](/looker/docs/git-options#configuration) page by selecting the **Settings** icon from the icon menu and then selecting the **Configuration** tab.
...
When advanced deploy mode is enabled, [deploying to production](/looker/docs/version-control-and-deploying-changes#deploying_to_production) from Looker is no longer an option for developers. Instead, when the developer makes a commit, the [**Git button**](/looker/docs/version-control-and-deploying-changes#executing_git_commands_in_looker) will prompt them to merge their changes to the primary branch instead of prompting them to deploy to production.

Changes are deployed to production using a webhook, the API, or the deployment manager.

## Deployment manager

For projects with advanced deploy mode enabled, Looker developers who have the [`deploy` permission](/looker/docs/admin-panel-users-roles#deploy) can use the deployment manager to deploy a commit or tag to their Looker production environment.

You can access the deployment manager by selecting the **Deploy** icon from the icon menu.

![](/static/looker/docs/images/dev-deploy-mgr-2110.png)

The deployment manager shows all the commits and tags that were previously deployed using advanced deploy mode.

**Note:** As described in the Deploying a commit from the deployment manager section on this page, you can deploy a commit with or without a tag. If you deploy a commit with a tag, the deployment manager displays the _tag_ SHA, which is different from a _commit_ SHA. You can use your Looker project's [commit history](/looker/docs/git-command-reference#history) to view the commit SHA that is associated with a tag. Otherwise, if you deploy a commit without a tag, the deployment manager displays the commit SHA, which is the same as the commit SHA you can see on your Git provider's interface or in your Looker project's commit history.
...
If you choose a commit that has not been deployed previously, the deployment manager will display the **Deploy Commit** menu. To deploy a commit from the **Deploy Commit** menu, follow these steps:
  ...
  2. Specify a tag for the commit. A [Git tag](https://git-scm.com/book/en/v2/Git-Basics-Tagging) marks the significance of the commit in the repository's history, such as a release number or version name. Note the following about Git tags:
     ...
     * Git tags cannot contain spaces or certain special characters. See the [Git reference](https://git-scm.com/docs/git-check-ref-format) documentation for the rules for naming references in Git.
...
**Note:** When advanced deploy mode is enabled for a new project, you must use the deployment manager for the project's initial deployment to production. Once your project has been deployed to production (even if it was initially deployed without advanced deployment mode), you can use the deploy webhook to deploy subsequent changes to production using advanced deploy mode.

To set up the deploy webhook, you must first [add a webhook secret](/looker/docs/webhook-deploy-secret#adding_a_deploy_secret) for your Looker project from the **Project Configuration** page. Adding a webhook secret ensures that only authorized parties can trigger the deploy webhook.
...
To deploy with the API, the API user making the call will need to have [`deploy`](/looker/docs/admin-panel-users-roles#deploy) permission. See the [Looker API authentication](/looker/docs/api-auth) and [Getting started with the API](/looker/docs/api-getting-started) documentation pages for more information about authenticating and using the Looker API.

To deploy with the API, use the [`deploy_ref_to_production`](/looker/docs/reference/looker-api/latest/methods/Project/deploy_ref_to_production) endpoint. This endpoint can be called in several different ways. The following examples are for the HTTPS and SDK methods.

### HTTPS

**Note:** When you're using the following examples, be sure to replace the information in the angle brackets `< >` with the information specific to your instance address, LookML project, branch name, SHA, or tag.

To manually deploy by using the [`deploy_ref_to_production`](/looker/docs/reference/looker-api/latest/methods/Project/deploy_ref_to_production) API endpoint, see the following examples, which use the HTTPS method. For more information and examples of manually calling the API by using CURL requests, see the [How to Authenticate to the API](https://github.com/looker-open-source/looker-sdk-ruby/blob/master/authentication.md) GitHub readme, or use the [API Explorer](/looker/docs/api-explorer). You can install the API Explorer on your Looker instance from the [Looker Marketplace](/looker/docs/marketplace).

Use the following examples in an HTTPS request to deploy either the head of a branch or a specific commit SHA or tag via the [`deploy_ref_to_production`](/looker/docs/reference/looker-api/latest/methods/Project/deploy_ref_to_production) API endpoint:
...
**Note:** When you're using the following examples, be sure to replace the information in the angle brackets `< >` with the information that is specific to your LookML project, and branch name, SHA, or tag.

Alternatively, use one of Looker's [SDKs](/looker/docs/api-sdk) instead of making manual requests to the API. SDKs handle the details of authentication, parameter and response serialization, and other concerns.

Deploying with the [`deploy_ref_to_production`](/looker/docs/reference/looker-api/latest/methods/Project/deploy_ref_to_production) with the SDK method looks like the following:
