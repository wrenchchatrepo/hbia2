...
To see your project settings, [open your project](/looker/docs/accessing-project-files), then select the **Settings** icon from the Looker IDE icon menu.

![](/static/looker/docs/images/dev-project-settings-2414.png)
...
**Note:** You must be a Looker admin to change options on the **Project Configuration** page. Looker developers who are not admins can view the **Project Configuration** page but cannot change the options there.
...
The **Configuration** tab of the **Projects Settings** page opens the **Project Configuration** page. On the **Project Configuration** page, you can configure the following settings:
  ...
  * **Name** : The name of your project. You can rename your project by editing the text and clicking the **Save Project Configuration** button. See the [Accessing and editing project information](/looker/docs/manage-projects#renaming_a_project) documentation page for more details.
  ...
  * **Code Quality** : Determines whether to require developers to successfully run the [LookML Validator](/looker/docs/lookml-validation#validating_your_changes) on the project before committing any changes to the project. **Code Quality** has the following options:
  ...
  * **Require data tests to pass before deploying this project to production** : If the LookML project has one or more [`test`](/looker/docs/reference/param-model-test) parameters, this option requires developers to run the data tests before deploying any changes. If the data tests pass, the IDE will allow the developer to deploy changes to production. See the [`test`](/looker/docs/reference/param-model-test) parameter documentation page for information on setting up data tests in your LookML project. See the [Using version control and deploying](/looker/docs/version-control-and-deploying-changes#running_data_tests) documentation page for information on running data tests on your project. By default, the **Require data tests to pass before deploying this project to production** option is enabled for new LookML projects.

  * **Git Integration** : Specifies the levels of integration with your Git provider. See Git integration options for details.

  * **Enable Advanced Deploy Mode** : A setting that, when enabled, lets users deploy any SHA, tag, or branch to production. See the [Advanced deploy mode](/looker/docs/advanced-deploy-mode) documentation page for more information about using version control with **Advanced Deploy Mode** enabled.

  * **Webhook Deploy Secret** : Sets up authentication for deploying changes to production on your Looker instance. See the [Configuring the webhook deploy secret](/looker/docs/webhook-deploy-secret#webhook_deploy_secret) documentation page for details.

  * **Reset Git Connection** : This button opens the **Configure Git** window, where you can update the [connection settings for your Git repository](/looker/docs/setting-up-git-connection#integrating_looker_with_git).

> Resetting your Git connection will preserve Git history for the main branch. It will also preserve the history of each Looker developer's personal branch once they sync their dev mode. To preserve history for all branches, see the [Migrating to a new Git repository](/looker/docs/best-practices/how-to-migrate-to-a-new-git-repo) Best Practices page.

  * **Delete Project** : This button [deletes the project](/looker/docs/manage-projects#deleting_a_project), removing all LookML from the project in all development and production environments across your Looker instance.
...
For existing projects, consider doing the following before changing the Git production branch name:
  ...
  * If your project uses [integrated pull requests](/looker/docs/git-options#integrating_pull_requests_for_your_project), finalize and merge any open pull requests, as appropriate.
...
![](/static/looker/docs/images/dev-git-default-branch-2500.png)
...
The **Import Credentials** section is where you can manage authentication credentials for private remote repositories. See the [Importing files from other projects ](/looker/docs/importing-projects#configuring_authentication_credentials_for_private_remote_repositories) documentation page for details.

### Branch Management

On the **Branch Management** tab of the **Projects Settings** page, you can see all the Git branches associated with the project. See the [Using version control and deploying](/looker/docs/version-control-and-deploying-changes#managing_git_branches) documentation page for details.

## Git integration options

**Deprecated:** Looker-hosted Git repositories are deprecated. See the [Migrating to a new Git repository](/looker/docs/best-practices/how-to-migrate-to-a-new-git-repo) Best Practices page for instructions on moving your project to a new Git repository. To check if your Git repository is Looker-hosted, go to the bottom of the **Branch Management** tab in the project's settings panel to see the **Remote Origin URL**. Looker-hosted repos have a URL starting with `git@github.com:llooker`.

Once you [set up your Git connection](/looker/docs/setting-up-git-connection#integrating_looker_with_git), Looker will use your Git provider for managing your LookML source files, as described on the [Using version control and deploying](/looker/docs/version-control-and-deploying-changes) documentation page.
...
![](/static/looker/docs/images/dev-git-show-links-file-718.png)
...
Instead of allowing Looker developers to merge their development branch into the Looker production branch, you can set up your project with either the **Pull Requests Recommended** or the **Pull Requests Required** option:
  ...
  * **Pull Requests Recommended** : After a developer commits changes to their development branch, the Git button in the Looker IDE prompts the developer to open a pull request to merge their development branch into the production branch. The developer can then open a pull request for other Looker developers to review and approve from the Git provider's web interface. Or the developer can instead use the [**Deploy to Production**](/looker/docs/git-command-reference#deploy_prod) option from the [Git Actions panel](/looker/docs/git-command-reference) to skip creating a pull request and deploy their changes to production. (The **Deploy to Production** option is not available if the project is enabled with [advanced deploy mode](/looker/docs/advanced-deploy-mode).)
...
Here are some additional notes on using pull requests with Looker:
  ...
  * If the [IP Allowlist](/looker/docs/admin-panel-server-ip-allowlist) feature is enabled on your instance, to integrate pull requests with any LookML projects you will need to add to the allowlist the range of IP addresses from which your Git provider makes outbound requests. For example, current GitHub IP addresses are listed in the [GitHub changelog](https://github.blog/changelog/2019-04-09-webhooks-ip-changes/). IPs are subject to change and will be different for other Git providers.

  * If you have enabled [advanced deploy mode](/looker/docs/advanced-deploy-mode), it is not necessary to configure a webhook in the Setting Up Your Project with Integrated Pull Requests section, since advanced deploy mode separates the merge and deployment functions.

  * If a Looker developer has issued a pull request that you would like to revert, see the [How to revert pull requests from Looker via GitHub](https://community.looker.com/general-looker-administration-35/how-to-revert-pull-requests-from-looker-via-github-3-20-818) Community post for more information.

  * Git pull requests can make it possible to use a staging instance for Looker, so you can have a staging instance and a production instance, with pull requests enabled on the staging instance. All development and code review can be done in the staging environment, and the reviewed code can then be deployed to the production instance. To set this up, see the [Git Workflow Using One Repository Across Multiple Instances — Development, Staging, and Production](https://community.looker.com/technical-tips-tricks-1021/git-workflow-using-one-repository-across-multiple-instances-development-staging-and-production-30246) Community post.
...
![](/static/looker/docs/images/dev-project-open-pr-718-2.png)
  ...
  3. Optionally, if you want to set up an automatic deploy webhook on your Git provider's interface, copy the webhook information and paste it into a text file. If your project is configured to use [advanced deploy mode](/looker/docs/advanced-deploy-mode), you can skip this step. You can also decide later and come back to the project settings to get the webhook information.
  4. Optionally, you can set up a [webhook deploy secret](/looker/docs/webhook-deploy-secret#webhook_deploy_secret) to authenticate an automatic deploy webhook from your Git provider or an [advanced deploy mode webhook](/looker/docs/advanced-deploy-mode#deploying_with_webhooks). To create a webhook secret, click **Set Webhook Secret**. Copy the deploy secret and paste it into a text file to use when you add the webhook to your Git provider's interface. You can also decide later and come back to the project settings to add a deploy secret. See the [Configuring the webhook deploy secret](/looker/docs/webhook-deploy-secret#webhook_deploy_secret) documentation page for more information.
...
Once you have set up your Looker project to use pull requests, do one of the following to set up how commits are deployed to your production environment:
  ...
  * To manually specify which branches or commits are deployed, enable advanced deploy mode. See the [Advanced deploy mode](/looker/docs/advanced-deploy-mode) documentation page for information on enabling and managing advanced deploy mode.

### Adding a webhook to your Git provider

For Looker projects enabled for [pull requests](/looker/docs/git-options#integrating_pull_requests_for_your_project), you can set up a deploy webhook on your Git provider's interface. This webhook will trigger Looker to deploy the latest commit from the production branch whenever you merge a pull request on the Git provider's interface.

> In most cases, you don't want to set up an automatic deployment webhook if your project is configured to use [advanced deploy mode](/looker/docs/advanced-deploy-mode). Advanced deploy mode lets you select the commit and branch that you want to deploy, so in most cases setting up an automatic deploy webhook would nullify the features of advanced deploy mode.
...
![](/static/looker/docs/images/dev-pr-webhook-info-720.png)
...
From the Looker project settings, you can also get a webhook deploy secret to authenticate your project's Git integrations with your Git provider. See the [Configuring the webhook deploy secret](/looker/docs/webhook-deploy-secret#webhook_deploy_secret) documentation page for more information.
...
![](/static/looker/docs/images/dev-project-open-pr-add-webhook-718-v2.png)
  ...
  4. Optionally, you can add a webhook deploy secret that authenticates your project's Git integrations with your Git provider. Copy the deploy secret from the project settings of your LookML project and paste the secret into the **Secret** field in the Git provider's interface. See the [Configuring the webhook deploy secret](/looker/docs/webhook-deploy-secret#webhook_deploy_secret) documentation page for more information.
...
If your LookML project also requires a [webhook deploy secret](/looker/docs/webhook-deploy-secret) for pushing changes from your main branch to your production branch, you will receive one of the following errors when you sync the production branch with the repository's main branch using the method described in the preceding section, [Using a deploy webhook to pull from a remote Git repository](/looker/docs/git-options#using_a_deploy_webhook_to_pull_from_a_remote_git_repo):
...
![](/static/looker/docs/images/dev-project-mergetype-718.png)
