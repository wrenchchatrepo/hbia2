# https://cloud.google.com/looker/docs/db-config-cloudera-impala
...
**Note:** Looker does not support connections to the Impala dialect called _Cloudera Impala_ (with no version number and without a native driver). Queries on connections to the _Cloudera Impala_ dialect will return an error.
...
Looker connects to databases through a JDBC connection. For Impala databases, Looker connects by default to the server that is running the `impalad` daemon on port 21050. For more information, see the [Configuring Impala to work with JDBC](http://www.cloudera.com/documentation/enterprise/latest/topics/impala_jdbc.html) section of the documentation on the Cloudera website.
...
For more information about configuring Impala databases to work with JDBC, see the [documentation on the external Cloudera website](http://www.cloudera.com/content/cloudera/en/documentation/core/latest/topics/impala_jdbc.html#jdbc_connect_unique_1).
...
Usually, Kerberos authentication with Cloudera environments is handled through [Apache Sentry](https://sentry.incubator.apache.org/). See the [Cloudera documentation](http://www.cloudera.com/content/www/en-us/documentation/enterprise/latest/topics/sg_sentry_service_config.html) for more details.
...
##### Java Cryptography Extension
  ...
  1. Download and install the Java Cryptography Extension (JCE) for Java 8 from [the Oracle website](http://www.oracle.com/technetwork/java/javase/downloads/jce8-download-2133166.html).
...
Fill out the connection details as follows (see the [Connecting Looker to your database](/looker/docs/connecting-to-your-db) documentation page for more information):
...
> See [the Cloudera documentation](http://www.cloudera.com/content/cloudera/en/documentation/core/latest/topics/sg_hive_encryption.html#concept_rqh_sff_cm_unique_) for more details about how to form the correct JDBC strings for Impala databases.
...
#### Debugging
  ...
  * [Cloudera's documentation about debugging Impala authentication issues.](http://www.cloudera.com/content/www/en-us/documentation/enterprise/latest/topics/cm_sg_sec_troubleshooting.html)
  * When you add debugging to the configuration, the extra debugging information will wind up in `looker/logs/looker.log`.

#### Resources

  * [Enabling Kerberos authentication for Impala (Cloudera documentation)](http://www.cloudera.com/content/www/en-us/documentation/enterprise/latest/topics/impala_kerberos.html)
  * [kinit man page](http://linux.die.net/man/1/kinit)
  * [klist man page](http://linux.die.net/man/1/klist)
  * [gss-jaas.conf documentation](https://docs.oracle.com/javase/8/docs/technotes/guides/security/jgss/tutorials/LoginConfigFile.html)
...
The user connecting to the scratch schema for [persistent derived tables (PDTs)](/looker/docs/derived-tables#persistent_derived_table) must have read/write permissions.
...
Cloudera Impala with Native Driver supports the following features as of Looker 25.2:
...
Cloudera Impala 3.1+ supports the following features as of Looker 25.2:
...
Cloudera Impala 3.1+ with Native Driver supports the following features as of Looker 25.2:
