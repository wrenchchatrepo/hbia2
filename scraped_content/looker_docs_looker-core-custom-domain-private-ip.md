# https://cloud.google.com/looker/docs/looker-core-custom-domain-private-ip
...
This documentation page describes how to set up a custom domain and set up access to a Looker (Google Cloud core) instance that meets the following criteria:
  ...
  * Traffic to the instance originates either from different regions than the instance is located in or through hybrid networking.
...
To get the permissions that you need to set up access to a private IP custom domain, ask your administrator to grant you the following IAM roles on the project the instance resides in: 
  ...
  * [Compute Load Balancer Admin ](https://cloud.google.com/iam/docs/understanding-roles#compute.loadBalancerAdmin) (`roles/compute.loadBalancerAdmin`)
...
**Note:** In a proof of concept scenario, you can set up one reverse proxy server and avoid setting up a load balancer.
...
![A Google Cloud network showing secure access to a Looker \(Google Cloud core\) instance using Cloud Router, an internal load balancer, and Private Services Access.](/static/looker/docs/images/private-ip-proxies.png)
...
**Note:** Because you will be using a load balancer, later in this process you will update the A record in the Cloud DNS private zone to map to the load balancer's virtual IP address.
...
**Note:** This configuration example uses NGNIX proxy servers on RHEL and is provided as a working starting point for your custom proxy configuration. This configuration has been tested with NGINX release 1.22.1 installed on Red Hat Enterprise Linux release 8.9.
...
The following example uses NGINX release 1.22.1 and Red Hat Enterprise Linux release 8.9 (Ootpa). To check what versions of NGNIX and Red Hat your VMs are using, run the following commands for each VM.
  ...
  1. First, [connect to the VM](/compute/docs/connect/ssh-internal-ip).
...
To set up each proxy server, use the following instructions for each of the two VMs that you created.
  ...
  1. [Connect to the VM](/compute/docs/connect/ssh-internal-ip).
...
**Note:** If you saved the certificate files in a different path, make sure that the path is reflected in the `/etc/nginx/snippets/self-signed.conf` configuration file.
...
**Note:** If you implement `ssl-params.conf`, uncomment `include snippets/ssl-params.conf` in the `nginx.conf` file.
...
**Note:** Disable strict transport security for now. If you want to enable strict transport security, and you understand the implications of doing so, you can uncomment the line beginning with `add_header Strict-Transport-Security`.
...
**Note:** The following configuration has been tested with Apache release 2.4.6 installed on Red Hat Enterprise Linux release 7.9.
...
This examples uses [zonal network endpoint groups (NEGs) with `GCE_VM_IP` endpoints](/load-balancing/docs/negs/zonal-neg-concepts#gce-vm-ip) as backends of the internal passthrough Network Load Balancer. If you prefer to use instance group based backends, follow the documentation available on the [Set up an internal passthrough Network Load Balancer with VM instance group backends](/load-balancing/docs/internal/setting-up-internal) documentation page.

  1. Create one separate zonal NEG for each compute zone where you plan to deploy proxy servers. For example, if you want to deploy proxy servers in each of the three compute zones of the region where Looker (Google Cloud core) is deployed, then create three zonal NEGs. See the [Quotas and limits](/load-balancing/docs/quotas#endpoints_per_neg) documentation page to check how many endpoints are supported per zonal NEG.
...
Repeat this step for any additional zone where you will deploy a proxy server VM.
  ...
  2. [Add each proxy server](/load-balancing/docs/negs/setting-up-zonal-negs#adding-endpoints) to the NEG in the same zone:
...
Repeat this step until each of the proxy server VM is added to a NEG as an endpoint.
  ...
  3. [Create a regional health check](/load-balancing/docs/health-checks#create-hc) that will be used by the internal load balancer. Use the [`compute health-checks create`](/sdk/gcloud/reference/compute/health-checks/create) command:
...
Replace the following:
     ...
     * `PORT_SPECIFICATION`: Defines the port specification using one of the [Port specification flags](/load-balancing/docs/health-checks#hc-port-flags).
     * `ADDITIONAL_FLAGS`: Other flags for specifying ports and options specific to the `PROTOCOL`. See [Additional flags for HTTP, HTTPS, and HTTP/2 health checks](/load-balancing/docs/health-checks#optional-flags-hc-protocol-http), [Additional flags for SSL and TCP health checks](/load-balancing/docs/health-checks#optional-flags-hc-protocol-ssl-tcp), or [Additional flag for gRPC health checks](/load-balancing/docs/health-checks#optional-flags-hc-protocol-grpc).
  4. [Create the backend service](/sdk/gcloud/reference/compute/backend-services/create):
...
Repeat this step for the additional NEG you created.
  ...
  6. [Reserve an internal IP address](/vpc/docs/reserve-static-internal-ip-address) in the VPC inside the IP range of the subnet where the proxy instances are connected. This will be the Virtual IP address (VIP) of the internal load balancer. Reserving the address will make sure the IP won't be used by any other object. To reserve the internal IP address, use the [`compute addresses create` command](/sdk/gcloud/reference/compute/addresses/create):
...
Replace the following:
  ...
  7. [Create](/sdk/gcloud/reference/compute/forwarding-rules/create) the forwarding rule and associate it with the backend service and the VIP:
...
For health checks to work, create ingress firewall rules applicable to the proxy VM being load balanced to allow traffic from [health check prober IP ranges](/load-balancing/docs/health-check-concepts#ip-ranges).

Additionally, create an [ingress firewall rule](/firewall/docs/using-firewalls#creating_firewall_rules) to allow traffic from on premises or multicloud environments to gain access the load balancer backend service.
...
### Troubleshooting
  ...
  * To troubleshoot certificate errors, see the [Troubleshoot SSL certificates](/load-balancing/docs/ssl-certificates/troubleshooting) documentation page. For Google-managed certificates, be sure to [explicitly authorize](/load-balancing/docs/ssl-certificates/google-managed-certs#caa) the Certificate Authority that you want to allow to issue your Google-managed certificate.
