# https://cloud.google.com/looker/docs/scatter-options
...
![](/static/looker/docs/images/scatter-menu-2106.png)
...
You can specify how [series](/looker/docs/creating-visualizations#series-def) are clustered visually on a chart. Not all of the following options apply to all chart types:
...
![](/static/looker/docs/images/grid-options-none-scatter-2106.png)

![](/static/looker/docs/images/grid-options-pivot-scatter-2106.png)

![](/static/looker/docs/images/grid-options-row-scatter-2106.png)
...
When used with [dashboards](/looker/docs/viewing-dashboards), grid layout [becomes responsive](/looker/docs/editing-user-defined-dashboards#tile_size_and_grid_layout) to the dashboard tile size and shape, if **Number of Charts per Row** is not set.
...
**Note:** The **Swap X and Y** option is not available when the **Cluster Points** option has been enabled.
...
![](/static/looker/docs/images/scatterplot-vis-cluster-points-enabled-2220.png)
...
![](/static/looker/docs/images/scatterplot-vis-cluster-points-zoomed-2220.png)
...
![Scatterplot chart with Number of Orders on the x-axis and Average Sale Price \(USD\) on the y-axis.](/static/looker/docs/images/scatterplot-vis-quadrants-enabled-2220.png)
...
![](/static/looker/docs/images/scatterplot-vis-quadrant-customization-2220.png)
...
![](/static/looker/docs/images/size-points-by-field-2106.png)
...
![](/static/looker/docs/images/plot-size-by-2106.png)
...
![](/static/looker/docs/images/vis-customizations-line-area-2106.png)
...
You can select a field from the drop-down menu that will be used as the value label for the data points in the visualization. The drop-down menu is populated with the dimensions in the query that are not [hidden from the visualization](/looker/docs/creating-visualizations#hiding_fields_from_visualizations). The default value for the value labels is the measure value that is being plotted in the chart.
...
![Scatterplot chart with Category on the x-axis and Average of Sale Price on the y-axis.](/static/looker/docs/images/scatter-value-label-default-2308.png)
...
![The data points on the scatterplot chart are labeled with values from the Products Category dimension.](/static/looker/docs/images/scatter-value-label-custom-2308.png)
...
You can define the colors of the value labels. This field takes a comma-separated list of one or more color values. The color values can be formatted as RGB hex strings, such as `#2ca6cd`, or as [CSS color names](https://www.w3schools.com/cssref/css_colors.asp), such as `mediumblue`.
...
You can set the font size of value labels using any valid [CSS size](https://developer.mozilla.org/en-US/Web/CSS/length). For example, you can specify a number of pixels, such as `10px` or `12px`.
...
MS Excel's complete guide details how to specify these formats [in their documentation](https://support.office.com/en-us/article/review-guidelines-for-customizing-a-number-format-c0a1d1fa-d3f4-4018-96b7-9c9354dd99f5). However, color formatting is not currently supported in Looker.

> For Looker developers: The formatting used in the **Value Format** field is the same as formatting that is used with the [`value_format` LookML parameter](/looker/docs/reference/param-field-value-format), except that the `value_format` parameter requires the formatting string to be enclosed in double quotes. In the **Value Format** field, the formatting string is not enclosed in quotes.

The [Adding custom formatting to numeric fields](/looker/docs/custom-formatting#custom_formatting_examples) documentation page provides more information on how to use custom formatting.
...
This option is available only for [Cartesian charts](/looker/docs/visualization-types#cartesian_charts) (column, bar, scatterplot, line, area, waterfall, or boxplot charts).

This option defaults to enabled. When it's enabled, you can zoom on a visualization in a dashboard, a Look, or an Explore.

See the [Viewing dashboards](/looker/docs/viewing-dashboards#zooming_on_cartesian_charts) or [Viewing Looks in Looker](/looker/docs/viewing-looks#zooming_on_cartesian_charts) documentation page for information about how to use the zoom feature.
...
You can specify how x-axis value labels appear for charts with a time dimension on the x-axis. This parameter accepts time formatting syntax, as shown in the following list. See [Time Formatting for Looker Charts](/looker/docs/time-formatting-for-charts) for all formatting options.
...
See the [Viewing dashboards](/looker/docs/viewing-dashboards#zooming_on_cartesian_charts) or [Viewing Looks](/looker/docs/viewing-looks#zooming_on_cartesian_charts) documentation page for information about how to use the zoom feature.
...
![](/static/looker/docs/images/explore-vis-y-scatter-example-2106.png)

On the left and right side of the chart, you can have one or more y-axes, each associated with one or more data series. Each data series is based on a measure, even if the measure has been pivoted. [Table calculations](/looker/docs/table-calculations) that use a measure are listed as a data series and can be moved to another axis.
...
Each measure goes on one axis. If you have a measure pivoted, all of its pivoted values go on the same axis. To specify the axis for a particular measure for each pivot, use [filtered measures](/looker/docs/reference/param-field-filters) instead. Create a filtered measure for each possible value (and maybe one more for any unanticipated values). Then you can specify which axis to use for each of your new measures.

Charts with the [**Series Positioning**](/looker/docs/column-options#series_positioning) option set to **Grouped** or **Overlay** can have multiple y-axes. Any **Stacked** or **Stacked Percentage** charts will have one stacked data series and one y-axis.
...
![A user drags the Total Sale Price field from the Left Axes section on the Y visualization options tab to the Right Axes section.](/static/looker/docs/images/explore-vis-left-right-axes-2106.png)
...
Excel's complete guide explains how to specify these formats [in the Excel documentation](https://support.office.com/en-us/article/review-guidelines-for-customizing-a-number-format-c0a1d1fa-d3f4-4018-96b7-9c9354dd99f5). At this time, date formatting and color formatting are not supported in Looker.

Some of the most common formatting options are shown on the [Adding custom formatting to numeric fields](/looker/docs/custom-formatting) documentation page.
...
![](/static/looker/docs/images/reference-line-scatter-2106.png)
...
Click the color box to reveal a palette picker that you can scroll through to choose a color. To choose a custom color, select the **Custom** tab on the palette picker and use the color picker that appears, or enter a [hex string](https://www.w3schools.com/colors/colors_hexadecimal.asp) or [CSS named color string](https://www.w3schools.com/cssref/css_colors.asp) into the color value box.

#### Value Format (for a reference line)

You can add custom formatting to the value shown in the reference line label.

You can use Excel-style formatting to format the label value. The [Adding custom formatting to numeric fields](/looker/docs/custom-formatting) documentation page provides more information on how to use custom formatting.
...
![](/static/looker/docs/images/trendline-scatter-2106.png)
...
![](/static/looker/docs/images/trend-label-scatter-2106.png)
...
You can specify the type of label to apply to the trend line:
  ...
  * **R^2** : Applies a [coefficient of determination](https://en.wikipedia.org/wiki/Coefficient_of_determination) label — also known as R squared — to the trend line. The R^2 label indicates the proportion of the variance in the **Series Index** data. It helps you understand how well the trend line matches the data, with 0 meaning no fit, 1 meaning perfect fit, and values in between indicating partial fit. This option will only work when **Trend Type** is set to Linear.
