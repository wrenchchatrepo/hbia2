# https://cloud.google.com/looker/docs/scheduling-and-sending-dashboards

Depth: 3

This page is about scheduling and sending a [dashboard](/looker/docs/viewing-dashboards). To learn about scheduling Looks or Explores, visit the [Using the Looker Scheduler to deliver content](/looker/docs/scheduling) documentation page.  
...
Depending on how your Looker admin has set up your [permissions](/looker/docs/admin-panel-users-roles#permissions_list) for data delivery, you may be able to deliver your content to one or more of Looker's native delivery destinations:
...
You may also be able to schedule a data delivery to a third-party service that is integrated with Looker, such as [Slack](https://slack.com/).
...
**Note:** If you are a Looker developer, you must turn off [Development Mode](/looker/docs/dev-mode-prod-mode#switching_in_and_out_of_development_mode) to schedule or send. Deliveries are executed using [Production Mode](/looker/docs/dev-mode-prod-mode) LookML.

Make sure the dashboard is not in [edit mode](/looker/docs/editing-user-defined-dashboards#entering_edit_mode_in_dashboards). Select the **Dashboard actions** three-dot menu in the upper right of the dashboard and choose **Schedule delivery**. If you do not see the **Schedule delivery** option, talk to your Looker admin about your assigned permissions.

![](/static/looker/docs/images/scheduling-dboard-beta-2408.png)
...
![The existing schedule window showing a scheduled dashboard delivery called Order Analysis.](/static/looker/docs/images/existing-dboard-beta-schedules-2220.png)
...
![The schedule and send window showing options to customize a dashboard delivery's name, recurrence, destination, and format.](/static/looker/docs/images/schedule-send-window-2220.png)
...
You can also start a delivery of a dashboard from [the folder](/looker/docs/finding-content#navigating_to_dashboards_and_looks_from_folders) where the dashboard is located.
...
![](/static/looker/docs/images/dash-beta-edit-delivery-name-2310.png)
...
![](/static/looker/docs/images/hourly-dash-beta-2100.png)
...
If your LookML developer has configured [datagroups](/looker/docs/reference/param-model-datagroup), you can schedule delivery of a dashboard to occur after a datagroup has been triggered, has managed the cache, and has [rebuilt relevant PDTs](/looker/docs/caching-and-datagroups#using_a_datagroup_for_a_pdt).
...
Webhooks are a way to trigger exchanges between internet-based services. With a web service like [Zapier](https://zapier.com), webhooks can let Looker data be delivered to a wide range of applications; for example, you may be able to schedule periodic delivery of a dashboard to a webhook.
...
The exact procedure differs depending on what web service and destination application you're using to deliver the data. See [the Scheduling Looks and Dashboards Using Webhooks](https://community.looker.com/looker-api-77/scheduling-looks-and-dashboards-using-webhooks-3-46-2593) Community post for guidelines on setting up another application to receive the webhook data from Looker.
...
Check out [the Scheduling (unlimited) Results to S3](https://community.looker.com/looker-api-77/scheduling-unlimited-results-to-s3-3785) Community post for more details about delivering data using an Amazon S3 bucket.
...
**Note:** To receive SFTP deliveries from Looker, be sure your network admin has added [Looker's IP addresses](/looker/docs/enabling-secure-db-access#option_1:_ip_address_whitelist) to your SFTP server's IP allowlist or inbound traffic rules.
...
**Note:** This example URL ends with the `/` character. This indicates that the file will be saved in a folder named `In`. If you leave out the trailing `/`, the file will be saved in the `Marketing` folder, and "`In`" will be prepended to the filename. You must have write access to the folder in which you want to create the file.
...
**Note:** If you receive the error `Java::JavaLang::NullPointerExceptions` when delivering data to an SFTP server, see the note in the [Delivery options for SFTP servers section](/looker/docs/delivering-looks-explores#delivery_options_for_sftp) of the [Delivering Looks and Explores documentation page](/looker/docs/delivering-looks-explores) for troubleshooting this Java error.

### Integrated service destinations

You can schedule or send dashboard deliveries to a service that is integrated with Looker, such as Slack, Dropbox, Google Drive, and others. Integrations must be [enabled by your Looker admin](/looker/docs/admin-panel-platform-actions). Looker admins must specifically give permissions for [users](/looker/docs/admin-panel-users-roles#send_to_integration) and [embed users](/looker/docs/single-sign-on-embedding#send_to_integration) to send and schedule data to third-party integrations.
...
---|---|---|---|---|---|---  
Azure Storage | Send and store a data file on Azure Storage. | [View README on GitHub](https://github.com/looker/actions/blob/master/src/actions/azure/README.md) | [View Community article](https://www.googlecloudcommunity.com/gc/Exploring-Curating-Data/Looker-Actions-Azure-Storage/td-p/565929) | None | No | No  
DigitalOcean Spaces | Send to and store a data file in DigitalOcean Storage. | [View README on GitHub](https://github.com/looker/actions/blob/master/src/actions/digitalocean/README.md) | [View Community article](https://www.googlecloudcommunity.com/gc/Exploring-Curating-Data/Looker-Actions-Digital-Ocean-Storage/td-p/583736) | None | No | No  
Dropbox | Send and store a data file on Dropbox. | No README available | [View documentation](/looker/docs/delivering-dropbox) | None | Yes | No  
Google Cloud Storage | Write data files to a Google Cloud Storage bucket. | [View README on GitHub](https://github.com/looker-open-source/actions/blob/master/src/actions/google/gcs/README.md) | [View Community article](https://www.googlecloudcommunity.com/gc/Administering-Looker/Looker-Actions-Google-Cloud-Storage/td-p/591972) | None | No | No  
Google Drive | Send data to Google Drive. | No README available | [View documentation](/looker/docs/best-practices/how-to-use-the-google-drive-integration) | None | Yes | No  
SendGrid | Send data and schedule results to send to a email address using SendGrid's API. | [View README on GitHub](https://github.com/looker/actions/blob/master/src/actions/sendgrid/README.md) | [View Community article](https://www.googlecloudcommunity.com/gc/Exploring-Curating-Data/Looker-Actions-Sendgrid/td-p/591971) | None | No | No  
...
Teams — Incoming Webhook | Send data to Microsoft Teams using an incoming webhook. | [View README on GitHub](https://github.com/looker/actions/blob/master/src/actions/teams/README.md) | See README | None | No | No  
...
**Note:** If you have selected email as a destination and your Looker admin has set your Looker instance's [emailed data policy](/looker/docs/admin-panel-alerts-and-schedules-scheduled-emails#emailed_data_policy) to [**Send Link Only**](/looker/docs/admin-panel-alerts-and-schedules-scheduled-emails#send_link_only), the option to select a data format is not available.
...
The **Run schedule as recipient** option lets users specify a list of recipients who will each receive the content delivery as if each one of them had run the query, based on each person's permissions. This means that each user's [access filters](/looker/docs/reference/param-explore-access-filter) and [user attributes](/looker/docs/admin-panel-users-user-attributes) will be applied to the data that is included in each email.

If a user is a Looker admin, the **Run schedule as recipient** option lets the admin specify a list of users who will each receive the content delivery as if each one of them ran the query, based on each person's permissions. Non-admin users may also enter their own email address to receive a delivery as if they had queried the content, based on their permissions. This means that each user's [access filters](/looker/docs/reference/param-explore-access-filter) and [user attributes](/looker/docs/admin-panel-users-user-attributes) will be applied to the data that is included in each email.
...
![An example schedule showing the External count of email addresses for recipients who are not Looker users.](/static/looker/docs/images/schedule-send-window-external-2220.png)
...
If your Looker admin has set your Looker instance's [emailed data policy](/looker/docs/admin-panel-alerts-and-schedules-scheduled-emails#emailed_data_policy) to [**Send Links and Data**](/looker/docs/admin-panel-alerts-and-schedules-scheduled-emails#send_links_and_data), the schedule and send window displays the **Include links** option. Select this option to include in the data delivery emails a **View full dashboard** link that goes to the dashboard in Looker. Dashboard links do not include any [`hide_filter` parameters](/looker/docs/filters-user-defined-dashboards#hiding_dashboard_filters).
...
To use this option, a Looker admin or a user with either the `schedule_look_emails` or the `create_alerts` permission first must specify allowed domains for the custom link and select the format of the custom link URL in the [**Embed**](/looker/docs/admin-panel-platform-embed#alerts_and_scheduling_custom_url) page in the **Admin** menu.

#### Domain

Select the URL domain from the list of domains configured on the [allowlist](/looker/docs/admin-panel-platform-embed#embed_domain_for_schedules_and_alerts_allowlist). For example, `app.customer.com`.

#### Path

The **Path** option is displayed only if **User defined URL** is selected in the **Select content path** field on the [**Embed**](/looker/docs/admin-panel-platform-embed#alerts_and_scheduling_custom_url) page in the **Admin** menu.
...
Use this field to enter new link text, which will replace the text **View full dashboard** in email deliveries. This field will display any text that is entered in the **URL label** field in the [**Embed**](/looker/docs/admin-panel-platform-embed#alerts_and_scheduling_custom_url) page in the **Admin** menu by default and can be changed in this field.
...
Choose the **With visualizations options applied** option to apply some of the visualization settings from the dashboard tiles to your dashboard delivery. This causes the files in your delivery to appear similar to [table charts](/looker/docs/legacy-table-options). Any of the following settings in the [**Plot**](/looker/docs/legacy-table-options#plot_menu_options), [**Series**](/looker/docs/legacy-table-options#series_menu_options), and [**Formatting**](/looker/docs/legacy-table-options#formatting_menu_options) menus that are configured for a visualization will be applied to the data delivery:
  ...
  * [Custom labels](/looker/docs/legacy-table-options#customizations) for each column
...
This field is available only for CSV ZIP file formats of dashboard deliveries. It contains two options: **Formatted** or **Unformatted**. You can choose one or the other.
  ...
  * Select **Formatted** if you want the data to appear similar to how it appears in the [**Explore** experience](/looker/docs/viewing-and-interacting-with-explores#the_explore_page) in Looker, although some features (such as [linking](/looker/docs/viewing-and-interacting-with-explores#using_links_and_actions)) aren't supported by all file types.
...
If the **Expand Tables to Show All Rows** option is selected, dashboard tiles that contain table visualizations may look slightly different in delivered PDFs than they do inside Looker. The following differences may be noticeable in the PDF:
  ...
  * Tables appear in the [white theme](/looker/docs/table-options#table_theme).
  * The [sort icon](/looker/docs/table-options#sorting_columns) does not appear on pivoted tables that were not manually sorted.
  * Tables with [**Size Columns to Fit**](/looker/docs/table-options#size_columns_to_fit) enabled stretch to the full width of the tile.

Additionally, for tables with more than 20,000 cells, the following differences may be noticeable in the PDF:

  * [Conditional formatting](/looker/docs/table-options#enable_conditional_formatting) options other than **Background color** no longer appear.
  * [Cell visualizations](/looker/docs/table-options#cell_visualization) on numeric columns no longer appear.

If you do not see the **Expand tables to show all rows** option when you've chosen **PDF** in the **Format** field, talk to your Looker admin about installing the appropriate version of the [Chromium renderer](/looker/docs/installation-of-rendering-software#chromium) for your Looker instance.
...
You have the option to specify the optimal size and orientation of dashboard PDFs by selecting from the **Paper size** drop-down menu. Large visualizations or groups of overlapping dashboard tiles may need to be resized to fit cleanly on a PDF page. If you do not see the **Paper size** option, talk to your Looker admin about installing the latest version of the [Chromium renderer](/looker/docs/installation-of-rendering-software#chromium) for your Looker instance.
...
By default, Looker uses the time zone associated with your account to determine when to send your data delivery. If you don't have a time zone associated with your account, Looker uses your [Application Time Zone](/looker/docs/admin-panel-general-settings#application_time_zone) setting.
...
![](/static/looker/docs/images/dash-beta-schedule-filters-2308.png)

If a dashboard filter [requires](/looker/docs/filters-user-defined-dashboards#requiring_a_filter_value) a filter value, you must enter a value in the **Filters** tab to create or save any edits to the delivery.
...
A schedule is created. Later, a new (non-required or [required](/looker/docs/filters-user-defined-dashboards#requiring_a_filter_value)) filter is added to the dashboard. | The filter is applied to the delivered data, using the filter value set in the **Filters** tab. If no value is set in the **Filters** tab, the value is interpreted as **is any value** , essentially removing the filter.  
...
If your Looker admin has configured user-specific values, called user attributes, and you have edit privileges for a dashboard, you can create a dashboard filter that uses the [**matches a user attribute** filter option](/looker/docs/filters-user-defined-dashboards#matches_a_user_attribute). A filter of this type automatically customizes the filter value for each dashboard viewer. With this kind of filter, you can deliver customized results to each delivery's recipients. To read more about user attributes, visit the [User attributes](/looker/docs/admin-panel-users-user-attributes) documentation page.
...
Your saved schedules for a dashboard appear in its existing schedules window. Your saved schedules for all content appear on the [**Schedules** page](/looker/docs/user-schedules) of your user profile.
...
You can also delete a schedule from the [**Schedules** page](/looker/docs/user-schedules) of your user profile.

## Scheduling challenges

**Note:** Looker schedules data deliveries according to the time zone that is indicated in the [**Application Time Zone**](/looker/docs/using-time-zone-settings#application_time_zone) setting on the Admin **Settings** page, or, if enabled, the schedule creator's [User Specific Time Zone](/looker/docs/using-time-zone-settings#user_specific_time_zones) setting.
...
Looker admins can see, edit, reassign, and delete any users' schedules. Additionally, Looker admins should bear some things in mind when setting up data delivery for their Looker instance and granting users permissions to send or schedule data deliveries. For more information, visit the following documentation pages:
  ...
  * [Admin settings - Schedules](/looker/docs/admin-panel-alerts-and-schedules-schedule)
  * [Admin settings - Schedule History](/looker/docs/admin-panel-alerts-and-schedules-schedule-history)
  * [Managing business user features](/looker/docs/managing-business-user-features#sharing_content)
  * [Labs - Render Long Tables](/looker/docs/admin-panel-general-labs#render_long_tables)