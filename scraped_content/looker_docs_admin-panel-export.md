# https://cloud.google.com/looker/docs/admin-panel-export
...
You will see the **Export** page in your instance if all of the following situations are true:
  ...
  * You have the [Admin role](/looker/docs/admin-panel-users-roles#permission_sets) in the Looker instance.

You use the **Export** page when you are performing [self-service migration](/looker/docs/looker-core-self-service-migration).

The **Export** page is where you perform these tasks:

  * Create a request to export the data from your existing Looker (original) instance so that it can be migrated to the [Looker (Google Cloud core) environment](/looker/docs/looker-core-overview).
  * View the status of any submitted requests.

See the [Self-service migration from Looker (original) to Looker (Google Cloud core)](/looker/docs/looker-core-self-service-migration) documentation page for information about how to generate the information that needs to be entered onto the **Export** page and how to complete the migration.
...
Follow the steps on the [Self-service migration from Looker (original) to Looker (Google Cloud core)](/looker/docs/looker-core-self-service-migration) documentation page up to the [Request the export](/looker/docs/looker-core-self-service-migration#request-export) section. Then enter the following information into the **Export** page in your Looker (original) instance:

  * The name of the Cloud Storage bucket that you created
  * The Cloud Storage folder name
  * The [name of the KMS key](/kms/docs/getting-resource-ids#getting_the_id_for_a_key_and_version) used to encrypt the migration data
...
Once the export has been completed and the data appears in your Cloud Storage bucket, continue with the steps on the [Self-service migration from Looker (original) to Looker (Google Cloud core)](/looker/docs/looker-core-self-service-migration) documentation page, beginning with the [Import the data into the new "empty" Looker (Google Cloud core) instance](/looker/docs/looker-core-self-service-migration#import-data) section, to complete the migration.
