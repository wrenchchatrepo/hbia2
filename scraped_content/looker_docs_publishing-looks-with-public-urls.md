# https://cloud.google.com/looker/docs/publishing-looks-with-public-urls

Depth: 3

Looker admins and other users with the `create_public_looks` permission can enable public access for a [saved Look](/looker/docs/saving-and-editing-looks). For users to be able to enable public access for a Look, an admin must also enable [the **Public URLs** setting](/looker/docs/managing-business-user-features#public_sharing,_importing,_and_embedding).
...
To enable public access for a saved Look:
  ...
  1. If you are in [Development Mode](/looker/docs/dev-mode-prod-mode), select **Exit Development Mode**.
  2. Navigate to the Look you want to share.
  3. Select the Look's gear menu.
  4. Select **Edit Settings**. Looker displays the **Look Settings** window. ![](/static/looker/docs/images/browse-looks-public-access-on-2218.png)
...
Looker applies any [`value_format`](/looker/docs/reference/param-field-value-format) settings that you've put in place. Looker also applies some visualization settings to your public URLs, causing your data to appear similar to a [table chart](/looker/docs/legacy-table-options). Any of the following [Plot](/looker/docs/legacy-table-options#plot_menu_options) and [Series](/looker/docs/legacy-table-options#series_menu_options) visualization menu settings for the Look will be applied to the public URLs:
  ...
  * [Custom labels](/looker/docs/legacy-table-options#customizations) for each column (note that JSON files will always use raw field names, not the field label)
...
The application of any [`value_format`](/looker/docs/reference/param-field-value-format) settings shows up in the URL as the `apply_formatting=true` argument. If you don't want `value_format` settings to be applied, remove the `apply_formatting=true` from the URL.
...
For other types of embedding, see the [Private embedding](/looker/docs/private-embedding) and [Signed embedding](/looker/docs/single-sign-on-embedding) documentation pages.
...
> You can also export data directly to Google Sheets using the Google Sheets action from the [Looker Action Hub](/looker/docs/action-hub#the_looker_action_hub) rather than implementing the following method. See the [Looker Actions — Google Sheets](/looker/docs/best-practices/how-to-use-the-google-sheets-integration) Best Practices page for more information.
...
There are some things to keep in mind when using this method to export data to a Google Sheet:
  ...
  * Google Sheets are [limited to 5 million cells](https://support.google.com/drive/answer/37603) for the entire Sheet, so your data table's columns, rows, or tabs cannot exceed that limit.
  ...
  * You may need to force the data in the Sheet to refresh. See the [Exporting Data to Google Sheets](https://www.googlecloudcommunity.com/gc/Technical-Tips-Tricks/Exporting-Data-to-Google-Sheets/ta-p/588441) Community post for more information.
...
> Both Google and Excel will only interact with servers that have valid SSL certificates. If your Looker instance is hosted by Looker this won't be a concern. However, if you have a customer-hosted Looker instance, your admins will need to have followed [these instructions for configuring an SSL certificate for proper HTTPS](/looker/docs/ssl-certificate-for-proper-https) when they initially set up Looker.
...
For any of the features that are described on this documentation page to work, you must [enable **Public URLs**](/looker/docs/managing-business-user-features#public_sharing,_importing,_and_embedding).