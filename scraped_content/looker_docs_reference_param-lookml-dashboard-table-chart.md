# https://cloud.google.com/looker/docs/reference/param-lookml-dashboard-table-chart

Depth: 3

This page demonstrates how to add and customize a LookML dashboard element of `type: looker_grid` with LookML dashboard parameters in a [`dashboard.lkml` file](/looker/docs/other-project-files#dashboard_files).

For information about building a table chart through the Looker UI, see the [Table chart options](/looker/docs/table-options) documentation page.

**Note:** If your Looker admin has enabled the [**Accessible Data Table Visualizations**](/looker/docs/admin-panel-general-labs#accessible_data_table_visualizations) Labs feature, the row number column will not be pinned to the left side of a table chart. Users will also be unable to double-click the cells in the table to select text manually.
...
`fill_fields` | Defines the dimensions that utilize the [dimension fill](/looker/docs/creating-visualizations#filling_in_missing_dates_and_values) option  
...
`filter_expression` | Defines a [custom filter](/looker/docs/filtering-and-limiting#custom_filters) that _cannot_ be changed for the element's query  
...
`analysis_config` |  ADDED 21.14  Defines the [forecast analysis](/looker/docs/forecasting-in-visualizations) that should be performed when the query is run. Requires the **Forecasting** [Labs feature](/looker/docs/admin-panel-general-labs#forecasting) to be enabled.   
...
`transpose` | Sets whether to [transpose](/looker/docs/table-options#transpose) table rows into columns  
...
`series_cell_visualizations` | Specifies whether columns use the [**Cell Visualization**](/looker/docs/table-options#cell_visualization) visualization option. This parameter has the subparameters `is_active`, `palette`, and `value_display`.  
...
`series_collapsed` | Specifies whether a column that has subtotals will appear [collapsed](/looker/docs/table-options#collapse_subtotal)  
`series_value_format` | Defines the value format for a column using [custom formatting](/looker/docs/custom-formatting#custom_formatting_examples)  
Formatting Parameters  
...
The `fill_fields` parameter defines the dimensions that utilize the [dimension fill](/looker/docs/creating-visualizations#filling_in_missing_dates_and_values) option. Use the syntax `view_name.dimension_name` to specify the dimensions.
...
The `subtotals` parameter defines the dimensions that utilize the [subtotals](/looker/docs/table-options#subtotals) option. Use the syntax `view_name.dimension_name` to specify the dimensions.
...
The `total` parameter sets whether a totals row is shown at the bottom of the table. See [Displaying totals](/looker/docs/creating-and-editing-explores#displaying_totals) for more information.
...
The `row_total` parameter sets whether a totals column is shown on the right or left of the table. Only works if a pivot is present. See [Displaying Totals](/looker/docs/creating-and-editing-explores#displaying_totals) for more information.
...
The `filter_expression` parameter defines a non-changeable [custom filter](/looker/docs/filtering-and-limiting#custom_filters) for the element's query. If you would like filters that a user _can_ change in the dashboard, you should set up the filters using [`filters` for dashboards](/looker/docs/reference/param-lookml-dashboard#filters-for-dashboard), then apply them to the elements using `listen`.
...
The [Looker filter expressions](/looker/docs/filter-expressions) documentation page lists the Looker filter expressions.
...
The `analysis_config` parameter and its subparameters describe any query analysis to use with the visualization, starting in Looker 21.14. The **Forecasting** [Labs feature](/looker/docs/admin-panel-general-labs#forecasting) must be enabled to perform analyses on visualizations.
...
For more information, see the [Forecasting in visualizations](/looker/docs/forecasting-in-visualizations) documentation page.

To add forecasts to visualizations, the **Forecasting** [Labs feature](/looker/docs/admin-panel-general-labs#forecasting) must be enabled.
...
See the [Prediction interval](/looker/docs/forecasting-in-visualizations#prediction_interval) section on the **Forecasting in visualizations** documentation page.
...
See the [Length](/looker/docs/forecasting-in-visualizations#length) section on the **Forecasting in visualizations** documentation page.
...
See the [**Length**](/looker/docs/forecasting-in-visualizations#length) documentation page.
...
The [**Automatic**](/looker/docs/forecasting-in-visualizations#automatic) seasonality setting is represented as a blank `seasonality` value.

See the [Seasonality](/looker/docs/forecasting-in-visualizations#seasonality) section on the **Forecasting in visualizations** documentation page.
...
The following parameters correspond to the ability to [move and pin columns](/looker/docs/table-options#manually_moving_and_pinning_columns) in table charts.
...
The following parameters correspond to the options in the [**Plot** menu](/looker/docs/table-options#plot_menu_options) of the visualization editor for table charts.
...
The `transpose` parameter lets you [transpose](/looker/docs/table-options#transpose) table rows into columns. It accepts `true` or `false`.
...
The following parameters correspond to the options in the [**Series** menu](/looker/docs/table-options#series_menu_options) of the visualization editor for table charts.
...
The `dynamic_fields` parameter and its subparameters describe any [table calculations](/looker/docs/table-calculations) or [custom fields](/looker/docs/custom-fields) to use with the visualization. You must have [permission to create custom fields](/looker/docs/admin-panel-users-roles#create_custom_fields) to add a `description` of up to 255 characters or to use `calculation_type` for [custom groups](/looker/docs/custom-fields#custom_grouping) or [custom bins](/looker/docs/custom-fields#custom_binning). You must have [permission to create table calculations](/looker/docs/admin-panel-users-roles#create_table_calculations) to add a `description` of up to 255 characters to table calculations or to use `calculation_type` for [shortcut calculations](/looker/docs/table-calculations#quick_calculations).
...
You can create a [table calculation](/looker/docs/table-calculations) using a syntax like this:
...
You can create [shortcut calculations](/looker/docs/table-calculations#quick_calculations) for several calculation types using a syntax like this:
...
You can create a [custom measure](/looker/docs/custom-fields#creating_a_custom_measure_from_a_dimension) to use in your visualization using a syntax like this:
...
You can create a [filtered custom measure](/looker/docs/custom-fields#creating_a_filtered_custom_measure_from_another_measure) to use in your visualization using a syntax like this:
...
You can create a [custom dimension](/looker/docs/custom-fields#creating_a_custom_dimension_using_a_looker_expression) to use in your visualization using a syntax like this:
...
You can create [custom groups](/looker/docs/custom-fields#custom_grouping) for a dimension to use in your visualization using a syntax like this:
...
You can create [custom bins](/looker/docs/custom-fields#custom_binning) for a dimension to use in your visualization using a syntax like this:
...
If you are using a table calculation, the `expression` subparameter defines the [Looker expression](/looker/docs/creating-looker-expressions) used to create the table calculation.

#### `filter_expression`

If you are using a custom filtered measure, the `filter_expression` subparameter defines the [Looker expression](/looker/docs/creating-looker-expressions) used to filter the measure.

#### `value_format`

The optional `value_format` subparameter defines the value format for a dynamic field when you're using [custom formatting](/looker/docs/custom-formatting#custom_formatting_examples). If you want to use a [default Looker format](/looker/docs/reference/param-field-value-format-name#default_format_names), use `value_format_name` instead.

#### `value_format_name`

The optional `value_format_name` subparameter applies a [default format](/looker/docs/reference/param-field-value-format-name#default_format_names) to the dynamic field. If you want to use a [custom format](/looker/docs/custom-formatting#custom_formatting_examples), use `value_format` instead.

#### `calculation_type`

The `calculation_type`subparameter defines the type of [**Shortcut Calculation**](/looker/docs/table-calculations#quick_calculations) or [**Group**](/looker/docs/custom-fields#custom_grouping) function to create a table calculation, or to create a custom group for a dimension:

**Custom fields`calculation_type` options:**

  * **`group_by`** — Groups dimension values under custom fixed labels, based on a specified custom condition. Similar to `CASE WHEN` in SQL, or the LookML [`case`](/looker/docs/reference/param-field-case) field parameter.
  * **`bin`** — Groups values in custom bins, or tiers, for numeric type dimensions and custom dimensions. Similar to the LookML [`tier`](/looker/docs/reference/param-dimension-filter-parameter-types#tier) dimension type.
...
If you are using [custom groups](/looker/docs/custom-fields#custom_grouping) for a dimension, `args` specifies the arguments for applying fixed labels to dimension values. `args` takes the following format:
...
If you are using [custom bins](/looker/docs/custom-fields#custom_binning) for a numeric dimension, `args` specifies the arguments for applying fixed tiers to dimension values. `args` takes the following format:
...
The `args` subparameter is where you specify the names of the numeric fields that you are using for a [**Shortcut Calculation**](/looker/docs/table-calculations#quick_calculations). An argument takes the following format:
...
Specify whether one or more columns use the [**Cell Visualization**](/looker/docs/table-options#cell_visualization) option by indicating series name using the `view_name.field_name` format. `series_cell_visualizations` has the subparameters `is_active`, `palette`, and `value_display`.
...
The `palette` subparameter is optional. If it is not used, the palette will default to a diverging palette in the instance's default [color collection](/looker/docs/color-collections).
...
The `fg_color` subparameter indicates the font color for cell text. The color value can take a hex string, such as `#2ca6cd`, or a [CSS named color string](https://www.w3schools.com/cssref/css_colors.asp), such as `mediumblue`.

#### `bg_color`

The `bg_color` subparameter indicates the cell background color. The color value can take a hex string, such as `#2ca6cd`, or a [CSS named color string](https://www.w3schools.com/cssref/css_colors.asp), such as `mediumblue`.
...
The `series_collapsed` parameter defines whether to [collapse](/looker/docs/table-options#collapse_subtotal) or expand the subtotals for a particular series. Identify the series using `view_name.field_name` syntax and `true` or `false`.
...
The formatting used in the `format_string` subparameter is the same as formatting used with the [`value_format` LookML parameter](/looker/docs/reference/param-field-value-format#examples). You can read about how to specify these formats on the [Adding custom formatting to numeric fields](/looker/docs/custom-formatting#custom_formatting_examples) documentation page.

## Formatting parameters

The following parameters correspond to the options in the [**Formatting** menu](/looker/docs/table-options#formatting_menu_options) of the visualization editor for table charts.
...
The color value can take a hex string, such as `#2ca6cd`, or a [CSS named color string](https://www.w3schools.com/cssref/css_colors.asp), such as `mediumblue`.
...
You can add colors to a LookML dashboard by collection ID and palette ID, if you have them. You can also use the UI to find the colors you want and generate the LookML to add them to your dashboard. Navigate to a piece of user-defined content (like a Look, a dashboard, or an Explore), and apply the colors you want to that content's visualization using the UI. Once you've done that, you can follow the [steps to get dashboard LookML](/looker/docs/building-lookml-dashboards#adding_a_visualization_to_an_existing_lookml_dashboard), copy the LookML that was produced, and paste it in the `color_application` section. For an overview of Looker's predefined color collections, see the [Color collections](/looker/docs/color-collections) documentation page.
