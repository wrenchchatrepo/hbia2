...
The last column in the following table uses a table calculation to combine three fields in the data using the [`concat`](/looker/docs/functions-and-operators#functions_for_any_looker_expression_2) function.

![A data table includes a table calculation that concatenates the values of the columns Accidents Aircraft Category, Accidents Country, and Accidents Count in a single column.](/static/looker/docs/images/table-calcs-overview-720.png)

Table calculations can perform mathematical, logical (true/false), lexical (text-based), and date-based calculations on the dimensions, measures, and other table calculations in your query. The formulas that you use to execute these calculations are called [Looker expressions (Lexp)](/looker/docs/creating-looker-expressions).
...
There are a few differences between table calculations and LookML-defined fields that are defined in LookML:
  ...
  * Table calculations give anyone who has the [appropriate permissions](/looker/docs/admin-panel-users-roles#create_table_calculations) the ability to create calculations that are _based on_ LookML-defined fields, rather than the ability to _create_ LookML-defined fields, which require that the user have [development permissions](/looker/docs/admin-panel-users-roles#develop) and understand LookML.
...
There are a few differences between table calculations and [custom fields](/looker/docs/custom-fields):
...
Table calculation fields appear next to dimensions and measures in the table. If you want to reuse your table calculations in the future, be sure to [save your Explore as a Look](/looker/docs/saving-and-editing-looks#saving_looks_from_an_explore) or as a [dashboard tile](/looker/docs/creating-user-defined-dashboards#adding_query_tiles_from_an_explore).

## Creating table calculations

To allow users or groups to create table calculations, your Looker admin must give those users or groups access to the feature by granting them the [`create_table_calculations`](/looker/docs/admin-panel-users-roles#create_table_calculations) permission.

Looker's Explore page has a built-in Looker expression editor to help you create table calculations, custom fields, and custom filters. If you are a Looker developer who is creating a [data test](/looker/docs/reference/param-model-test) to verify the logic of your model, you can also use the Looker expression editor to build a Looker expression, then copy the expression into your data test's `expression` parameter.

**Caution:** Before you create a table calculation, make sure that all the fields you want to use in the table calculation have been selected from the field picker and that you have run the query. 
...
If you have the [permissions to create table calculations](/looker/docs/admin-panel-users-roles#create_table_calculations), you can use the **Custom Fields** section of the [field picker](/looker/docs/creating-and-editing-explores#field_picker) to open the **Create table calculation** dialog.
...
To create a table calculation using the **Custom Fields** section, follow these steps:
  ...
  2. Select **Table Calculation** to open the **Create table calculation** dialog.
     ...
     * If the [**In-page Table Calculations**](/looker/docs/admin-panel-general-labs#in-page_table_calculations) Labs feature is enabled, the table calculation expression editor will open in the **Data** section of the [Explore](/looker/docs/viewing-and-interacting-with-explores#the_explore_page) page. Use the instructions in the Creating table calculations with In-page Table Calculations section to finish creating your table calculation.
**Caution:** The **Table Calculation** option will not be enabled until fields have been selected from the field picker and an Explore query has been run. 
...
![](/static/looker/docs/images/in-page-table-calculation-create-edit-2218.png)
...
Several types of calculations are available as shortcut options. The following table lists each available calculation along with its description, input, and [Looker expression (Lexp)](/looker/docs/creating-looker-expressions#looker_expressions), and the default value format and default name of the calculation. You can specify a different name or value format by editing your calculation.
...
**Caution:** The order in which an Explore's data table is [sorted](/looker/docs/creating-and-editing-explores#sorting_data) can impact calculations that are based on a value's position relative to other values in a column, such as **% of previous row** , **% change from previous row** , **% of previous column** , **% change from previous column** and **% of row**. Make sure an Explore's data table is sorted accordingly.  Calculation | Description | Input | Lexp | Format | Name  
---|---|---|---|---|---  
...
When Explore results are [pivoted](/looker/docs/creating-and-editing-explores#pivoting_dimensions), there are more common calculation shortcuts available for pivoted measures.
...
As with other fields, you can select the calculation's name to add or remove it from a query. You can also hover over the field to reveal [more options](/looker/docs/creating-and-editing-explores#field-specific_information_and_actions) that are available for that calculation, including editing the calculation.
...
This method lets you select a format or name other than the default, or add a description as you create the calculation.
  ...
  1. Open the **Create table calculation** dialog if you are creating a table calculation, or open the **Edit table calculation** dialog if you are editing a table calculation.
     ...
     * If the [**In-page Table Calculations**](/looker/docs/admin-panel-general-labs#in-page_table_calculations) Labs feature is enabled, the table calculation expression editor will open in the **Data** section of the [Explore](/looker/docs/viewing-and-interacting-with-explores#the_explore_page) page. Use the instructions in the Creating table calculations with In-Page Table Calculations section to finish creating a table calculation with shortcut calculations.

![The Edit table calculation dialog displays a % of previous row calculation based on an Inventory Items Count field.](/static/looker/docs/images/editing-table-calc-custom-fields-2304.png)
  ...
  4. Optionally, use the **Format** drop-down to choose a predefined format or create a custom format for the results. If you create a custom format, use spreadsheet-style formatting as described on the [Adding custom formatting to numeric fields](/looker/docs/custom-formatting) documentation page. If no selection is made, Looker uses a default format.
...
You can duplicate table calculations in these ways:
  ...
  * Using a calculation's [three-dot **More** menu](/looker/docs/creating-and-editing-explores#three-dot_more_menu) in the **Custom Fields** section of the field picker
...
To edit a table calculation, follow these steps:
  ...
  3. Select **Edit** to open the **Edit table calculation** dialog. 
     * If the [**In-page Table Calculations**](/looker/docs/admin-panel-general-labs#in-page_table_calculations) Labs feature is enabled, the table calculation expression editor will open in the **Data** section of the [Explore](/looker/docs/viewing-and-interacting-with-explores#the_explore_page) page. Use the instructions in the Creating table calculations with In-page Table Calculations section to finish editing your table calculation.
...
You can also use the [keyboard shortcuts](/looker/docs/keyboard-shortcuts#shortcuts_for_exploring) Command-K (Mac) or Ctrl+K (Windows) to delete table calculations and clear fields from an Explore query.
...
To sort on a table calculation, select the field name at the top of the column, just as you would a dimension or measure. For more information on sorting, see the [Creating and editing Explores](/looker/docs/creating-and-editing-explores#sorting_data) documentation page.
...
Sorting on a table calculation works similarly to sorting on a dimension or measure in Looker. However, there are two important differences that prevent sorting in some scenarios:
  ...
  * Some table calculations are applied to multiple rows within the same column, such as when using an `offset()` function (see more on [using the `offset` and `pivot_offset` functions](/looker/docs/best-practices/how-to-calculate-percent-change-percent-previous) in Looker's Best Practices). In these cases, sorting the table calculation would change its results and is therefore disabled.
...
**Caution:** Any table calculation that uses an `offset` function cannot be sorted, because the order in which an Explore's data table is [sorted](/looker/docs/creating-and-editing-explores#sorting_data) can impact calculations that are based on a value's position relative to other values in a column. The sort order of the rows can change the results that are displayed by the `offset` calculation.
...
![](/static/looker/docs/images/explore-table-calc-vis-2308.png)
...
![](/static/looker/docs/images/explore-table-calc-yesno-2308.png)
...
![](/static/looker/docs/images/explore-table-calc-hide-nos-vis-2308.png)

**Caution:** Looker does not re-run queries to exclude fields or values that are hidden using [**Hide from Visualization**](/looker/docs/creating-visualizations#hiding_fields_from_visualizations) or **Hide "No"s from Visualization** for table calculations. As a result, calculations that are based on fields that have hidden Explore values may display unexpected results. 
...
To filter out rows with null values, create a new table calculation using the [`is_null`](/looker/docs/functions-and-operators#is_null) logical function:
...
## Considerations for using table calculations
  ...
  * If you add a [**Total**](/looker/docs/creating-and-editing-explores#displaying_totals) row to your data table, some table calculations that perform aggregations might not add up as you expect, for example, calculations that use `percentile` or `median`. This is because table calculations calculate totals using the values in the **Total** row, not by using the values in the data column. See the [Display potentially confusing table calculation totals as nulls](/looker/docs/best-practices/how-to-display-confusing-table-calculation-totals-as-nulls) Best Practices page for troubleshooting tips.
  * Always use leading zeroes for decimals less than 1. For example, 0.95 will work, but .95 will cause an error.
  * Using the Command-K (Mac) or Ctrl+K (Windows) [keyboard shortcut](/looker/docs/keyboard-shortcuts#shortcuts_for_exploring) will clear all table calculations, including custom fields. To reinstate your table calculations, select the back arrow on your browser. You may also need to re-run your query.