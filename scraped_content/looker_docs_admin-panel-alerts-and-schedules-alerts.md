# https://cloud.google.com/looker/docs/admin-panel-alerts-and-schedules-alerts

Depth: 3

Looker admins can manage alerts created by all users on the **Alerts** management admin page, which can be accessed from the **Alerts & Schedules** section of the **Admin** panel. The **Alerts** management admin page lists information about all active and inactive [alerts](/looker/docs/alerts-overview).
...
Admins can also see alerts that they have created or followed, or for which they are listed as a recipient, on the [**Manage Alerts** user page](/looker/docs/user-alerts). Users with other alerts permissions can manage the alerts they have created and followed from the [**Manage Alerts** user page](/looker/docs/user-alerts).

## Alert information

The **Manage Alerts** admin page provides information about all alerts on the Looker instance. Admins and users who have been granted the [`see_alerts`](/looker/docs/admin-panel-users-roles#see_alerts) permission can view these settings in the **Manage Alerts** admin page table:
...
Status | This column indicates whether the alert is currently active or disabled.
  ...
  * An alert is marked as **Disabled** when a user disables their own alert (using the three-dot menu) in the alert creation pop-up or the [alert management user page](/looker/docs/user-alerts) or when a Looker admin disables the alert. Hover over the **Disabled** status to see the alert owner's or Looker admin's explanation as to why the alert was disabled. Disabling an alert will pause notifications for any alert recipients, but the alert will still appear on the [alert view window](/looker/docs/viewing-alerts). The alert owner may see that the alert status shows as **Active** until the next time the alert query is run.
  * If an admin disables the alert, followers will no longer receive alert notifications.

  
Following | A button that indicates your [following](/looker/docs/alerts-overview#following_alerts) status for the alert. See more in the Following alerts section on this page.  
...
![](/static/looker/docs/images/enhanced-alerts-filters-2214.png)
...
![](/static/looker/docs/images/enhanced-alerts-warnings-2114.png)
...
There are a few possible explanations for why the data did not change between queries. If this warning persists, talk to your Looker admin or developer to verify that the [caching policies](/looker/docs/reference/param-model-datagroup) that are defined for this dashboard tile are aligned with the [alert frequency setting](/looker/docs/alerts-overview#setting_alert_frequency).
...
You will see an **Unfollow** button next to the alert if you are following the alert. If you click **Unfollow** , you will stop receiving notifications when this alert is triggered. After you click **Unfollow** :
  ...
  * If you were listed as a [recipient](/looker/docs/alerts-overview#setting_alert_notifications) for an alert's notifications but did not follow the alert from the [alert view window](/looker/docs/alerts-overview#viewing_alerts_from_a_dashboard_tile), you will see an unclickable **Follow** button. To unfollow the alert, you must [unsubscribe](/looker/docs/glossary#unsubscribe) directly from your [alert notification](/looker/docs/alerts-overview#receiving_an_alert_notification).
...
You will see an unclickable **Follow** button next to the alert if any of these conditions are true:
  ...
  * You were listed as an email [recipient](/looker/docs/alerts-overview#setting_alert_notifications) for the alert notifications but did not follow the alert from the [alert view window](/looker/docs/alerts-overview#viewing_alerts_from_a_dashboard_tile). To unfollow the alert, you must [unsubscribe](/looker/docs/glossary#unsubscribe) directly from your [alert notification](/looker/docs/alerts-overview#alert_notifications_to_email).
...
---|---|---  
**Ran At** | The timestamp of when the alert query was initiated | Any timestamp in the format `DD/MM HH:MM`Example: `05/30 18:25`  
...
**Source Time** | The timestamp of the database row from which the alert query checked for the conditional value | Any timestamp in the format `DD/MM HH:MM`Example: `05/25 16:01`  
...
Both tabs on the **Alert Details** page show the alert's synchronization status and the name of the dashboard with which the alert is associated:
  ...
  * A yellow triangular warning symbol indicates that the alert has become unsynced from its dashboard. The status includes a timestamp of when the synchronization was broken. An unsynced alert displays a **None** value in the **Dashboard** column. For information about resolving an unsynced alert, see the [Effect of dashboard edits on alerts](/looker/docs/effect-of-dashboard-edits-on-alerts#resolving_an_unsynced_alert_with_enhanced_alerts_enabled) documentation page.