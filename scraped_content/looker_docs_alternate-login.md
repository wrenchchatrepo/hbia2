# https://cloud.google.com/looker/docs/alternate-login

Depth: 3

**Note:** Alternate login is not available for [Looker (Google Cloud core)](/looker/docs/looker-core-overview). By default, the backup authentication method for Looker (Google Cloud core) is [OAuth](/looker/docs/looker-core-oauth-authentication#using_oauth_as_a_backup_authentication_method).

Looker can authenticate users using one of several authentication server types, such as [Google OAuth](/looker/docs/admin-panel-authentication-google), [LDAP](/looker/docs/admin-panel-authentication-ldap), [SAML](/looker/docs/admin-panel-authentication-saml), or [OpenID Connect](/looker/docs/admin-panel-authentication-openid-connect). Enabling any of these authentication methods will disable other authentication systems, such as email and password.

Looker admins can give a user an alternate login option that uses their email address if the user has either an Admin [role](/looker/docs/admin-panel-users-roles) or the [`login_special_email` permission](/looker/docs/admin-panel-users-roles#login_special_email).
...
Before you can enable the alternate login option for a user, the Looker instance must be configured to accept email credentials:
  ...
  1. Navigate to the **Authentication** section of the **Admin** panel, and select the currently enabled authentication type, [Google OAuth](/looker/docs/admin-panel-authentication-google), [LDAP](/looker/docs/admin-panel-authentication-ldap), [SAML](/looker/docs/admin-panel-authentication-saml), or [OpenID Connect](/looker/docs/admin-panel-authentication-openid-connect).
...
Only users with the Admin role or the [`login_special_email` permission](/looker/docs/admin-panel-users-roles#login_special_email) may use alternate login. One way to grant the `login_special_email` permission to a non-admin user is to create a new role that contains that permission and then assign that role to the user, as follows:
...
Once the user has been granted permission to use email credentials, the next step is to create the credentials. To create email credentials, a Looker admin can either use the [Looker API](/looker/docs/reference/looker-api/latest) to make a `POST` request or use the [Looker API SDK](https://github.com/looker-open-source/sdk-codegen/tree/main/examples) in the programming language of the admin's choice.
...
This example uses a `curl` command to make a `POST` request to the `create_user_credentials_email` API endpoint using a temporary access token:
  ...
  1. To generate the temporary token (`ACCESS_TOKEN`), follow the **Authentication without an SDK** instructions on the [Looker API authentication](/looker/docs/api-auth#authentication_without_a_sdk) documentation page.
...
Rather than going through the manual steps of making requests directly to the Looker API, you can instead use a Looker-provided SDK to interact with the API in a programming language of your choice. After you have imported the Looker API SDK and established a client connection, follow these steps:
  ...
  1. Use the `create_user_credentials_email(user_id, body)` function, inserting the `user_id` and `body` as specified in the [Looker API documentation](/looker/docs/reference/looker-api/latest/methods/User/create_user_credentials_email). You can follow a similar example from this Looker Community post about [automatically provisioning users with the Looker API](https://www.googlecloudcommunity.com/gc/Developing-Applications/Automatically-Provisioning-Users-with-the-Looker-API/td-p/577037).
