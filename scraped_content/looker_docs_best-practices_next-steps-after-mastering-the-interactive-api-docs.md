# https://cloud.google.com/looker/docs/best-practices/next-steps-after-mastering-the-interactive-api-docs

Depth: 3

Using Looker's [API Explorer](/looker/docs/api-explorer), users can test API calls nearly instantly without having to write a single line of code. If you have installed the [API Explorer extension](/looker/docs/api-explorer) from the [Looker Marketplace](/looker/docs/marketplace), you can click **API Explorer** in the Looker **Applications** menu to open the API Explorer and view current API documentation. If you have not installed the API Explorer extension, you can install it from the **Applications** section of the Looker Marketplace. 
...
To find a client ID and client secret: 
  ...
  * If you are a Looker admin, visit the [**Users** page](/looker/docs/admin-panel-users-users#api3_keys) in the Looker UI for the user you are interested in and go to **Edit keys**. 
...
If you host your own instance, check with your system admin for the port number. It may be set in the [**API Host URL**](/looker/docs/admin-panel-platform-api#api_host_url) field of Looker's admin panel. You can see this by going to the **Admin** menu drop-down in Looker and selecting **API**. 

For more information on ports, go to the [Getting started with the Looker API](/looker/docs/api-getting-started#looker_api_path_and_port) documentation page. The following examples use an API port of 19999, but you should confirm the port being used by your instance. 

##  Option 1: Use a Looker software development kit (SDK) 

Looker offers official Looker API client SDKs in [Python](https://github.com/looker-open-source/sdk-codegen/tree/master/python), [Ruby](https://github.com/looker-open-source/looker-sdk-ruby), [Typescript and JavaScript](https://github.com/looker-open-source/sdk-codegen/tree/master/packages/sdk), [Swift](https://github.com/looker-open-source/sdk-codegen/tree/main/swift/looker), [Kotlin](https://github.com/looker-open-source/sdk-codegen/tree/main/kotlin), and [R](https://github.com/looker-open-source/lookr). You can find source code and examples in Looker's [`sdk-examples` GitHub repo](https://github.com/looker-open-source/sdk-examples). 

An SDK provides tools or libraries that allow developers to interact with a given platform or application. In this case, Looker's SDKs generally contain APIs. To borrow an example from web developer and author [Kristopher Sandoval](https://nordicapis.com/author/sandovaleffect/), "APIs are telephone lines, allowing for communication in and out of the house. The SDK is the house itself and all of its contents." He explains what an SDK is and how that relates to APIs in a great article, [What Is the Difference Between an API and an SDK?](https://nordicapis.com/what-is-the-difference-between-an-api-and-an-sdk/)
...
What if you do not want to write a script or spend months or years learning a new programming language? In that case, you can use [curl](https://curl.haxx.se/) to make HTTP requests to utilize Looker's API. 

An HTTP request sends a message to a destination, which can be a server, a phone, or even your smart television. There are a few different types of HTTP requests. How you use these requests with Looker's API depends on the nature of [the method](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods) you pass as part of the API call. Some methods give you data, some send data to Looker, some update data, and some delete or remove data from Looker. 
...
Let's start curling. For some background, Zendesk has a great tutorial, [Installing and using cURL](https://developer.zendesk.com/documentation/developer-tools/getting-started/installing-and-using-curl). 

To start making HTTP calls to the Looker API, the first thing you need to do is call the Looker API's `login` endpoint using your client ID and client secret. This creates [an access token](/looker/docs/api-auth#authentication_without_an_sdk). Then, you take this access token and pass it with each call. The access token ensures that the call is coming from an authorized user. 
...
Let's say you want to create a new user. To do this: 
  ...
  2. Include a body, in this case formatted as JSON, to tell Looker what attributes you want your new user to have. (There are some required fields for API calls, so please consult the [Looker API 4.0 reference](/looker/docs/reference/looker-api/latest/methods/User/create_user) documentation.)
...
The `-H` stands for header, and the `-d` stands for data. For more information on curl commands, go to [this GitHub gist](https://gist.github.com/subfuzion/08c5d85437d5d4f00e58). 

You just created a user with the first name, last name, and email address that has the values you entered previously. 

What if you want to write this in a script, so you do not have to write out these commands every time you want to complete this workflow? You can use a programming language and library like [Python's `requests` library](https://requests.readthedocs.io/en/master/). 
...
![](/static/looker/docs/images/mastered-api-hc-3-2210.png)
...
If you have a question or want to share a cool idea, check out the [Looker Community](https://discourse.looker.com/). Feel free to let us know if there is anything we can improve or if there are any other examples you would like added to our documentation.