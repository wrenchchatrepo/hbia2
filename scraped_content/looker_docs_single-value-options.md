...
![Single value visualization in the Looker Explore UI with the Edit button highlighted](/static/looker/docs/images/vis-single-value-menu-2502.png)
...
If your query has multiple fields, you can [hide fields from the visualization](/looker/docs/creating-visualizations#hide-fields) that you don't want displayed. If your query has multiple unhidden fields, the single value visualization will use the following priority order to choose which field to display:

  * The leftmost measure
  * The leftmost [table calculation](/looker/docs/table-calculations) based on a measure
  * The leftmost dimension
  * The leftmost [table calculation](/looker/docs/table-calculations) that does _not_ involve a measure
...
**Note:** If your Looker admin has enabled the [**Smart Single Value Text Size** Labs feature](/looker/docs/admin-panel-general-labs#smart-single-value-text-size), the displayed fonts on single value visualizations in dashboard tiles automatically resize to display at least 8-10 value characters within the tile at any given size.
...
When you click the color swatch under **Value Color** , Looker displays a color picker where you can select a color, input a hex string like `#2ca6cd`, or input a [CSS named color string](https://www.w3schools.com/cssref/css_colors.asp) like `mediumblue`.

![](/static/looker/docs/images/single-value-picker-64.png)
...
Depending on your needs, consider using the following alternate techniques:
  ...
  * To change the font color for a particular field's value everywhere, including the data table, a LookML developer can add a [`html`](/looker/docs/reference/param-field-html) parameter to the field definition.
...
You can use a second field to add comparison information to your single value visualization. In general, the comparison value uses the [next field that would be displayed](/looker/docs/single-value-options#single-value-vis-order) as the single value visualization. If there is no other field in the first row to compare with, a field in the next row is used for comparison. You can affect which field displays by changing the order of fields or choosing [**Hide from Visualization**](/looker/docs/creating-visualizations#hide-fields) for fields that you want to skip.
...
![Single value visualization with Edit panel open showing the Calculate Progress \(With Percentage\) option of the Value Label field](/static/looker/docs/images/single-value-progress-percent-2502.png)
...
Click the color swatches next to **Background color** or **Font color** to pull up the palette picker, which lets you pick or customize a color:
  ...
  * To select a custom color, click the **Custom** tab of the palette picker, and then either use the color wheel and value slider to choose a custom color, or enter an RGB [hex string](https://www.w3schools.com/colors/colors_hexadecimal.asp), such as `#2ca6cd`, or [CSS color names](https://www.w3schools.com/cssref/css_colors.asp), such as `mediumblue`, into the input field.

![Alt](/static/looker/docs/images/single-value-vis-palette-picker-2502.png)

#### Multiple rules

You can have multiple rules for conditional formatting:

![](/static/looker/docs/images/single-value-vis-conditional-formatting-rules-2502.png)
