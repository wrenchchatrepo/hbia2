...
![](/static/looker/docs/images/looker-ide-2414.png)

## Accessing the Looker IDE

To see a LookML project, you must have the [`develop`](/looker/docs/admin-panel-users-roles#develop) permission for at least one model in the project. You can access the Looker IDE through the **Develop** panel of the Looker main navigation menu:
...
Alternatively, you can open the IDE to specific LookML objects [from an Explore](/looker/docs/accessing-project-files#accessing_lookml_from_an_explore) or [from the object browser](/looker/docs/accessing-project-files#accessing_lookml_from_the_object_browser_panel). See [Accessing project files](/looker/docs/accessing-project-files) for details and other options.

## Elements of the Looker IDE

The Looker IDE has the following elements:

![](/static/looker/docs/images/looker-ide-elements-all-2414.png)

  1. Development Mode banner: This banner indicates that you are in [Development Mode](/looker/docs/dev-mode-prod-mode#development_mode), where you can make changes to LookML files and preview how they will affect content on your instance. You can click the **Exit Development Mode** link in the banner to go into [Production Mode](/looker/docs/dev-mode-prod-mode#production_mode).
  2. IDE navigation bar: Use the navigation bar to access different features of the Looker IDE. Some of the feature are available only when you are in Development Mode. Once you've selected a feature from the IDE navigation bar, you can select the navigation bar item a second time to collapse the feature panel. The IDE navigation bar provides access to the following IDE features: 
     * **File Browser** : Opens the [**file browser**](/looker/docs/ide-folders), which gives you access to the files and folders in your LookML project.
     * **Object Browser** : Opens the [**object browser**](/looker/docs/object-browser), which displays the LookML objects in your project and shows the hierarchical relationships between the objects.
     ...
     * **Git Actions** : (Visible in Development Mode only) Displays the Git branches on your project and shows the Git commands that are available for your Git branch of the project. See the [Using version control and deploying](/looker/docs/version-control-and-deploying-changes) documentation page for information about using Git with your LookML project.
     * **Settings** : (Visible in Development Mode only) Displays the project settings pages for your project where you can configure [project version control options](/looker/docs/git-options#configuration), authentication credentials for [imported projects](/looker/docs/importing-projects), [Git branches](/looker/docs/version-control-and-deploying-changes#managing_git_branches) used by your project, and IDE settings.
     * **Deploy** : (Visible in Development Mode only) For projects that are set up with [advanced deploy mode](/looker/docs/advanced-deploy-mode), opens the **Deployment Manager** where developers who have the [`deploy` permission](/looker/docs/admin-panel-users-roles#deploy) can deploy a commit or tag to their Looker production environment.
  ...
  5. Side panel selector: Click an option to open the IDE side panel features: 
     ...
     * **Quick Help** : Opens the **Quick Help** panel, which includes a tab to access the **Metadata** panel.
     * **Help me code** : (For [Looker (Google Cloud core)](/looker/docs/looker-core-overview) instances with [Gemini enabled](/looker/docs/looker-core-admin-gemini#enable-and-disable)) Opens the **Help me code** panel. Click the side panel selector option a second time to hide the IDE side panel.
  6. IDE side panel: Displays the selected side panel feature: 
     * **Project Health** : Lets you [check for unbuilt PDTs](/looker/docs/version-control-and-deploying-changes#checking_for_unbuilt_pdts) or [validate your LookML](/looker/docs/lookml-validation). If your LookML developers have defined data tests to verify the logic of the project's LookML, you can also [run data tests](/looker/docs/version-control-and-deploying-changes#running_data_tests) from the **Project Health** panel.
     ...
     * [**Metadata**](/looker/docs/lookml-metadata): Provides contextually relevant information about the LookML object at the cursor's location.
     * **Help me code**: (For [Looker (Google Cloud core)](/looker/docs/looker-core-overview) instances with [Gemini enabled](/looker/docs/looker-core-admin-gemini#enable-and-disable)): Lets you use Gemini in Looker to help write LookML.
  7. Git button: Depending on the state of your project, the button shows the next action that you should take to get your project to production, such as **Validate LookML** , **Push Branch to Remote** , or **Commit**. Click the button to perform the action. See the [Git command reference](/looker/docs/git-command-reference) for the possible commands that the button can display and execute.
...
**Tip:** You can also select your text editor preference from your [Looker user account settings page](/looker/docs/user-account#text-editor).
...
![](/static/looker/docs/images/ide-resize-panels-2416.png)
...
![](/static/looker/docs/images/ide-collapse-side-panels-2414.png)
...
![](/static/looker/docs/images/develop-ide-error-check-720.png)
...
When you are in Development Mode, you can use the [format file keyboard shortcut](/looker/docs/keyboard-shortcuts#format_file) to automatically indent your LookML file. The indentation indicates the hierarchy of parameters and subparameters, making your LookML easier to read and understand.
...
![](/static/looker/docs/images/develop-ide-fold-720.png)
...
![](/static/looker/docs/images/develop-ide-fold-unfold-720.png)

You can also use [keyboard shortcuts](/looker/docs/keyboard-shortcuts#shortcuts_in_the_lookml_development_environment) to fold and unfold LookML.
...
You can comment out an entire block of code using [keyboard shortcuts](/looker/docs/keyboard-shortcuts) by performing these steps:
...
When you are in [Development Mode](/looker/docs/dev-mode-prod-mode), the Looker IDE persists your IDE state across refreshes and logins. Your IDE state includes the following:
...
In the Looker IDE, you can do a quick search to navigate directly to LookML objects and project files by using the **Jump to object or file** feature. Click the **Jump to object or file** icon from the [file browser](/looker/docs/ide-folders) or the [object browser](/looker/docs/object-browser), or use the [keyboard shortcut](/looker/docs/keyboard-shortcuts) Command-J (Mac) or Ctrl+J (Windows).

![](/static/looker/docs/images/dev-file-browser-jump-to-2106.png)

See the [Accessing LookML project files](/looker/docs/accessing-project-files#searching_for_and_jumping_to_lookml_objects_or_project_files) documentation page for more information.

## Quick Help panel

To open the **Quick Help** panel, select the **Quick Help** icon from the [file browser](/looker/docs/ide-folders) or the [object browser](/looker/docs/object-browser):
...
**Note:** You can also use the metadata panel to view contextually relevant information about the objects in the Looker IDE metadata panel. See the [Metadata for LookML objects](/looker/docs/lookml-metadata) documentation page for more information.

### Help me code panel

If you have a [Looker (Google Cloud core)](/looker/docs/looker-core-overview) instance with [Gemini enabled](/looker/docs/looker-core-admin-gemini#enable-and-disable), you can use Gemini in Looker to help write LookML:

![Looker IDE with the Help me code icon highlighted.](/static/looker/docs/images/ide-help-me-code-2416.png)

See the [Write LookML with Gemini in Looker assistance](/looker/docs/looker-core-lookml-gemini) documentation for details.
...
From the options menus, you can create folders and files, upload files, and delete and move files and folders using the file browser. For more information, see the [Managing LookML files and folders](/looker/docs/creating-project-files) documentation page.

As you organize your LookML files with folders, you need to [commit your changes](/looker/docs/version-control-and-deploying-changes#committing_changes) and [deploy them to production](/looker/docs/version-control-and-deploying-changes#deploying_to_production) before other Looker developers can see them. This is just like any change you make to your LookML project.
...
**Tip:** The Looker IDE find and replace is a great tool for locating LookML elements in large LookML projects and for replacing text in descriptions and labels. If you want to search and replace in your project for the names of models, Explores, and fields, use [the Content Validator's find and replace function](/looker/docs/content-validation#using_the_content_validator_to_find_and_replace) instead. Changing the names of models, Explores, and fields can break references within your model, and the Content Validator can validate your LookML references and fix errors in your LookML after making these types of changes.

To use the Looker IDE find function, follow these steps:

![](/static/looker/docs/images/develop-search-720.png)
...
If you are in [Development Mode](/looker/docs/dev-mode-prod-mode), you will also see the **Replace With** field. To replace text in your project, follow these steps:

![](/static/looker/docs/images/develop-search-replace-720.png)
...
![](/static/looker/docs/images/develop-search-replace-result-720.png)
...
Check out [substitution operators](/looker/docs/sql-and-referring-to-lookml#substitution_operator_\($\)) to learn more about `${TABLE}.state`.
...
![](/static/looker/docs/images/develop-ide-explore-720.png)
...
![Data table for a sample Users Explore with the Users from CA or NY \(Yes/No\) and Users Average Age fields selected.](/static/looker/docs/images/develop-ide-explore-results-720.png)

**Note:** You must be in Development Mode to access these new fields until you have committed and pushed your changes to production.