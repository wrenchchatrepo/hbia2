# https://cloud.google.com/looker/docs/tableau-connector
...
**Note:** If you were part of the Looker–Tableau BI Connector Private Preview, see the Upgrading from the Looker–Tableau BI Connector Public Preview section.
...
Your Looker instance must meet the following requirements to make use of the Looker–Tableau BI Connector:
  ...
  * Enabled for the **Tableau Desktop** BI connector. A Looker admin must enable the [**Tableau Desktop**](/looker/docs/bi-connectors#tableau_desktop) toggle on the Looker [**BI Connectors**](/looker/docs/bi-connectors) page.
  * Contains a LookML model that uses data from a Google BigQuery connection: 
    * The LookML project must include a model that is configured with at least one Google BigQuery connection in the **Allowed Connections** field. See the [Accessing and editing project information](/looker/docs/manage-projects#adding_and_editing_configurations) documentation page for information on configuring a model and seeing the allowed connections for the model.
    * The LookML project must have a [model](/looker/docs/lookml-project-files#model_files) file that specifies a BigQuery connection in its [`connection`](/looker/docs/reference/param-model-connection) parameter.

In addition, each person using the Looker–Tableau BI Connector must have a Looker account with a [user role](/looker/docs/admin-panel-users-roles#roles) that includes the [`explore`](/looker/docs/admin-panel-users-roles#explore) permission on the LookML model that they want to access from Tableau.
...
**Note:** This procedure assumes that you already have Tableau Desktop installed on your computer.
...
The Tableau connector file is called `looker_v1.0.0.taco`. Each user must download the TACO file and save it to their computer by using the following steps:
  ...
  1. To download the Tableau connector file, click the following link: [`looker_v1.0.0.taco`](/looker/docs/file-downloads/looker_v1.0.0.taco)
...
**Note:** In case your setup doesn't use the default location for the `My Tableau Repository` directory, you can get the actual location by selecting **File > Repository Location** from the Tableau Desktop app on your computer.
...
**Note:** For macOS, manually create the `JDBC` directory if it does not already exist.
...
After you have downloaded and saved the `looker_v1.0.0.taco` and the `avatica-<release_number>-looker.jar` files, you can use Tableau Desktop to connect to data from your Looker instance by following these steps:
  ...
  8. Use the **Schema** drop-down menu to select a Looker [model](/looker/docs/lookml-project-files#model_files) with the data that you want to view in Tableau Desktop. Tableau will populate the **Table** list with the Looker [Explores](/looker/docs/creating-and-editing-explores) from the Looker model that you selected.
...
Queries from Tableau are made through the Open SQL Interface. Looker admins can use the Looker UI to identify which queries originated from the Open SQL Interface, as described in the [Open SQL Interface](/looker/docs/sql-interface#identify_queries) documentation.
...
Your Looker instance must meet the following requirements to use the Looker–Tableau BI Connector on Tableau Server:
  ...
  * Contains a LookML project that uses data from a Google BigQuery connection: 
    * The LookML project must be configured so that the **Allowed Connections** field is set to the **Only these connections** option with at least one Google BigQuery connection selected. See the [Accessing and editing project information](/looker/docs/manage-projects#adding_and_editing_configurations) documentation page for information on configuring a model and seeing the allowed connections for the model.
    * The LookML project must have a [model](/looker/docs/lookml-project-files#model_files) file that specifies a BigQuery connection in its [`connection`](/looker/docs/reference/param-model-connection) parameter.

In addition, each person who uses the Looker–Tableau BI Connector must have a Looker account with a [user role](/looker/docs/admin-panel-users-roles#roles) that includes the [`explore`](/looker/docs/admin-panel-users-roles#explore) permission on the LookML model that they want to access from Tableau.

### Set up OAuth for the Looker–Tableau BI Connector

To use the Looker–Tableau BI Connector for Tableau Server, you need to set up OAuth integration on your Looker instance.

You can use the Looker [API Explorer](/looker/docs/api-explorer) to set up OAuth integration for the Looker–Tableau BI Connector:
...
To install Looker–Tableau BI Connector on Tableau Server, perform the following steps:
  ...
  1. Download the Tableau connector file by clicking the following link: [`looker_v1.0.0.taco`](/looker/docs/file-downloads/looker_v1.0.0.taco)
...
After you have set up OAuth for the Looker–Tableau BI Connector and installed the Looker–Tableau BI Connector, you can complete the setup by performing the following procedures on your Tableau Server host machine:
  ...
  * Register the OAuth client ID and the OAuth instance URL (the redirect URL) that you specified when you set up OAuth for the Looker–Tableau BI Connector. Refer to [Tableau's documentation](https://help.tableau.com/current/server/en-us/config_oauth_google.htm#2-register-the-oauth-client-id-and-client-secret) for an example using a similar connector.
  * Validate and update saved credentials. Refer to [Tableau's documentation](https://help.tableau.com/current/server/en-us/config_oauth_google.htm#3-validate-and-update-saved-credentials) for an example using a similar connector.

## Things to consider

When you're exploring Looker data with Tableau Desktop, note the following considerations:

  * The Looker–Tableau BI Connector is built upon the Looker Open SQL Interface, and it has the same LookML limitations and SQL limitations as the Open SQL Interface. See the [Open SQL Interface](/looker/docs/sql-interface#lookml_limitations) documentation for more details.
  * Tableau automatically queries the database whenever a change is made to the query, including when fields are added and removed using the Looker–Tableau BI Connector. You can turn off automatic updates in Tableau: Refer to the [Tableau documentation](https://help.tableau.com/current/pro/desktop/en-us/queries_autoupdates.htm) for more information.
  * You cannot use Tableau to join two Looker Explores. If you want to join Explores, use Looker to create the joins in your Looker model. See the Looker documentation pages [Working with joins in LookML](/looker/docs/working-with-joins) and [join parameters](/looker/docs/reference/param-join) for information on using joins in Looker.
  * The Looker–Tableau BI Connector is designed to work with a [live connection](https://www.tableau.com/blog/tableau-cloud-tips-extracts-live-connections-cloud-data) in Tableau. Tableau's [data extract mode](https://help.tableau.com/current/pro/desktop/en-us/extracting_data.htm) extracts [Looker measures](/looker/docs/lookml-terms-and-concepts#dimension_and_measure_fields) with a value of "null" and therefore won't produce accurate results. If you want to use extract mode, you can create [aggregate fields](https://help.tableau.com/current/pro/desktop/en-us/calculations_calculatedfields_aggregate_create.htm) in Tableau directly off the Looker dimension fields instead of using Looker measures.
...
To upgrade to the latest Looker–Tableau BI Connector version, a Looker admin must perform the following tasks:
  ...
  2. On the Looker instance, enable the **Tableau Desktop** toggle on the [**BI Connectors**](/looker/docs/bi-connectors) page.
...
After a Looker admin enables the **Tableau Desktop** toggle on the Looker instance, your Looker instance users can do the following to use the latest Looker–Tableau BI Connector:
  ...
  * Download and save the Tableau Connector file
  * Download and save the JDBC driver (`looker.jar`) file
...
**Note:** Once you remove the Private Preview version of the Tableau OAuth app from your Looker instance, your users will need the latest version of the Tableau Connector file to connect to data from your Looker using Tableau Desktop.
