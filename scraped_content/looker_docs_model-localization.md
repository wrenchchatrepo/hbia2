# https://cloud.google.com/looker/docs/model-localization
...
**Note:** Model localization is not currently compatible with [project import](/looker/docs/importing-projects).
...
Model localization often occurs in conjunction with an admin who specifies [number format localization](/looker/docs/localizing-number-formatting) and [user-interface language](/looker/docs/supported-user-interface-languages) settings.
...
You can localize labels, group labels, and descriptions in your model, including the following:
  ...
  * `label` for [field](/looker/docs/reference/param-field-label), [model](/looker/docs/reference/param-model-label), [Explore](/looker/docs/reference/param-explore-label), or [view](/looker/docs/reference/param-view-label)
  * [`label` for applications](/looker/docs/reference/param-manifest-application#label) in the Looker extension framework
  * [`group_label` for Explore](/looker/docs/reference/param-explore-group-label) and [`group_label` for field](/looker/docs/reference/param-field-group-label)
  * [`group_item_label` for field](/looker/docs/reference/param-field-group-item-label)
  * [`description` for Explore](/looker/docs/reference/param-explore-description) and [`description` for field](/looker/docs/reference/param-field-description)

**Tip:** Localization is not supported for [`dimension_group`](/looker/docs/reference/param-field-dimension-group). Instead, use `group_label` and `group_item_label` at the field level to create your own set of dimensions that can be localized.

You can also create localized [LookML dashboards](/looker/docs/building-lookml-dashboards) in your project. The following LookML dashboard parameters can be localized:

  * [`title`](/looker/docs/reference/param-lookml-dashboard#title_\(for_dashboard\))
  * [`description`](/looker/docs/reference/param-lookml-dashboard#description_\(for_dashboard\))
  * [`text`](/looker/docs/reference/param-lookml-dashboard-column-chart#text) (which is a subparameter of the `note` parameter and can be applied to all dashboard element types, besides elements of `type: text`)
  * [`comparison_label`](/looker/docs/reference/param-lookml-dashboard-single-value-chart#comparison_label)
  * [`single_value_title`](/looker/docs/reference/param-lookml-dashboard-single-value-chart#single_value_title)
...
**Tip:** Model localization doesn't have to be based on geographic location or language. However, if you are using model localization and you would also like to use Looker's natively localized dashboard and visualization user interface, you must match the title of the strings file with Looker's [supported locale codes](/looker/docs/supported-user-interface-languages#localizing_the_looker_user_interface).
...
To enable localization for your project, add the [`localization_settings`](/looker/docs/reference/param-manifest-localization-settings) parameter to your project's manifest file.

**Tip:** If your project doesn't already have one, you can [create a manifest file](/looker/docs/ide-folders#creating_files) from the **+** icon at the top of the file browser in the Looker IDE.
...
The default locale _for your project_ is not to be confused with the default locale _for Looker users_. Your Looker admin can [set a default locale](/looker/docs/admin-panel-general-localization) for your instance. If no default is set, Looker will default to `en`. If your admin does not specifically enter a [**Locale**](/looker/docs/admin-panel-users-users#locale) value for a [user](/looker/docs/admin-panel-users-user-attributes#assigning_values_to_individual_users) or a [user group](/looker/docs/admin-panel-users-user-attributes#assigning_values_to_user_groups) the user belongs to, Looker assigns the user to the default instance locale. And, if the admin has not set a default instance locale, Looker assigns the user to the `en` locale.
...
Once you have your locale strings files set up, you can assign users to a locale that corresponds to one of the locale strings files. This can be done at the [instance](/looker/docs/admin-panel-general-localization), [user group](/looker/docs/admin-panel-users-user-attributes#assigning_values_to_user_groups), or [individual user](/looker/docs/admin-panel-users-users#locale) level, using the **Locale** field or `locale` user attribute.

For example, if you want a user to see the labels and descriptions defined in the `es_ES.strings.json` file, your Looker admin should set the user's [**Locale**](/looker/docs/admin-panel-users-users#locale) setting to `es_ES`.

Custom locales you create with string files can be entered in the **Locale** field by clicking the field and typing the string filename rather than selecting a built-in locale from the drop-down menu. For more information, see the [Users](/looker/docs/admin-panel-users-users#setting_a_custom_locale) documentation page.

**Note:** When a locale is not set for users at the individual or user group level, Looker will assign users to the instance locale. However, if there is no `.strings` file set up for the instance locale, model localization will not work for those users. For that reason, a `.strings` file should be set up for each instance's default locale. You can learn more in the `default_locale` section or the **Creating locale strings files** sections.

## Setting locale for signed embed users

You can include a user's locale value in a [signed embed URL](/looker/docs/single-sign-on-embedding#user_attributes_param) just like any other user attribute. The exact format required for signed embeds depends on the programming language that is used to build your signed embed URL script, but the name of the user attribute is `locale`. See the [Signed embedding](/looker/docs/single-sign-on-embedding) documentation page for more information on signed embed URLs and tools for building your signed embed URL.

## Using locale in Liquid variables

As described previously, model localization lets you customize the display of your model's labels and descriptions for different locales. But you can also include localization keys in [Liquid variables](/looker/docs/liquid-variable-reference), which lets you localize your data values as well.
...
You can also use Liquid in [LookML dashboard filters](/looker/docs/reference/param-lookml-dashboard#default_value) and [LookML dashboard element filters](/looker/docs/reference/param-lookml-dashboard-table-chart#filters) to localize the default value in a filter. For example, if a LookML dashboard has a tile using data from this localized model, and there is a filter on that tile defined in LookML as follows:
...
Be aware that localization rules apply when you are [extending views, Explores, or LookML dashboards](/looker/docs/reusing-code-with-extends), and when you are [refining views or Explores](/looker/docs/lookml-refinements).
