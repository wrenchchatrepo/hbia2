# https://cloud.google.com/looker/docs/access-levels

Depth: 3

Different levels of content access determine which users may view and edit content in Looker [folders](/looker/docs/organizing-spaces). Whereas [**permissions**](/looker/docs/access-control-and-permission-management) are associated with a user according to that person's role, [_content access_](/looker/docs/admin-panel-users-content-access) is associated with a folder, and defines how open the folder is to users at various levels.

## Types of access to folders

One of two access levels can be assigned to each Looker [user](/looker/docs/admin-panel-users-users) or [group](/looker/docs/admin-panel-users-groups) for any given folder:
  ...
  * **Manage Access, Edit** : This access level lets a user do everything that the **View** access level does, plus make changes to the folder, such as the following:
    ...
    * [Editing Looks](/looker/docs/saving-and-editing-looks) and [editing dashboards](/looker/docs/editing-user-defined-dashboards) in the folder
    * [Specifying which users and groups of users can view or manage the folder](/looker/docs/organizing-spaces#managing_access_levels_for_a_space)
    * [Creating subfolders](/looker/docs/organizing-spaces#creating_a_space)
    * [Renaming, moving, and deleting a folder](/looker/docs/organizing-spaces#renaming,_moving,_and_deleting_a_space)
    * [Moving Looks and dashboards](/looker/docs/organizing-spaces#copying_and_moving_looks_and_dashboards)
    * [Deleting Looks and dashboards](/looker/docs/organizing-spaces#deleting_looks_and_dashboards)

For additional discussion of content access and permissions, see [Controlling user content access](/looker/docs/access-control-and-permission-management#controlling_user_content_access) and [How content access and permissions interact](/looker/docs/access-control-and-permission-management#how_content_access_and_permissions_interact).
...
Once you determine what kind of system you want, this page will walk you through the steps to configure it. For the initial setup, we recommend using the [**Content Access**](/looker/docs/admin-panel-users-content-access) section of the **Admin** panel, as it's a single place to make changes to each folder.
...
If you're planning to restrict access to content, Looker groups make things much easier. Groups can be granted access to folders and subfolders the same way that users are, and groups can contain other groups. For information about how to configure groups, see the [Groups page](http://looker.com/docs/admin-options/settings/groups).
...
**Note:** Only enable the [Closed System option](/looker/docs/admin-panel-general-settings#closed_system) if you plan to private label Looker or use signed embed for your customers. Internal use cases should use a different system. You need to be a Looker admin to fully configure your system in the way described in the following sections.

Looker offers a **Closed System** option that makes the following changes:

  * Removes the **All Users** group. There will be no way to refer to all the users in the system as one group. Instead, Looker admins should create one group per tenant, or customer, as discussed in the [Configure groups to provide granular access](/looker/docs/looker-core-create-oauth#configure_the_user_consent_screen_scopes_and_test_users) section. For this discussion, we'll assume that each customer is a company.
...
**Note:** The [`see_queries`](/looker/docs/admin-panel-users-roles#see_queries), [`see_schedules`](/looker/docs/admin-panel-users-roles#see_schedules), and [`see_users`](/looker/docs/admin-panel-users-roles#see_users) permissions all provide access to an **Admin** panel and override the **Closed System** option. So, if a user has one or more of the `see_queries`, `see_schedules`, or `see_users` permissions, that user will be able to see the members of Companies A, B, and C in the associated **Admin** panel.

The [Homepage: Recently Viewed Content](/looker/docs/finding-content#recently_viewed_content) is an example of a place in Looker where that user will only see other Company C members and their content.
...
To request that the **Closed System** option be enabled for your instance, [contact a Google Cloud sales specialist](https://cloud.google.com/contact) or [open a support request](http://console.cloud.google.com/support/cases).
...
For information about how to configure groups, see the [Groups](/looker/docs/admin-panel-users-groups) documentation page.
