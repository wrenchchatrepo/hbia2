...
Consider a [derived table](/looker/docs/derived-tables) that calculates a customer's lifetime spend, within the northeast region:
...
**Note:** If a derived table makes use of a templated filter, you cannot [make the table persistent](/looker/docs/derived-tables#creating_persistent_derived_tables).
...
Both `filter` and `parameter` fields can accept a series of child parameters, allowing you to customize how they operate. See the [Field parameters](/looker/docs/reference/param-field) documentation page for a complete list. There are two options that bear special mentioning for `parameter` fields.

First, `parameter` fields can have a special type called [unquoted](/looker/docs/reference/param-dimension-filter-parameter-types#unquoted):
...
Second, `parameter` fields have an option called [allowed values](/looker/docs/reference/param-field-parameter#specifying_allowed_values) that let you associate a user-friendly name with the value you want to insert. For example:
...
The second step is to use [Liquid](/looker/docs/liquid-variable-reference) to add the templated filter or Liquid parameter as desired.
...
**Note:** This is often a point of confusion among Looker developers. Templated filters always result in a logical expression of some kind, and not the individual value entered by a user.
...
Although templated filters and Liquid parameters are similar, there is an important difference between them:
  ...
  * **Liquid parameters** insert user input directly (or using the values you define with [allowed values](/looker/docs/reference/param-field-parameter#specifying_allowed_values)).
