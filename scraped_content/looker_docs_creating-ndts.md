...
A [derived table](/looker/docs/derived-tables) is a query whose results are used as if the derived table were a physical table in the database. A [_native derived table_](/looker/docs/derived-tables#native_derived_tables) is based on a query that you define using LookML terms. This is different from a [_SQL-based derived table_](/looker/docs/derived-tables#sql-based_derived_tables), which is based on a query that you define with SQL terms. Compared to SQL-based derived tables, native derived tables are much easier to read and understand as you model your data. See the [Native derived tables and SQL-based derived tables](/looker/docs/derived-tables#native_derived_tables_and_sql-based_derived_tables) section of the [Derived tables in Looker](/looker/docs/derived-tables) documentation page for more information.

Both native and SQL-based derived tables are defined in LookML using the [`derived_table`](/looker/docs/reference/param-view-derived-table) parameter at the [view](/looker/docs/reference/param-view) level. However, with native derived tables, you do not need to create a SQL query. Instead, you use the [`explore_source`](/looker/docs/reference/param-view-explore-source) parameter to specify the Explore on which to base the derived table, the desired columns, and other desired characteristics.

> You can also have Looker create the derived table LookML from a [SQL Runner](/looker/docs/sql-runner-basics) query, as described on the [Using SQL Runner to create derived tables](/looker/docs/sql-runner-basics-create-dts#creating_a_derived_table_from_a_query) documentation page.
...
![](/static/looker/docs/images/explore-get-derived-table-lookml-2314.png)

  2. Click the **Derived Table** tab to see the LookML for creating a native derived table for the Explore.

![](/static/looker/docs/images/explore-get-derived-table-code-2402.png)

  3. Copy the LookML.

Now that you have copied the generated LookML, paste it into a view file:

  1. In [Development Mode](/looker/docs/dev-mode-prod-mode#development_mode), navigate to your [project files](/looker/docs/accessing-project-files).

  2. Click the **+** at the top of the project file list in the Looker IDE and select **Create View**. Alternatively, you can click a folder's menu and select **Create View** from the menu to [create the file inside the folder](/looker/docs/ide-folders#creating_items_within_a_folder).

  3. Set the view name to something meaningful.

  4. Optionally, change column names, specify derived columns, and add filters.

> When you use a [measure](/looker/docs/reference/param-measure-types#count) of `type: count` in an Explore, the visualization labels the resulting values with the view name rather than the word _Count_. To avoid confusion, pluralize your view name. You can change the view name by either selecting **Show Full Field Name** under **Series** in the visualization settings or by using the [`view_label`](/looker/docs/reference/param-field-view-label) parameter with a pluralized version of your view name.

## Defining a native derived table in LookML

Whether you use derived tables declared in SQL or native LookML, the output of a [`derived_table`'s](/looker/docs/reference/param-view-derived-table) query is a table with a set of columns. When the derived table is expressed in SQL, the output column names are implied by the SQL query. For example, the following SQL query will have the output columns `user_id`, `lifetime_number_of_orders`, and `lifetime_customer_value`:
...
In the native derived table's view file, you use the [`explore_source`](/looker/docs/reference/param-view-explore-source) parameter to point to an Explore and to define the columns and other characteristics for the native derived table.

In the native derived table's view file, you're not required to use the [`include`](/looker/docs/reference/param-model-include) parameter to point to the file that contains the Explore's definition. If you don't have the `include` statement, the Looker IDE won't [autosuggest](/looker/docs/looker-ide#autosuggest) field names or verify your field references as you build the native derived table. Instead, you can use the [LookML Validator](/looker/docs/lookml-validation#validating_your_lookml) to verify the fields that you are referencing in your native derived table.
...
Explores are often defined within a model file, but, in the case of native derived tables, it's cleaner to create a separate file for the Explore. LookML Explore files have the `.explore.lkml` file extension, as described in the documentation for [Creating Explore Files](/looker/docs/lookml-project-files#creating_explore_files). That way, in your native derived table view file you can include a single Explore file and not the entire model file.

**Note:** When you create an Explore file, the file must have the `.explore.lkml` extension at the end of its filename. If you're [uploading the file](/looker/docs/creating-project-files#uploading_files), you must use the full `.explore.lkml` extension. If you're [using the Looker IDE to create the file](/looker/docs/creating-project-files#creating_files) use the **Generic LookML files** option. Looker will add `.lkml` to the end of the filename if the extension isn't there, but you still must manually add the `.explore` part of the extension to the filename.
...
**Note:** Explore files will listen to the connection of the model that the Explore files are [included](/looker/docs/reference/param-model-include#including_explores_in_a_model) in. Consider this fact when you include Explore files in models that are configured with a connection that is different from the Explore file's parent model. If the schema for the including model's connection differs from the schema for the parent model's connection, it can cause query errors.
...
You can add `derived_column` parameters to specify columns that don't exist in the [`explore_source`](/looker/docs/reference/param-view-explore-source) parameter's Explore. Each `derived_column` parameter has a `sql` parameter specifying how to construct the value.
...
You would just add a filter to the [`derived_table`](/looker/docs/reference/param-view-derived-table) that filters for transactions in the last 90 days. The [`filters`](/looker/docs/reference/param-view-explore-source#filters) parameter for a derived table uses the same syntax as you use to create a [filtered measure](/looker/docs/reference/param-field-filters).

**Note:** In the `filters` parameter, use the `view_name.field_name` format to scope the field name with the view name. You must include the view name for the field, even if the field is in the Explore's base view.
...
In addition, you can use the [`dev_filters`](/looker/docs/reference/param-view-explore-source#dev_filters) subparameter of `explore_source` with a native derived table. The `dev_filters` parameter lets you specify filters that Looker applies only to development versions of the derived table, which means that you can build smaller, filtered versions of the table to iterate and test without waiting for the full table to build after each change.
...
See [Working faster in Development Mode](/looker/docs/derived-tables#working_faster_in_development_mode) for more information.

### Using templated filters

You can use [`bind_filters`](/looker/docs/reference/param-view-explore-source#bind_filters_example) to include [templated filters](/looker/docs/templated-filters):
...
You can also use the [`bind_all_filters`](/looker/docs/reference/param-view-explore-source#bind_all_filters_example) subparameter of `explore_source` to pass all runtime filters from an Explore to a native derived table subquery. See the [`explore_source`](/looker/docs/reference/param-view-explore-source#defining_a_native_derived_table) parameter documentation page for more information.

## Sorting and limiting native derived tables

You can also [sort](/looker/docs/reference/param-view-explore-source#sort) and [limit](/looker/docs/reference/param-view-explore-source#limit) the derived tables, if desired:
...
When you use the `timezone` subparameter, all time-based data in the native derived table will be converted to the time zone you specify. See the [`timezone` values](/looker/docs/reference/param-view-timezone-values) documentation page for a list of the supported time zones.

If you don't specify a time zone in your native derived table definition, the native derived table will not perform any time zone conversion on time-based data, and instead time-based data will default to your [database time zone](/looker/docs/connecting-to-your-db#database_time_zone).

If the native derived table is not [persistent](/looker/docs/derived-tables#adding_persistence), you can set the time zone value to `"query_timezone"` to automatically use the time zone of the currently running query.