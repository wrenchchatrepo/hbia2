...
**Caution:** In Explores, [basic filters](/looker/docs/filtering-and-limiting#basic_filters) use some phrases that differ in meaning from the filters expressions documented on this page. The filter expressions described on this page should only be used with the Explore [**matches (advanced)** option](/looker/docs/filtering-and-limiting#advanced_matches_filters).**Tip:** When using filter expressions in LookML, you should place the expression in quotation marks (see the [`filters`](/looker/docs/reference/param-field-filters) documentation page for proper use). This is especially important for logical values like `NULL`. 

When using filter expressions in the Explore section of Looker, you don't need to place the expression in quotation marks.

**Note:** Filter expressions aren't supported with access grants. See the [`access_grant`](/looker/docs/reference/param-model-access-grant#values_listed_in_allowed_values_must_match_user_attribute_values_exactly) documentation page for more information.

## String

**Note:** Matches in string filters depend on the [`case_sensitive`](/looker/docs/reference/param-model-case-sensitive) setting in your model file, and on whether your dialect supports case-sensitivity. For example, if `case_sensitive` is enabled in your model, the expression `FOO%` will _not_ match the word "food". If `case_sensitive` isn't enabled, or if your dialect doesn't support case-sensitivity, the expression `FOO%` will match the word "food". Example | Description  
---|---  
...
`NULL` | value is null (when it is used as part of a LookML filter expression, place `NULL` in quotes, as shown on the [`filters`](/looker/docs/reference/param-field-filters) documentation page)  
...
`-NULL` | value of column is not null (when it is used as part of a LookML filter expression, place `-NULL` in quotes, as shown on the [`filters`](/looker/docs/reference/param-field-filters) documentation page)  
...
**Note:** In some cases you may also want to escape or include special characters in string filters when using custom drills to Looker content. See the [Things to consider when linking to Looker content](/looker/docs/reference/param-field-link#things_to_consider_when_linking_to_looker_content) section on the `Link` parameter page for more information.
...
`FY2018` | entire fiscal year starting in 2018 (if your Looker developers have [specified that your fiscal year](/looker/docs/reference/param-model-fiscal-month-offset) starts in April then this is 2018/04/01 00:00 through 2019/03/31 23:59)  
`FY2018-Q1` | first quarter of the fiscal year starting in 2018 (if your Looker developers have [specified that your fiscal year](/looker/docs/reference/param-model-fiscal-month-offset) starts in April then this is 2018/04/01 00:00:00 through 2018/06/30 23:59:59)  
...
For all of the following examples, assume today is **Friday, 2018/05/18 18:30:02**. In Looker, weeks start on Monday unless you change that setting with [`week_start_day`](/looker/docs/reference/param-model-week-start-day).
...
**Note:** If your Looker developers have [specified using a fiscal year](/looker/docs/reference/param-model-fiscal-month-offset) then you can type `fiscal` in these expressions to use a fiscal quarter instead of a calendar quarter. For example, you can use `last fiscal quarter`.
...
**Note:** If your Looker developers have [specified using a fiscal year](/looker/docs/reference/param-model-fiscal-month-offset) then you can type `fiscal` in these expressions to use a fiscal year instead of a calendar quarter. For example, you can use `last fiscal year`.
...
Example | Description  
---|---  
`yes` or `Yes` | field evaluates to true Looker developers: for [`type: yesno`](/looker/docs/reference/param-dimension-filter-parameter-types#yesno) dimensions use lowercase, for `filters` parameters (like those [used in a measure](/looker/docs/reference/param-field-filters) or [used in an `always_filter`](/looker/docs/reference/param-explore-always-filter)) use uppercase  
`no` or `No` | field evaluates to false Looker developers: for [`type: yesno`](/looker/docs/reference/param-dimension-filter-parameter-types#yesno) dimensions use lowercase, for `filters` parameters (like those [used in a measure](/looker/docs/reference/param-field-filters) or [used in an `always_filter`](/looker/docs/reference/param-explore-always-filter)) use uppercase  
...
**Note:** The syntax for numeric filter expressions using `NOT` may not be intuitive. If the first filter condition contains a `NOT`, and no other filter conditions contains a `NOT`, then all of the filter conditions will be negated. See the following examples for more information. Example | Description  
---|---  
`5` | is exactly 5  
...
`NULL` | has no data in it (when it is used as part of a LookML filter expression, place `NULL` in quotes, as shown on the [`filters`](/looker/docs/reference/param-field-filters) documentation page)  
`NOT NULL` | has some data in it (when it is used as part of a LookML filter expression, place `NOT NULL` in quotes, as shown on the [`filters`](/looker/docs/reference/param-field-filters) documentation page)  
...
`NOT NULL (works the same as -NULL)` | location has both a non-null latitude and a non-null longitude (when it is used as part of a LookML filter expression, place `NOT NULL` in quotes, as shown on the [`filters`](/looker/docs/reference/param-field-filters) documentation page)  
`-NULL (works the same as NOT NULL)` | location has both a non-null latitude and a non-null longitude (when it is used as part of a LookML filter expression, place `-NULL` in quotes, as shown on the [`filters`](/looker/docs/reference/param-field-filters) documentation page)  
`NULL` | location has a null latitude, or a null longitude, or both are null (when it is used as part of a LookML filter expression, place `NULL` in quotes, as shown on the [`filters`](/looker/docs/reference/param-field-filters) documentation page)  
...
To use the value of a [user attribute](/looker/docs/admin-panel-users-user-attributes) in a filter expression, reference the user attribute with the `_user_attributes` [Liquid variable](/looker/docs/liquid-variable-reference) using the syntax that is required by your database dialect:
    
    
    {{ _user_attributes['name_of_attribute'] }}
    

For example, suppose you need to apply an `sf_` prefix to the value of the `salesforce_username` user attribute because that is how the values are stored in your database. To add the prefix to the user attribute value, you can add a **matches (advanced)** filter on the relevant field and use the [`_user_attributes`](/looker/docs/liquid-variable-reference#user_attributes) Liquid variable in the filter expression as follows:
