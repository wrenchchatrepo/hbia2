# https://cloud.google.com/looker/docs/delivering-looks-explores

Depth: 3

This page provides an overview of delivering Looks and Explores to Looker's built-in delivery destinations and to any integrated services that are available for your Looker instance. For information about scheduling [dashboards](/looker/docs/viewing-dashboards), visit the [Scheduling and sending dashboards](/looker/docs/scheduling-and-sending-dashboards) documentation page.
...
> If you are a Looker developer, you must turn off [Development Mode](/looker/docs/dev-mode-prod-mode#switching_in_and_out_of_development_mode) to enable these scheduling options.
> 
> The delivery options available to you are dependent on the [delivery-related permissions](/looker/docs/configuring-deliveries) granted to you by your Looker admin.

## Opening the Scheduler from a folder

> For Explores, you can open the Scheduler from within the [Explore](/looker/docs/send-once#sending_a_query_from_an_explore) only, not from a folder.

To open the Scheduler from within a folder or subfolder, [find the folder or subfolder](/looker/docs/finding-content#navigating_to_dashboards_and_looks_from_folders) containing the dashboard you want to deliver. When you navigate to a folder, any Looks and dashboards within the folder are listed in [list view](/looker/docs/finding-content#list_view) or [grid view](/looker/docs/finding-content#grid_view).
...
![](/static/looker/docs/images/browse-folder-schedule-icon-718.png)

In [list view](/looker/docs/finding-content#list_view) you can open the Scheduler by:
...
In [grid view](/looker/docs/finding-content#grid_view), you can open the Scheduler by:
...
**Note:** Because Explores can be sent as one-time deliveries only, there is not a **Schedule** option in an Explore's gear menu. To schedule an Explore, choose the **Save and schedule** option to first [save the Explore as a Look](/looker/docs/saving-and-editing-looks#saving_looks_from_an_explore), and then complete the following instructions to schedule the newly saved Look.
...
Give the schedule a unique title in the **Give your schedule a name** field. The title will appear in the left panel after you save the schedule, in the list of [schedules you've created](/looker/docs/user-schedules), and on the schedule management pages in the **Admin** panel.
...
Destinations include:
  ...
  * Amazon S3 bucket
  * SFTP
  * Any [integrated services](/looker/docs/admin-panel-platform-actions#list_of_integrated_services) that your Looker admin has enabled for your instance — you can see a list of available services for delivering Looks and Explores in the Delivery options for third-party integrations section of this page
...
Depending on the settings for your Looker instance and on your assigned permissions, you may be able to send emails to email addresses that are unassociated with any user account on your Looker instance. These are classified as external emails. To deliver content to external users, your Looker admin must have granted you permissions to [deliver content to external users](/looker/docs/admin-panel-users-roles#schedule_external_look_emails) or have added those recipients' email domains to the [Email Domain Allowist for Scheduled Content](/looker/docs/admin-panel-general-settings#email_domain_allowlist_for_scheduled_content).

If a recipient is another Looker user, that person will see the option to link back to the dashboard from the email, unless your Looker admin has set your Looker instance's [emailed data policy](/looker/docs/admin-panel-alerts-and-schedules-scheduled-emails#emailed_data_policy) to [**Send Data Only**](/looker/docs/admin-panel-alerts-and-schedules-scheduled-emails#send_data_only) or the **Include links** option is deselected.

If all recipients unsubscribe from a scheduled email delivery, that schedule is deleted from Looker, even from the schedule management pages for [users](/looker/docs/user-schedules) and [admins](/looker/docs/admin-panel-alerts-and-schedules-schedule). If a recipient unsubscribes on behalf of an email group, no members of the group will receive scheduled email deliveries.
...
Webhooks are an increasingly common way to trigger exchanges between internet-based services. With a web service like [Zapier](https://zapier.com), webhooks can let Looker data be delivered to a wide range of applications; for example, you may be able to schedule periodic delivery of a Look or dashboard to a webhook. When you set up a delivery, Looker will [prompt you for the webhook address](/looker/docs/scheduling-webhook#delivering_to_webhook).
...
The exact procedure differs depending on what web service and destination application you're using to deliver the dashboard. See this Looker Community post on [Scheduling Looks and dashboards using webhooks (3.46+)](https://community.looker.com/looker-api-77/scheduling-looks-and-dashboards-using-webhooks-3-46-2593) for guidelines on setting up another application to receive the webhook data from Looker.
...
If you choose the Amazon S3 destination option, Looker prompts you for additional settings about formatting and about the Amazon S3 bucket as described on this page. See this Looker Community post on [Scheduling (unlimited) results to S3](https://community.looker.com/looker-api-77/scheduling-unlimited-results-to-s3-3785) for more details about delivering data using an Amazon S3 bucket.
...
Choose **SFTP** in the **Where should this data go?** section to deliver your dashboard to an [SFTP](https://en.wikipedia.org/wiki/File_Transfer_Protocol#Simple_File_Transfer_Protocol) server. Sending results to an SFTP server is a good method to use when your data or visualization is too large to send using email. If you choose this delivery option, Looker prompts you for the SFTP server address, username, and password as described next.

Note the following about SFTP deliveries:

  * To receive SFTP deliveries from Looker, be sure your network admin has added [Looker's IP addresses](/looker/docs/enabling-secure-db-access#option_1:_ip_address_whitelist) to your SFTP server's IP allowlist or inbound traffic rules.
...
Looker stores [SFTP fingerprints](/looker/docs/managing-business-user-features#storing_sftp_fingerprints) for your SFTP server. If you encounter errors with your SFTP delivery, it could indicate that the SFTP fingerprints are invalid. In this case, contact your Looker admin.

> If you receive the error `Java::JavaLang::NullPointerExceptions` when delivering data to an SFTP server, this may be due to a [bug in JRuby](https://github.com/jruby/jruby-openssl/issues/105) that prevents the use of [elliptic-curve (EC) algorithms](https://en.wikipedia.org/wiki/Elliptic-curve_cryptography) or host keys for SSH connections from Looker to SFTP destinations. To avoid issues with sending data to SFTP destinations, don't use EC algorithms in the `~/.ssh/config` or `/etc/ssh_config` files or store EC host keys in `~/.ssh` on your Looker host.
...
You can schedule or send a Look or the results of an Explore to a service that is integrated with Looker, such as Slack, Dropbox, Google Drive, and others. Integrations must be [enabled by your Looker admin](/looker/docs/admin-panel-platform-actions). Looker admins must specifically give permissions for [users](/looker/docs/admin-panel-users-roles#send_to_integration) and [embed users](/looker/docs/single-sign-on-embedding#send_to_integration) to send and schedule data to third-party integrations.
...
---|---|---|---|---|---|---|---  
Airtable | Add records to a table in Airtable. | [View README on GitHub](https://github.com/looker/actions/blob/master/src/actions/airtable/README.md) | [View Community article](https://www.googlecloudcommunity.com/gc/Exploring-Curating-Data/Looker-Actions-Airtable/td-p/566854) | None | Query | No | No  
...
Auger | Uses query result to build a predictive model. | [View README on GitHub](https://github.com/looker/actions/blob/master/src/actions/auger/README.md) | See README | None | Query | No | Yes  
Azure Storage | Send and store a data file on Azure Storage. | [View README on GitHub](https://github.com/looker/actions/blob/master/src/actions/azure/README.md) | [View Community article](https://www.googlecloudcommunity.com/gc/Exploring-Curating-Data/Looker-Actions-Azure-Storage/td-p/565929) | None | Query | No | Yes  
Braze | The Braze action lets you flag users within Braze using the REST API Endpoint from a Look. Ensure there's a `braze_id` field tagged in the results. MAX EXPORT: 10000. | [View README on GitHub](https://github.com/looker/actions/blob/master/src/actions/braze/README.md) | See README | braze_id | Query | No | Yes  
DataRobot | Send data to DataRobot and create a new project. | [View README on GitHub](https://github.com/looker-open-source/actions/blob/master/src/actions/datarobot/README.md) | See README | None | Query | No | Yes  
DigitalOcean — Stop Droplet | Stop DigitalOcean process using the DigitalOcean API. | [View README on GitHub](https://github.com/looker/actions/blob/master/src/actions/digitalocean/README.md) | [View Community article](https://www.googlecloudcommunity.com/gc/Exploring-Curating-Data/Looker-Actions-Digital-Ocean-Stop-Droplet/td-p/583753) | digitalocean_droplet_id | Field, query | No | No  
DigitalOcean Spaces | Send to and store a data file in DigitalOcean Storage. | [View README on GitHub](https://github.com/looker/actions/blob/master/src/actions/digitalocean/README.md) | [View Community article](https://www.googlecloudcommunity.com/gc/Exploring-Curating-Data/Looker-Actions-Digital-Ocean-Storage/td-p/583736) | None | Query | No | Yes  
Dropbox | Send and store a data file on Dropbox. | No README available | [View documentation](/looker/docs/delivering-dropbox) | None | Query | Yes | No  
Facebook Custom Audiences | Upload data to Facebook Ads Custom Audiences from Customer List. | [View README on GitHub](https://github.com/looker-open-source/actions/blob/master/src/actions/facebook/README.md) | See README | None | Query | Yes | Yes  
Firebase | Use Firebase to send push notifications to mobile. | No README available | No article available | None | Query | No | No  
Google Ads Customer Match | Upload data to Google Ads Customer Match. | [View README on GitHub](https://github.com/looker-open-source/actions/blob/master/src/actions/google/ads/README.md) | [View documentation](/looker/docs/best-practices/how-to-use-the-google-ads-integration) | None | Query | Yes | Yes  
Google Analytics Data Import | Upload data to a Google Analytics dataset. | [View README on GitHub](https://github.com/looker-open-source/actions/blob/master/src/actions/google/analytics/README.md) | See README | None | Query | Yes | Yes  
Google Cloud Storage | Write data files to a Google Cloud Storage bucket. | [View README on GitHub](https://github.com/looker-open-source/actions/blob/master/src/actions/google/gcs/README.md) | [View Community article](https://www.googlecloudcommunity.com/gc/Administering-Looker/Looker-Actions-Google-Cloud-Storage/td-p/591972) | None | Query | No | Yes  
Google Drive | Send data to Google Drive. | No README available | [View documentation](/looker/docs/best-practices/how-to-use-the-google-drive-integration) | None | Query | Yes | Yes  
Google Sheets | Send CSV data to a Google Sheet. | No README available | [View documentation](/looker/docs/best-practices/how-to-use-the-google-sheets-integration) | None | Query | Yes | Yes  
Hubspot Companies | Add properties to your Companies using the Hubspot V3 API. | [View README on GitHub](https://github.com/looker/actions/blob/master/src/actions/hubspot/README.md) | See README | hubspot_company_id | Query | No | Yes  
Hubspot Contacts | Add properties to your Contacts using the Hubspot V3 API. | [View README on GitHub](https://github.com/looker/actions/blob/master/src/actions/hubspot/README.md) | See README | hubspot_contact_id | Query | No | Yes  
Kloudio | Add data to a Google Sheet. | [View README on GitHub](https://github.com/looker/actions/blob/master/src/actions/kloudio/README.md) | See README | None | Query | No | No  
mParticle | Bulk export your user or event data from Looker to mParticle. | [View README on GitHub](https://github.com/looker/actions/blob/master/src/actions/mparticle/README.md) | See README | See README | Query | No | Yes  
Salesforce Campaigns | Add contacts or leads to Salesforce campaign. | [View README on GitHub](https://github.com/looker-open-source/actions/tree/master/src/actions/salesforce/campaigns/README.md) | See README | sfdc_contact_id or sfdc_lead_id | Query | Yes | No  
Segment Group | Add traits and/or users to your Segment groups. | [View README on GitHub](https://github.com/looker/actions/blob/master/src/actions/segment/README.md) | [View documentation](/looker/docs/segment) | segment_group_id and user_id, or segment_group_id and segment_anonymous_id | Query | No | Yes  
Segment Identify | Add traits to your Segment users using Identify. | [View README on GitHub](https://github.com/looker/actions/blob/master/src/actions/segment/README.md) | [View documentation](/looker/docs/segment) | email or user_id or segment_anonymous_id or segment_group_id | Query | No | Yes  
Segment Track | Connect to a number of integrations provided by Segment to identify and target users for marketing workflows. | [View README on GitHub](https://github.com/looker/actions/blob/master/src/actions/segment/README.md) | [View documentation](/looker/docs/segment) | email or user_id or segment_anonymous_id or segment_group_id | Query | No | Yes  
SendGrid | Send data and schedule results to send to a email address using SendGrid's API. | [View README on GitHub](https://github.com/looker/actions/blob/master/src/actions/sendgrid/README.md) | [View Community article](https://www.googlecloudcommunity.com/gc/Exploring-Curating-Data/Looker-Actions-Sendgrid/td-p/591971) | None | Query | No | No  
...
> For deliveries to email, the option to select a data format is not available if your Looker admin has set your Looker instance's [emailed data policy](/looker/docs/admin-panel-alerts-and-schedules-scheduled-emails#emailed_data_policy) to [**Send Link Only**](/looker/docs/admin-panel-alerts-and-schedules-scheduled-emails#send_link_only).
...
JSON — Simple | The data table as a JSON file attachment. In this format, Looker uses a dimension or measure's field name as its rendered value rather than the field's [label](/looker/docs/reference/param-field-label). | The data table as a JSON file attachment. In this format, Looker uses a dimension or measure's field name as its rendered value rather than the field's [label](/looker/docs/reference/param-field-label). | The data table as a JSON file attachment. In this format, Looker uses a dimension or measure's field name as its rendered value rather than the field's [label](/looker/docs/reference/param-field-label). | The data table as a JSON file attachment. In this format, Looker uses a dimension or measure's field name as its rendered value rather than the field's [label](/looker/docs/reference/param-field-label).  
JSON — Label | Not available for this destination | The data table in JSON format. In this format, Looker uses field [labels](/looker/docs/reference/param-field-label) as its rendered value in its JSON output. | Not available for this destination | Not available for this destination  
JSON — Simple, Inline | Not available for this destination | The data table submitted directly in JSON format. In this format, Looker uses a dimension or measure's field name as its rendered value rather than the field's [label](/looker/docs/reference/param-field-label). | Not available for this destination | Not available for this destination  
...
When scheduling recurring deliveries, you have two options to trigger deliveries:
  ...
  * **Datagroup update** : If your LookML developer has configured [datagroups](/looker/docs/reference/param-model-datagroup), you can schedule delivery to occur after a datagroup has been triggered, has managed the cache, and has [rebuilt relevant PDTs](/looker/docs/caching-and-datagroups#using_a_datagroup_for_a_pdt).
...
![](/static/looker/docs/images/delivery-schedule-datagroup-718.png)

To schedule content using a datagroup trigger, the model file on which the content is based needs to include a [datagroup](/looker/docs/reference/param-model-datagroup). Looker lists all datagroups defined in the models that are included in the given Look or dashboard tile, even if the datagroups are not used to handle caching for queries or PDTs.
...
![](/static/looker/docs/images/delivery-schedule-interval-718.png)
...
If you are delivering a Look or dashboard that has [filters](/looker/docs/filters-legacy-dashboards#adding_dashboard_filters_a_user_can_change), you can modify the filter conditions to further filter the results delivered.

Click **Filters** to see and modify the filters associated with this Look or dashboard for your delivery. Changing these filter options in the Scheduler does _not_ change the filters in your Look or dashboard. If a dashboard filter [requires](/looker/docs/filters-legacy-dashboards#requiring_a_filter_value) a filter value and the schedule filter is blank, a schedule will display no results upon delivery.
...
To further customize deliveries, you can use the **matches a user attribute** filter option (if your Looker admins have configured user-specific values called _user attributes_). Setting a filter to one of these attributes automatically customizes the results for each user, meaning that you can send customized results to many users even if you send a one-time delivery. You can read more about this on the [User attributes](/looker/docs/admin-panel-users-user-attributes) documentation page.

**Warning:** Complex filters are not applied to Look schedules. If you update a Look filter to a complex condition, such as applying multiple `AND` filters on the same field or applying multiple `OR` filters to different fields, the filter becomes a complex filter and Looker removes it from the schedule. This can cause Looker to send unfiltered data.
...
Format | Email | Webhook | Amazon S3 | SFTP  
---|---|---|---|---  
Data Table | Send this schedule if | _Not available for this destination_ | _Not available for this destination_ | _Not available for this destination_ | Limit  
Email options | Run schedule as recipient  
Include links  
Format options | Apply visualization options  
Formatted data values  
Timezone  
Visualization | Limit | _Not available for this destination_ | _Not available for this destination_ | _Not available for this destination_  
Email options | Run schedule as recipient  
Include links  
Table resolution  
Timezone  
...
Formatted data values | Formatted data values | Formatted data values | Formatted data values  
Timezone | Timezone | Timezone | Timezone  
JSON — Detailed, Inline | _Not available for this destination_ | Limit | Limit | Limit  
_Not available for this destination_ | Timezone | Timezone | Timezone  
Text  
Send this schedule if | Send this schedule if | Send this schedule if | Send this schedule if  
Limit | Limit | Limit | Limit  
Email options | Run schedule as recipient | _Not available for this destination_ | _Not available for this destination_ | _Not available for this destination_  
...
For recurring deliveries of Looks, you can customize the schedule to deliver the Look content only when certain conditions are met. [Alerts](/looker/docs/alerts-overview) provide a similar function for dashboard tiles.
...
You can limit the size of your delivery by selecting one of the following limit options for your data:
  ...
  * **All Results:** Data is streamed (see more in the [What are all the row limits in Looker?](/looker/docs/best-practices/row-limits-in-looker) Best Practices page). This is useful for large datasets, as it bypasses file-size, browser, and memory limitations.
...
The **Run schedule as recipient** option lets admins specify a list of users who will each receive the content delivery as if they ran the query, based on their permissions. Non-admin users may also enter their own email address to receive a delivery as if they had queried the content, based on their permissions. This means that each user's [access filters](/looker/docs/reference/param-explore-access-filter) and [user attributes](/looker/docs/admin-panel-users-user-attributes) will be applied to the data included in each email.
...
![](/static/looker/docs/images/delivery-run-as-recip-full-2316.png)
...
![](/static/looker/docs/images/delivery-run-as-recip-access-filter-2308.png)
...
In some cases, the **Run schedule as recipient** option is not available:
  ...
  3. When a Looker admin adds an email that does not belong to a Looker user (a Looker account is indicated by a Looker icon next to the recipient) ![](/static/looker/docs/images/delivery-schedule-looker-icon-718.png)
...
If your Looker admin has set your Looker instance's [emailed data policy](/looker/docs/admin-panel-alerts-and-schedules-scheduled-emails#emailed_data_policy) to [**Send Links and Data**](/looker/docs/admin-panel-alerts-and-schedules-scheduled-emails#send_links_and_data), the Scheduler will display an option to **Include links**. When this option is checked, email data deliveries include a link back to Looker that says "View full report" for delivered Looks and Explores and "View full dashboard" for delivered dashboards.
...
> This option is not available for Look or Explore deliveries in Visualization format.
  ...
  * Select **Apply visualization options** for your data delivery to appear similar to a [table chart](/looker/docs/legacy-table-options). Any of the following settings in the [Plot](/looker/docs/legacy-table-options#plot_menu_options), [Series](/looker/docs/legacy-table-options#series_menu_options), and [Formatting](/looker/docs/legacy-table-options#formatting_menu_options) menus that are configured for the visualization will be applied:

    * [Show Row Numbers](/looker/docs/legacy-table-options#show_row_numbers)
    * [Hide Totals](/looker/docs/legacy-table-options#hide_totals)
    * [Hide Row Totals](/looker/docs/legacy-table-options#hide_row_totals)
    * [Limit Displayed Rows](/looker/docs/legacy-table-options#limit_displayed_rows) to a maximum of 500 rows shown or hidden
    * [Show Full Field Name](/looker/docs/legacy-table-options#show_full_field_name)
    * [Custom labels](/looker/docs/legacy-table-options#customizations) for each column (the **JSON — Detail, Inline** format always uses raw field names, not the field [label](/looker/docs/reference/param-field-label) — see the [Change in JSON formatting](https://community.looker.com/lookml-5/change-in-json-render-formatting-6-22-14465) Community post for more information about how Looker renders fields in JSON format)
    * [Conditional Formatting](/looker/docs/legacy-table-options#conditional_formatting) for deliveries of table chart visualizations in Excel format
...
> Conditional formatting displays in Excel deliveries of Looks and Explores with table chart visualizations only if the **Along a scale** rule is applied.
  ...
  * Unselect **Apply visualization options** for Looker to send the data as it appears in the **Data** section of the Look or a dashboard tile's **Explore from Here** window. **JSON — Label** uses the field [label](/looker/docs/reference/param-field-label), even if **Apply visualization options** is unselected.
...
By default, Looker uses the time zone associated with your account to determine when to send your data delivery. If you don't have a time zone associated with your account, Looker uses your [Application time zone](/looker/docs/admin-panel-general-settings#application_time_zone). If you want to specify a different time zone, select **Use custom timezone**. The time zone you select does not affect the data in your Look or dashboard, just the timing of the delivery.
...
You can see a list of all schedules that you have created on the **Schedules You've Created** page, which you can reach by clicking on your user profile icon and selecting **Schedules**. See the [Viewing Your Scheduled Data Deliveries](/looker/docs/user-schedules) page for more information about viewing and managing the schedules that you've created.

### Viewing deliveries created by other users

If your Looker admin has granted you [permissions to view the **Schedules** and **Schedule History** admin pages](/looker/docs/configuring-deliveries#granting_scheduling_permissions_for_users), you can see the list of all schedules for the entire Looker instance. You will not be able to edit or delete those schedules, however.
...
A few other challenges can arise:
  ...
  * If your company has set up its own [email settings](/looker/docs/admin-panel-platform-smtp) in the **Admin** section of Looker, your Looker admins will need to make sure that the settings are correct and that your email provider is functioning properly.
