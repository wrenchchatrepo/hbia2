# https://cloud.google.com/looker/docs/custom-fields

Depth: 3

**Note:** This page is part of the [Retrieve and chart data](/looker/docs/retrieve-and-chart-data) learning series.

Most fields in the [field picker](/looker/docs/changing-explore-menu-and-field-picker#field_picker) are created by your Looker developers. By using custom fields, however, you can create new ad hoc custom dimensions and measures in an Explore. This page discusses how to create and use custom fields to enhance your data analysis.

To use custom fields, your Looker admin must grant the [`create_custom_fields`](/looker/docs/admin-panel-users-roles#create_custom_fields) permission to users or groups to allow access to the feature.
...
There are a few differences between custom fields and dimensions and measures that are defined in LookML, including:
  ...
  * Custom fields only persist in an Explore's field picker for [certain users](/looker/docs/custom-fields#viewing_and_using_custom_fields).

There are a few differences between custom fields and [table calculations](/looker/docs/table-calculations):
...
S = Works with the [`string` data type](/looker/docs/reference/param-dimension-filter-parameter-types#string)

N = Works with the [`number` data type](/looker/docs/reference/param-dimension-filter-parameter-types#number) and numeric measures

T = Works with the [`tier` data type](/looker/docs/reference/param-dimension-filter-parameter-types#tier)

ZC = Works with the [`zipcode` data type](/looker/docs/reference/param-dimension-filter-parameter-types#zipcode)

YN = Works with the [`yesno` data type](/looker/docs/reference/param-dimension-filter-parameter-types#yesno)

DI = Works with the [`distance` data type](/looker/docs/reference/param-dimension-filter-parameter-types#distance)

DU = Works with the [`duration` data type](/looker/docs/reference/param-dimension-filter-parameter-types#duration)

TS = Works with most [timestamp data types*****](/looker/docs/reference/param-dimension-filter-parameter-types#time)

ID = Works with most [integer date data types](/looker/docs/reference/param-dimension-filter-parameter-types#time)*****

SD = Works with most [string date data types](/looker/docs/reference/param-dimension-filter-parameter-types#time)*****

L = Works with the [`location` data type](/looker/docs/reference/param-dimension-filter-parameter-types#location)

Each custom field type that you can create from a dimension falls into one of the following categories:

  * **Aggregate** — Aggregated fields are [measures](/looker/docs/reference/param-measure-types) types that perform aggregations, such as sum and average. Aggregate measures can reference only dimensions, not other measures.
  * **Non-aggregate** — Non-aggregated fields are [dimensions](/looker/docs/reference/param-dimension-filter-parameter-types) that can be grouped by in an Explore query.
...
Count distinct  |  Aggregate  |  Creates a [`count_distinct`](/looker/docs/reference/param-measure-types#count_distinct) measure that calculates the number of distinct values for a selected dimension.  |  S N T ZC YN DI DU TS ID SD L  
  
### Sum

Sum  |  Aggregate  |  Creates a [`sum`](/looker/docs/reference/param-measure-types#sum) measure that adds up the values of a selected dimension.  |  N DI DU ID  
  
### Average

Average  |  Aggregate  |  Creates an [`average`](/looker/docs/reference/param-measure-types#average) measure that averages the values of a selected dimension.  |  N DI DU ID  
  
### Min

Min  |  Aggregate  |  Creates a [`min`](/looker/docs/reference/param-measure-types#min) measure that finds the smallest value of a selected dimension.  |  N DI DU ID  
  
### Max

Max  |  Aggregate  |  Creates a [`max`](/looker/docs/reference/param-measure-types#max) measure that finds the largest value of a selected dimension.  |  N DI DU ID  
  
### Median

Median  |  Aggregate  |  Creates a [`median`](/looker/docs/reference/param-measure-types#median) measure that finds the midpoint value for the values of a selected dimension.  |  N DI DU ID  
  
### List of unique values

List of unique values  |  Aggregate  |  Creates a [`list`](/looker/docs/reference/param-measure-types#list) measure that creates a list of the distinct values of a selected dimension.  |  S T ZC YN SD L  
...
***** **Note about date and time data:** Looker interprets some [timeframes](/looker/docs/reference/param-field-dimension-group#timeframe_options) and [time-based types](/looker/docs/reference/param-dimension-filter-parameter-types#available_time-based_types) as different data types when you're creating custom fields, and this can affect the custom field types that are available for those timeframe and time-based fields. For example, Looker interprets the [`hour_of_day` timeframe](/looker/docs/reference/param-field-dimension-group#hour_of_day) as a [`number` data type](/looker/docs/reference/param-dimension-filter-parameter-types#number) and therefore has custom field options available to other `type: number` fields. For more specific information, see the **How custom fields interpret date and time data types** section on this page.  
...
LI = Works with the [`list` measure type](/looker/docs/reference/param-measure-types#list)

DT = Works with the [`date` measure type](/looker/docs/reference/param-measure-types#date)

YN = Works with the [`yesno` measure type](/looker/docs/reference/param-measure-types#yesno)

N = Works with [numeric measure types](/looker/docs/reference/param-dimension-filter-parameter-types#number) *
...
Custom fields treat the following LookML time data types as timestamp data types:
  ...
  * [`date`](/looker/docs/reference/param-field-dimension-group#date)
  * [`time`](/looker/docs/reference/param-field-dimension-group#time_table)
  * [`microsecond`](/looker/docs/reference/param-field-dimension-group#microsecond)
  * [`millisecond`](/looker/docs/reference/param-field-dimension-group#millisecond)
  * [`millisecondX`](/looker/docs/reference/param-field-dimension-group#millisecondx_table)
  * [`second`](/looker/docs/reference/param-field-dimension-group#second)
  * [`minute`](/looker/docs/reference/param-field-dimension-group#minute)
  * [`minuteX`](/looker/docs/reference/param-field-dimension-group#minutex_table)
  * [`hour`](/looker/docs/reference/param-field-dimension-group#hour)
  * [`hourX`](/looker/docs/reference/param-field-dimension-group#hourx_table)
  * [`week`](/looker/docs/reference/param-field-dimension-group#week)
  * [`month`](/looker/docs/reference/param-field-dimension-group#month)
  * [`quarter`](/looker/docs/reference/param-field-dimension-group#quarter)
  * [`fiscal_quarter`](/looker/docs/reference/param-field-dimension-group#fiscal_quarter)
  * [`fiscal_year`](/looker/docs/reference/param-field-dimension-group#fiscal_year)
  * [`year`](/looker/docs/reference/param-field-dimension-group#year)

#### Integer date data types

Custom fields treat the following LookML date and time data types as integer date types:

  * [`hour_of_day`](/looker/docs/reference/param-field-dimension-group#hour_of_day)
  * [`day_of_month`](/looker/docs/reference/param-field-dimension-group#day_of_month)
  * [`day_of_year`](/looker/docs/reference/param-field-dimension-group#day_of_year)
  * [`month_num`](/looker/docs/reference/param-field-dimension-group#month_num)
  * [`fiscal_month_num`](/looker/docs/reference/param-field-dimension-group#fiscal_month_num)
  * [`week_of_year`](/looker/docs/reference/param-field-dimension-group#week_of_year)
  * [`day_of_week_index`](/looker/docs/reference/param-field-dimension-group#day_of_week_index)

#### String date data types

Custom fields treat the following LookML date and time data types as string data types:

  * [`time_of_day`](/looker/docs/reference/param-field-dimension-group#time_of_day)
  * [`day_of_week`](/looker/docs/reference/param-field-dimension-group#day_of_week)
  * [`month_name`](/looker/docs/reference/param-field-dimension-group#month_name)
  * [`quarter_of_year`](/looker/docs/reference/param-field-dimension-group#quarter_of_year)
  * [`fiscal_quarter_of_year`](/looker/docs/reference/param-field-dimension-group#fiscal_quarter_of_year)
...
Custom fields treat the following measure types as numeric types:
  ...
  * [`average_distinct`](/looker/docs/reference/param-measure-types#average_distinct)
  * [`count`](/looker/docs/reference/param-measure-types#count)
  * [`count_distinct`](/looker/docs/reference/param-measure-types#count_distinct)
  * [`max`](/looker/docs/reference/param-measure-types#max)
  * [`median`](/looker/docs/reference/param-measure-types#median)
  * [`median_distinct`](/looker/docs/reference/param-measure-types#median_distinct)
  * [`min`](/looker/docs/reference/param-measure-types#min)
  * [`percent_of_previous`](/looker/docs/reference/param-measure-types#percent_of_previous)
  * [`percent_of_total`](/looker/docs/reference/param-measure-types#percent_of_total)
  * [`percentile`](/looker/docs/reference/param-measure-types#percentile)
  * [`percentile_distinct`](/looker/docs/reference/param-measure-types#percentile_distinct)
  * [`running_total`](/looker/docs/reference/param-measure-types#running_total)
  * [`sum`](/looker/docs/reference/param-measure-types#sum)
  * [`sum_distinct`](/looker/docs/reference/param-measure-types#sum_distinct)
...
**Warning:** The [`type: number` measure type](/looker/docs/reference/param-measure-types#number) is not compatible with custom fields. 

## Creating a custom measure from a dimension

You can create a custom measure from a dimension in one of the following ways:

  * Using the dimension's [three-dot **More** menu](/looker/docs/creating-and-editing-explores#three-dot_more_menu)
...
In many cases, you can use this shortcut technique:
  ...
  2. Select the dimension's [three-dot **More** menu](/looker/docs/creating-and-editing-explores#three-dot_more_menu).
...
As with other fields, you can select a custom field's name to add or remove it from a query. You can also hover over the field to reveal [more options](/looker/docs/creating-and-editing-explores#field-specific_information_and_actions) that are available for that field, including selecting its **Filter** icon to use it as a [filter](/looker/docs/filtering-and-limiting) in a query. You can also edit the field as necessary.
...
If you want to define a format or a filter for your custom measure while creating it, start with the **Add** button on the **Custom Fields** section:
  ...
  4. If you want to add a filter condition, select a field from the **Filter name** drop-down on the **Filters** tab. You can add or remove filter conditions using the **Add** and **Remove** plus and minus **Filter value** buttons. 
     * You can also select the arrow next to **Custom filter** to create a [custom filter](/looker/docs/filtering-and-limiting#custom_filters) expression using any [Looker functions and operators](/looker/docs/functions-and-operators) that can be used in custom filters. The Looker expression editor will [suggest](/looker/docs/creating-looker-expressions#seeing_all_suggestions) field names and display syntax help for any functions you use. Fields that are both currently used in an Explore and eligible to be used with the field type that you're creating are marked with a black circle. Refer to the Adding a custom filter to a custom measure section on this page for more details about adding filters to measures.
...
As with other fields, you can select a custom field's name to display it in a query. You can also select its **Filter** button to use it as a [filter](/looker/docs/filtering-and-limiting) in a query.

## Custom grouping

The **Group** custom field type lets you create ad hoc custom groups for dimensions without needing to use [logical functions](/looker/docs/functions-and-operators#logical_functions,_operators,_and_constants) in [Looker expressions](/looker/docs/creating-looker-expressions) or needing to develop `CASE WHEN` logic in [`sql`](/looker/docs/reference/param-field-sql) parameters or [`type: case`](/looker/docs/reference/param-field-case) fields.
...
To create a group, follow these steps:
  ...
  2. You can perform the next step in two different ways:
     ...
     * Expand the view, select the dimension's [three-dot **More** menu](/looker/docs/creating-and-editing-explores#three-dot_more_menu), and then select **Group** to open the **Group By** menu.
...
![The Group By menu for the State dimension.](/static/looker/docs/images/custom-fields-group-2-2308.png)
  ...
  5. Select the condition, and enter or select one or more values. In this case, one of the conditions applied is **State is Oregon**. You can add or remove conditions using the **Add** and **Remove** plus and minus buttons. To save, click outside the filter condition, or use the escape key.
     ...
     * When available, value [suggestions](/looker/docs/changing-filter-suggestions) will appear in a drop-down list, which is indicated with a downward-facing arrow in the value input box, for users to select or search against. Suggestions are most commonly available for fields of `type: string`.
...
![A user hovers their cursor over the Add plus sign button next to the existing Group value, and the Group remaining values checkbox is checked.](/static/looker/docs/images/custom-fields-group-3-2304.png)
...
![The text State Groups is entered into the Field name field, and the description States by region is entered in the Description text box.](/static/looker/docs/images/custom-fields-group-4-2216.png)
...
![](/static/looker/docs/images/custom-fields-group-5-2218.png)
...
![](/static/looker/docs/images/custom-fields-group-6-2308.png)

The new field will appear in the **Custom Fields** section of the field picker.

As with other fields, you can select the field's name to add or remove it from a query. You can also hover over the field to reveal [more options](/looker/docs/creating-and-editing-explores#field-specific_information_and_actions) that are available for that field, including selecting its **Filter** icon to use it as a [filter](/looker/docs/filtering-and-limiting) in a query. You can also edit the custom group as desired.

## Custom binning

The **Bin** custom field type lets you create ad hoc custom bins, or tiers, for numeric type dimensions and custom dimensions without needing to use [logical functions](/looker/docs/functions-and-operators#logical_functions,_operators,_and_constants) in [Looker expressions](/looker/docs/creating-looker-expressions) or needing to develop [`type: tier`](/looker/docs/reference/param-dimension-filter-parameter-types#tier) LookML fields.
...
**Bin** custom fields appear in the [`classic`](/looker/docs/reference/param-dimension-filter-parameter-types#style_classic) tier notation style.
...
To create a bin, follow these steps:
  ...
  2. You can perform the next step in two different ways:
     ...
     * Select the dimension's [three-dot **More** menu](/looker/docs/creating-and-editing-explores#three-dot_more_menu), and then select **Bin** to open the **Bin** menu.
...
![](/static/looker/docs/images/custom-binning-1-2304.png)
...
**Warning:** A maximum of 500 total bins can be created for each **Bin** custom field type. 
...
![](/static/looker/docs/images/custom-binning-2-2304.png)
...
![](/static/looker/docs/images/custom-binning-3-2308.png)

The new field will appear in the **Custom Fields** section of the field picker.

As with other fields, you can select the field's name to add or remove it from a query. You can also hover over the field to reveal [more options](/looker/docs/creating-and-editing-explores#field-specific_information_and_actions) that are available for that field — for example, you can select the field's **Filter** icon to use it as a [filter](/looker/docs/filtering-and-limiting) in a query. You can also edit the custom bin as desired.

## Adding a filter to a custom measure

Applying [filters](/looker/docs/filtering-and-limiting) to custom measures allows a measure to limit the data, such as only including orders from certain regions. You can add a custom filter to a custom measure when you're creating or editing a custom measure. To add a filter to a custom measure:

  1. In the field picker, expand the **Custom Fields** section.
  2. Select **Add** and select **Custom Measure** to create a new custom measure, or select an existing custom measure's [three-dot **More** menu](/looker/docs/creating-and-editing-explores#three-dot_more_menu) to edit an existing custom measure.
  ...
  6. Select the condition, and enter or select a value. Select the **Add** plus sign button next to the value to add multiple conditions for the selected field. To save, click outside the filter condition, or use the escape key. 
     * When available, value [suggestions](/looker/docs/changing-filter-suggestions) will appear in a drop-down list, which is indicated with a downward-facing arrow in the value input box, for users to select or search against. Suggestions are most commonly available for dimensions of `type: string`.
  ...
  8. Optionally, click the downward-facing arrow next to **Custom filter** at the bottom of the **Filters** tab to expand the **Expression** text box to add a [custom filter](/looker/docs/filtering-and-limiting#custom_filters) instead of or in addition to a UI-based filter. Enter a [Looker expression](/looker/docs/creating-looker-expressions) in the **Custom filter** box using any [Looker functions and operators](/looker/docs/functions-and-operators) that can be used in custom filters. The Looker expression editor will [suggest](/looker/docs/creating-looker-expressions#seeing_all_suggestions) field names and display syntax help for any functions you use. Fields that are both currently used in an Explore and eligible to be used with the field type that you're creating are marked with a black circle.
...
To create a custom measure that copies an existing measure and adds a [filter](/looker/docs/filtering-and-limiting), follow these steps:

  1. Expand the view that contains the measure to which you want to add a custom filter. The measure _cannot_ be a custom measure or a [`type: number`](/looker/docs/reference/param-measure-types#number) measure. To add a filter expression to an existing custom measure, you will need to edit the custom measure.
  2. Select that measure's [three-dot **More** menu](/looker/docs/creating-and-editing-explores#three-dot_more_menu).
  ...
  8. To add more filters, select the **Add** plus sign button in the **Filter value** section; to remove filters select the **Remove** minus sign button. 
     * Looker automatically applies SQL conditions such as `AND` or `OR` logic when you create multiple filters, based on the field types, conditions, and values that you specify.
  9. Optionally, click the arrow next to **Custom filter** at the bottom of the **Filters** tab to expand the **Expression** text box to add a [custom filter](/looker/docs/filtering-and-limiting#custom_filters) instead of or in addition to a UI-based filter. Enter a [Looker expression](/looker/docs/creating-looker-expressions) in the **Custom filter** box using any [Looker functions and operators](/looker/docs/functions-and-operators) that can be used in custom filters. The Looker expression editor will [suggest](/looker/docs/creating-looker-expressions#seeing_all_suggestions) field names and display syntax help for any functions you use. Fields that are both currently used in an Explore and eligible to be used with the field type that you're creating are marked with a black circle.
...
As with other measures, you can select a custom measure's name to add or remove it from a query. You can also hover over the field to reveal [more options](/looker/docs/creating-and-editing-explores#field-specific_information_and_actions) that are available for that field, including selecting its **Filter** icon to use it as a [filter](/looker/docs/filtering-and-limiting) in a query. You can also edit the field as necessary.

## Creating a custom dimension using a Looker expression

To create a custom dimension using a [Looker expression](/looker/docs/creating-looker-expressions) and one or more other dimensions, follow these steps:

  1. Select **Add** in the **Custom Fields** section of the field picker.
  2. Choose **Custom Dimension**.
  3. In the **Expression** box, enter a [Looker expression](/looker/docs/creating-looker-expressions) that calculates the value for your dimension, using any [Looker functions and operators](/looker/docs/functions-and-operators). The Looker expression editor will [suggest](/looker/docs/creating-looker-expressions#seeing_all_suggestions) field names and display syntax help for any functions you use. Fields that are currently used in an Explore and that are eligible to be used with the field that you're creating are marked with a black circle.
...
As with other fields, you can select a custom dimension's name to add or remove it from a query. You can also hover over the field to reveal [more options](/looker/docs/creating-and-editing-explores#field-specific_information_and_actions) that are available for that field, including selecting its **Filter** icon to use it as a [filter](/looker/docs/filtering-and-limiting) in a query. You can also edit the field as necessary.

## Viewing and using custom fields

The ability to see custom fields, and how you can interact with them, will be different depending on whether you're [allowed to create them](/looker/docs/managing-business-user-features#using_custom_fields) or not.
...
If you're [allowed to create custom fields](/looker/docs/managing-business-user-features#using_custom_fields), you can also duplicate existing custom fields. Duplicating and then editing custom fields can be helpful if you'd like to create multiple custom fields with only small differences (for example, 30-day, 60-day, or 90-day sums).

To duplicate a custom field, follow these steps:

  1. In the field picker, expand the **Custom Fields** section.
  2. Select the [three-dot **More** menu](/looker/docs/creating-and-editing-explores#three-dot_more_menu) for the custom field that you want to duplicate.
  3. Select **Duplicate**.
...
If you're [allowed to create custom fields](/looker/docs/managing-business-user-features#using_custom_fields), you also can edit custom fields that you or other users have created.
...
To edit a custom dimension, follow these steps:
  ...
  1. In the field picker, expand the **Custom Fields** section.
  2. Select the [three-dot **More** menu](/looker/docs/creating-and-editing-explores#three-dot_more_menu) for the custom field that you want to edit.
  3. Select **Edit**.
...
If you're [allowed to create custom fields](/looker/docs/managing-business-user-features#using_custom_fields), you can also delete custom fields you or other users have created. When you delete a custom field, it disappears from the Explore but not from any Looks or dashboard tiles that use that field. Additionally, anyone using a URL for an Explore that had the custom field will still have the field.

To delete a custom field from the field picker, follow these steps:

  1. In the field picker, expand the **Custom Fields** section.
  2. Select the [three-dot **More** menu](/looker/docs/creating-and-editing-explores#three-dot_more_menu) for the custom field that you want to delete.
  3. Select **Delete**.

Alternatively, select **Delete** from the custom field's data table gear menu.

You can also use the [keyboard shortcuts](/looker/docs/keyboard-shortcuts) Command-K (Mac) or Ctrl+K (Windows) to delete custom fields.
