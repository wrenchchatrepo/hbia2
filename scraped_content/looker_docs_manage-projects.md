# https://cloud.google.com/looker/docs/manage-projects
...
To access the **LookML projects** page, perform the following steps:
  ...
  1. Open the **Develop** panel by selecting the **Develop** option in the [navigation panel](/looker/docs/finding-content).
...
This documentation page assumes that you have already created a project using one of the following methods:
  ...
  * as part of model generation through the [**Create a Model** page](/looker/docs/generating-a-model) in Looker
  * as a blank project through the [**Create a Model** page](/looker/docs/generating-a-model#blank-project) in Looker
  * using the [legacy process for creating a project](/looker/docs/create-projects)
...
In Production Mode, the **LookML projects** page lists projects that have been generated and pushed to production. In Development Mode, the page lists projects that you have pulled from production or that you have generated yourself in Development Mode. The **LookML Projects** page includes the following basic categories:
  ...
  * Top section: Projects that are listed in the top section have been created and may include [generated model and view files](/looker/docs/generating-a-model). If you're a Looker admin, or if you have [`develop`](/looker/docs/admin-panel-users-roles#develop) permission for at least one model in a project, you can see the project in the **Develop** panel and in the top section of the page.
  * **Pending Projects** section: Projects that are listed in the **Pending Projects** section are projects where the Looker admin has [configured the allowed connections for a model](/looker/docs/generating-a-model#configuring_planned_models) but the Looker developers have not yet [added the LookML](/looker/docs/create-projects). If you have [`develop`](/looker/docs/admin-panel-users-roles#develop) permission, you can see these projects.
  * **Marketplace Projects** section: Projects that are listed in the **Marketplace Projects** section were created when an application, a block, or a plug-in was installed using the [Looker Marketplace](/looker/docs/marketplace).

**Note:** To see a project, you must have either the [`manage_models`](/looker/docs/admin-panel-users-roles#manage-models) permission, or the [`develop`](/looker/docs/admin-panel-users-roles#develop) permission for at least one model in the project. If you have the [`manage_models`](/looker/docs/admin-panel-users-roles#manage-models) permission, or have the [`manage_project_models`](/looker/docs/admin-panel-users-roles#manage-project-models) permission for at least one model in the project you can add or edit the configuration for any model that is associated with a listed project.

For information on specifying a new project or a new model for an existing project, see the [**Generating a Model**](/looker/docs/generating-a-model) documentation page.
...
Connection 'x' does not exist | The [`connection`](/looker/docs/reference/param-model-connection) declared in the model file does not exist (such as if someone misspelled it or has not created it yet).  
Model 'x' is not allowed to use connection 'y' | The [`connection`](/looker/docs/reference/param-model-connection) declared in the model file is not permitted according to the configuration on this page.  
...
The **Marketplace Projects** section of the **LookML Projects** page lists projects that were created during the [installation of a Marketplace application, block, or plug-in](/looker/docs/marketplace). The listing for each Marketplace project includes the following elements:

  * **Listing** : A link to the listing for the application, block, or plug-in in the [Looker Marketplace](/looker/docs/marketplace).
  * **Project** : The name of the project. You can click the name of the project to view or edit its LookML.
  * **View LookML** or **Edit LookML** button: You can navigate to a Marketplace project by selecting the **View LookML** button for a Marketplace block's read-only core project, or the **Edit LookML** for its config project, the latter of which can be customized. For more information on customizing Marketplace blocks, see the [Customizing Looker Marketplace Blocks](/looker/docs/marketplace-customize-blocks) documentation page.
...
The **Pending Projects** section contains projects and models that are [configured](/looker/docs/generating-a-model#configuring_planned_models) but that don't yet have LookML [generated](/looker/docs/create-projects#automatically_generating_a_model_from_your_database) or [manually defined](/looker/docs/creating-project-files#creating_files).

Configuring models before the project exists is useful if you have developers who have not been given either the [`manage_models`](/looker/docs/admin-panel-users-roles#manage-models) or the [`manage_project_models`](/looker/docs/admin-panel-users-roles#manage_project_models) permission. Because the model has already been configured, those developers can run queries as soon as they create the project and that model. When the project is generated, then the listing for that project moves to the top section of the **LookML Projects** page.
...
The **Pending Projects** section includes the following elements:
  ...
  * **Add LookML** button: The **Add LookML** button opens the [**New Project**](/looker/docs/create-projects) page for that project, where you can [automatically generate LookML](/looker/docs/create-projects#automatically_generating_a_model_from_your_database) that is based on a set of tables in your database.
...
![](/static/looker/docs/images/dev-configure-a-model-2316.png)
...
![](/static/looker/docs/images/develop-edit-model-config-2316.png)

When you edit the configuration for a model, you can view the model name but not rename it. Models are named according to the filenames that you use when you [edit the model files in the Looker IDE](/looker/docs/ide-folders).
...
When a Looker admin [creates a LookML project](/looker/docs/create-projects), the admin provides a project name. The project name should be considered a permanent ID for the project, since Looker uses the project name in the following ways:
  ...
  * To identify the project for [local project import](/looker/docs/importing-projects#importing_local_projects).
  * To display the project on the [**LookML Projects** page](/looker/docs/manage-projects).
  * To display the project in the **Develop** section of the [Looker navigation panel](/looker/docs/finding-content).
...
If you have the Looker [`manage_models`](/looker/docs/admin-panel-users-roles#manage_models) permission, you can change a LookML project's name in the project's [**Project Configuration** page](/looker/docs/git-options#configuration). Looker developers who are not admins can view the **Project Configuration** page but cannot change the options on the page.

**Warning:** If you rename a LookML project, you will need to do the following: 

  * Update the [`local_dependency`](/looker/docs/reference/param-manifest-local-dependency) parameter in the LookML manifest file of any LookML projects that use local project import to import the renamed project.
  * Update any web apps with API calls that use the `project_id` parameter to refer to the project.
  * Update the [`project_name`](/looker/docs/reference/param-manifest-project-name) parameter in the LookML manifest file of the renamed LookML project, if the project has a manifest file.
  * Update any browser bookmarks or saved URLs, if you or any of your LookML developers rely on URLs to access the renamed project's LookML files.
...
To rename a project, perform the following steps:
  ...
  2. [Navigate to your project](/looker/docs/accessing-project-files) in the Looker IDE.
...
If you're a Looker admin, you can delete an existing project with the project's [project settings](/looker/docs/git-options#project_settings). To see the project settings, [open your project](/looker/docs/accessing-project-files) and click **Settings** tune in the IDE navigation bar.

**Important:** Selecting the **Delete Project** button removes all LookML from the project, in all development and production environments across your Looker instance. Be certain that you want to delete a project before selecting this button. Once you delete a project, the deletion operation cannot be reversed, and it's difficult and laborious to recover a deleted project.
...
To remove the project configuration, which removes the project from your development environment completely:
  ...
  1. [Navigate to your project](/looker/docs/accessing-project-files) in the Looker IDE.
