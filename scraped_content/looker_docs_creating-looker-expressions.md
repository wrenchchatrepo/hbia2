...
**Note:** If your admin has granted you [permissions](/looker/docs/admin-panel-users-roles#create_table_calculations) to create [table calculations](/looker/docs/table-calculations), you can use the following features to quickly perform common functions without creating Looker expressions: 
...
Looker expressions (sometimes referred to as _Lexp_) are used to perform calculations for:
  ...
  * [Data tests](/looker/docs/reference/param-model-test)
...
A Looker expression is built from a combination of these elements:
  ...
  * **A Looker operator:** There are several types of operators (which are listed on the [Looker functions and operators](/looker/docs/functions-and-operators) documentation page):

    * Mathematical operators (such as `+`, `-`, `*`, and `/`)
    * Comparison operators (such as `=`, `>`, and `<=`)
    * Logical operators (such as `AND`, `OR`, and `NOT`)
  * **A Looker function:** These are similar in nature to Excel functions. Functions let you transform your data or reference data in complex ways. All available functions are listed on the [Looker functions and operators](/looker/docs/functions-and-operators) documentation page.

## Creating Looker expressions

[Table calculations](/looker/docs/table-calculations), [custom fields](/looker/docs/custom-fields), and [custom filters](/looker/docs/filtering-and-limiting#custom-filters) use the Looker expression editor. As you type your expression, Looker prompts you with functions, operators, and field names that you might want to use.

**Tip:** If you are a Looker developer creating a [data test](/looker/docs/reference/param-model-test) to verify the logic of your model, you can use the Looker expression editor to build a Looker expression, then copy the expression into your data test's `expression` parameter.

### Seeing all suggestions

Access the Looker expression editor in an Explore by creating a [table calculations](/looker/docs/table-calculations), [custom field](/looker/docs/custom-fields), or [custom filter](/looker/docs/filtering-and-limiting#custom-filters).
...
You can reference the full list of functions that Looker offers on the [Looker functions and operators](/looker/docs/functions-and-operators) documentation page.
...
There are several ways to retrieve a value:
  ...
  * **Get a value from a different row:** You can also get a field's value from a different row. For example, you might want the logic "for any given row, grab the Product Category from the _previous_ row." To do that, you can use an offset function (see this [list of positional functions](/looker/docs/functions-and-operators#rows)). The offset function might look like this: `offset(${product.category}, -1)`.

  * **Get a value from a pivoted column:** You can also get values from pivoted columns. For example, you might want the logic "for any given row, grab the Total Sales from the first pivoted column." To work with pivoted columns, you'll need to use pivot functions (see this [list of pivot functions](/looker/docs/functions-and-operators#pivots)). The pivot function might look like this: `pivot_index(${order.total_sales}, 1)`.

  * **Get a total from a row or a column:** If you [added totals to your Explore](/looker/docs/creating-and-editing-explores#displaying_totals), you can get total values from the column or row by adding `:total` (for column totals) or `:row_total` (for row totals) to the field name, using the format `${field_name:total}`. For example, if you want a percentage of the total of an **Orders** count, you could create a table calculation like this: `${orders.count} / ${orders.count:total}`.

**Note:** When referencing fields in a [custom filter](/looker/docs/filtering-and-limiting#custom-filters), you can use only dimensions — not measures, totals, or table calculations. Additionally, you cannot refer to values in other rows or pivoted columns. This is because Looker must be able to turn your filter into SQL in order to retrieve your data. 

Not all time and date options are supported in [custom filters](/looker/docs/filtering-and-limiting#custom-filters) or [custom fields](/looker/docs/custom-fields). See the [dimension group](/looker/docs/looker/docs/reference/param-field-dimension-group#custom_fields_do_not_support_all_timeframes) parameter description for more information. 

When referencing fields in a [table calculation](/looker/docs/table-calculations), you can reference any value from any dimension, measure, or other table calculation. 
...
In Looker, you should use `yes` and `no` instead of `true` and `false`. These logical constants are _not_ the same thing as the words `"yes"` and `"no"`, which are enclosed in quotes. See the [logical constants description](/looker/docs/functions-and-operators#logical_constants) for more detail.
...
For example, the [`now`](/looker/docs/functions-and-operators#function-now) function does not take any arguments, and gives you the current date and time. You use it like this: `now()`.

The [`round`](/looker/docs/functions-and-operators#function-round) function _does_ take one argument, which is a number. You use it like this: `round(3.2)`. The result is `3`.
...
There are two ways to know which arguments you'll need to provide, if any:
  ...
  * You can also navigate directly to the [Looker functions and operators](/looker/docs/functions-and-operators) documentation page and look up the function you want to use.

Consider the [`contains`](/looker/docs/functions-and-operators#function-contains) function, which has documentation that looks like this:
...
The [`is_null`](/looker/docs/functions-and-operators#function-is_null) function is nested inside of an [`if`](/looker/docs/functions-and-operators#function-if) function, which is itself inside a [`contains`](/looker/docs/functions-and-operators#function-contains) function. It works like this:
...
**Note:** [Custom filters](/looker/docs/filtering-and-limiting#custom-filters) and [custom fields](/looker/docs/custom-fields) can use most functions, but they cannot use some mathematical functions, or functions that refer to other rows or pivot columns. The [Looker functions and operators](/looker/docs/functions-and-operators) documentation page lets you know which functions you can use. 

You can use any function in a [table calculation](/looker/docs/table-calculations). You can also use any function in the `expression` parameter of a [data test](/looker/docs/reference/param-model-test), since the `expression` parameter is essentially a table calculation that results in a yesno (Boolean).