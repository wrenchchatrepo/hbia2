# https://cloud.google.com/looker/docs/looker-core-qs-model-data
...
The easiest way to model your data in Looker is to have Looker itself automatically generate LookML files that are based on the tables in your database. You can practice doing that with the [Generate a model from sample data](/looker/docs/looker-core-qs-model-generation) quickstart, and you can see the full procedure on the [Generating a LookML model](/looker/docs/generating-a-model) documentation page.
...
As a starting point, this quickstart uses the [sample LookML project](/looker/docs/looker-core-sample-project) that is automatically configured on Looker (Google Cloud core) instances.
...
To follow along with this quickstart, you'll need the following:
  ...
  * Access to a Looker (Google Cloud core) instance: 
    * The instance must include the [sample LookML project](/looker/docs/looker-core-sample-project).
    * The instance's Google Cloud project must have the BigQuery API enabled. The [**Looker Admin** IAM role](/looker/docs/looker-core-access-control#iam-roles) is required to enable the BigQuery API. 

[ Enable the API](https://console.cloud.google.com/flows/enableapi?apiid=bigquery.googleapis.com)

  * A Looker user account with either the [**Looker Admin**](/looker/docs/admin-panel-users-roles#default_roles) role or the [`develop` permission](/looker/docs/admin-panel-users-roles#develop).

## Turn on Development Mode

Looker projects can exist in one of two modes: Production Mode and Development Mode. Development Mode lets you make changes to LookML files and preview how they will affect content on your instance without affecting the production environment (what other users see). See the [Development Mode and Production Mode](/looker/docs/dev-mode-prod-mode) documentation page for more information.
...
A [LookML project](/looker/docs/lookml-terms-and-concepts#lookml_project) is a collection of LookML files that tell Looker how to connect to your database, how to query your data, and how to present the data in the user interface. In this quickstart, you will use the [sample LookML project](/looker/docs/looker-core-sample-project) that is automatically configured on Looker (Google Cloud core) instances.
...
**Tip:** [LookML is case-sensitive](/looker/docs/lookml-terms-and-concepts#case_sensitivity). Generally, it's a good idea to consistently use lowercase letters when you name LookML objects and folders. See the [important notes for creating files and folders](/looker/docs/creating-project-files#file_and_folder_naming_conventions) for limitations and guidelines for naming your files and folders.
  3. Click **Create**.
...
To learn more about using folders in your LookML project, see the following documentation pages:
  ...
  * To create your own folders and files in the future, and for considerations for creating folders in your project, see the [Managing LookML files and folders](/looker/docs/creating-project-files) documentation page.
  * To learn more about the Looker IDE file browser, see the [Working with the IDE file browser](/looker/docs/ide-folders) documentation page.

## Create a new LookML model file

A LookML [model file](/looker/docs/lookml-project-files#model_files) specifies a database connection and the set of Looker Explores that use that connection. An Explore is a starting place for your users to query your data in Looker once your data is modeled in LookML (see [Viewing and interacting with Explores in Looker](/looker/docs/viewing-and-interacting-with-explores)).
...
To create your own model files in the future, note the following resources:
  ...
  * For information about model files in general, see the [Model files](/looker/docs/lookml-project-files#model_files) section of the [LookML project files](/looker/docs/lookml-project-files) documentation page.
  * For the full procedure and other options for creating files in your project, see the [Managing LookML files and folders](/looker/docs/creating-project-files) documentation page.
  * For information about the naming conventions for LookML files, see the [Before you begin: Important file and folder naming conventions](/looker/docs/creating-project-files#before_you_begin_important_file_and_folder_naming_conventions) section on the Managing LookML files and folders page.
  * For information about the LookML parameters that you can use in a model file, see the [Model parameters](/looker/docs/reference/param-model) documentation page.

## Create a new LookML view file

In LookML, a view corresponds to either a single table in your database or a single [derived table](/looker/docs/derived-tables). The [view file](/looker/docs/lookml-project-files#view_files) specifies the table in your database to query and the fields (dimensions and measures) to include from that database table. Once you define a field in the view file, you can use the [`$` substitution operator](/looker/docs/sql-and-referring-to-lookml#substitution_operator) to reference the field in other parts of your LookML. This lets you define the field in a single location while using it in multiple places in your LookML project.

There are several ways to [create a new view file](/looker/docs/creating-project-files#considerations_for_creating_view_files), but the easiest way to get started is to have Looker itself automatically generate a view file based on an existing table in your database.
...
![The Create Views from Tables dialog.](/static/looker/docs/images/quickstart-model-custom-db-2420.png)
...
To create your own view files in the future, note the following resources:
  ...
  * For information about view files in general, see the [View files](/looker/docs/lookml-project-files#view_files) section of the [LookML project files](/looker/docs/lookml-project-files) documentation page.
  * For the full procedure and other options for creating files in your project, see the [Managing LookML files and folders](/looker/docs/creating-project-files) documentation page.
  * For considerations specific to creating view files, see the [Considerations for creating view files](/looker/docs/creating-project-files#considerations_for_creating_view_files) section on the Managing LookML files and folders page.
  * For information about the naming conventions for LookML files, see the [Before you begin: Important file and folder naming conventions](/looker/docs/creating-project-files#before_you_begin_important_file_and_folder_naming_conventions) section on the Managing LookML files and folders page.
  * For information about the LookML parameters that you can use in a view file, see the [View parameters](/looker/docs/reference/param-view) documentation page.
...
![](/static/looker/docs/images/develop-ide-quick-help-2206.png)
...
Explores are typically defined in a model file. (You can create a separate [Explore file](/looker/docs/lookml-project-files#explore_files), but for the purposes of this quickstart, you will create the Explore in the model file.)

In the LookML file where you define an Explore you must use the [`include`](/looker/docs/reference/param-model-include) parameter so that you can reference the view file in the Explore definition. This procedure walks you through these steps.
...
**Tip:** You can verify that you have successfully included the `order_items` view file by looking in the **Quick Help** panel for an alert box that says, "Files included: quickstart/order_items.view.lkml". If you have tried to include a file that does not exist (or made a typographical error in the filename), the **Quick Help** panel will display an alert box that says, "This include does not match any files".
...
To create your own Explores in the future and to build out your Explores, note the following resources:
  ...
  * For more information about creating Explores, see the [Creating and editing Explores](/looker/docs/creating-and-editing-explores) documentation page.
  * For information about joining in other views, see the [Working with joins in LookML](/looker/docs/working-with-joins) documentation page.
  * For information about the LookML parameters that you can use for an Explore, see the [Explore parameters](/looker/docs/reference/param-explore) documentation page.
  * For information about querying data in an Explore page in the Looker UI, see the [Viewing and interacting with Explores in Looker](/looker/docs/viewing-and-interacting-with-explores#the_explore_page) documentation page.
...
After you have created the view files in the Create a new LookML view file section of this quickstart, you can see the fields that Looker created automatically. When you create a view file that is based on a database table, Looker automatically creates dimensions, dimension groups, and measures:
  ...
  * A [dimension group](/looker/docs/reference/param-field-dimension-group) is used to create a set of time-based or duration-based dimensions all at once. For example, the dimension group for a timestamp column will contain individual dimensions for date, month, quarter, time, week, and year. Looker automatically creates a dimension group for the time-based fields that Looker detects in your database table.
  * A [measure](/looker/docs/reference/param-field-measure) is a field in an Explore that represents measurable information about your data, such as sums, counts, etc. A measure is declared in a view file and can be of an aggregate or non-aggregate type. Looker automatically creates a measure of [`type:count`](/looker/docs/reference/param-measure-types#count) for your view file, which is the equivalent of a `COUNT(*)` in SQL.
...
**Tip:** As you type in the IDE, see the **Quick Help** panel for information about the parameters and possible subparameters that are supported at your current cursor location.
...
To create your own fields in the future, note the following resources:
  ...
  * For information about LookML fields in general, see the [LookML field reference](/looker/docs/reference/param-field) documentation page.
  * For information about the types of dimensions, filters, and parameters that you can create in a view file, see the [Dimension, filter, and parameter types](/looker/docs/reference/param-dimension-filter-parameter-types) documentation page.
  * For information about the types of measures that you can create in a view file, see the [Measure types](/looker/docs/reference/param-measure-types) documentation page.
...
![The See file actions menu, with the Explore Order Items - quickstart_practice option selected.](/static/looker/docs/images/quickstart-model-file-actions-2420.png)

**Tip:** You can also navigate to the Explore from the Looker **Main menu** menu. From the main menu, if you are in the **Develop** menu, click the **Close Develop** arrow, then click **Explore**. In the **Explore** menu, click **Quickstart Practice** to expand it (if it isn't already expanded), and then click the **Order Items** Explore.
...
As you edit your LookML, the Looker IDE will alert you to any unresolved syntax errors within a single file (see the [Looker IDE overview](/looker/docs/looker-ide#ad_hoc_error_checking) documentation page). You can also perform a full model validation, which will check your LookML for any errors. See the [Validating your LookML](/looker/docs/lookml-validation) documentation page for more information.

## Commit your changes

In this quickstart, you are using a pre-configured Git connection that is set up for the sample LookML project. However, in the future, when you create your own projects to model your data, you will need to [set up a Git connection](/looker/docs/setting-up-git-connection) for your project.
...
## What's next
  ...
  * [Types of files in a LookML project](/looker/docs/lookml-project-files)
