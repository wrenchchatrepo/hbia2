...
Each view file in your LookML project defines a single _view_ within Looker, which specifies a table to query and which fields — dimensions and measures — from that table will be surfaced in the Looker UI. A [view](/looker/docs/reference/param-view-view) corresponds to either a single table in your database or a single [derived table](/looker/docs/derived-tables).
...
To learn more about using SQL to define and customize derived tables in LookML using SQL, see [SQL concepts for derived tables](/looker/docs/sql-experts-derived-tables).
...
The first element of the view definition is the [`sql_table_name`](/looker/docs/reference/param-view-sql-table-name) parameter, which specifies the table in your database that will be queried by a view. This value is the only place in the entire model where the table name is defined, because all other references to the view will use the table alias `${TABLE}`. If you want to change the database table name, it only needs to be changed in the `sql_table_name` parameter. There are some [things to consider](/looker/docs/reference/param-view-sql-table-name#things_to_consider) when you reference a database table.
...
When you create an Explore query with the age cohort dimension, you can use the SQL tab of the Explore to [view the SQL that Looker generates](/looker/docs/how-looker-generates-sql). With the age cohort dimension, the SQL will look something like this:
...
You can achieve the same effect as a SQL CASE statement with an expression that is database-independent. The LookML [`case`](/looker/docs/reference/param-field-case) parameter lets you define the cohort buckets that are made up of `when` statements that use `sql` expressions to capture specific conditions and strings for labeling the results.
...
## Related resources
  ...
  * [SQL concepts for joins](/looker/docs/sql-experts-joins)
  * [SQL concepts for derived tables](/looker/docs/sql-experts-derived-tables)
  * [Using Looker's SQL generator](https://www.googlecloudcommunity.com/gc/Technical-Tips-Tricks/Using-Looker-s-SQL-Generator/ta-p/814623)
