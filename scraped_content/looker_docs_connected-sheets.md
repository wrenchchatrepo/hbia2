# https://cloud.google.com/looker/docs/connected-sheets

Depth: 3

**Note:** This page is part of the [Retrieve and chart data](/looker/docs/retrieve-and-chart-data) learning series.  
  
Connected Sheets for Looker lets users interactively explore data from LookML models through the familiar Google Sheets spreadsheet interface. Sheets users will have access to governed data from [50+ databases supported by Looker](/looker/docs/dialects). Exploration is intuitive since Connected Sheets presents an organized view of the model with views, fields, and measures just as they were defined in Looker. Looker data through Connected Sheets can be used in standard Sheets constructs, like pivot tables, charts, and formulas, allowing for flexibility of analysis through familiar constructs.

**Note:** Connected Sheets for Looker is available only for Looker-hosted instances.

## Enabling Connected Sheets

To use Connected Sheets:

  * In Looker (original), your Looker admin must enable either the **All Looker BI Connectors** or the **Connected Sheets** toggle on the [BI Connectors panel](/looker/docs/bi-connectors) in the **Platform** section of Looker's **Admin** menu. In Looker (Google Cloud core), these options are enabled by default.
  ...
  * Access to services that are not controlled individually must be turned on in your Google Workspace admin console. This is typically enabled by default. For more information, see the [Manage access to services that aren't controlled individually](https://support.google.com/a/answer/7646040) help article.

## Connecting to a Look from Google Sheets

In Looker, a [Look](/looker/docs/saving-and-editing-looks) is a single query that was previously saved. You can add the results of a Look to a sheet and then interact with the Look output in Sheets.
...
**Note:** The first time you connect to a Looker instance from Google Sheets, you will be required to link your Google Account and yourLooker account. You can unlink your accounts at any time. For more information about Google Account linking, see the [Manage third-party accounts you've linked with Google](https://support.google.com/accounts/answer/13864929) help article.
...
![](/static/looker/docs/images/connected_sheets_look_2500.png)
...
An [Explore](/looker/docs/creating-and-editing-explores#explores_are_the_starting_point_for_exploration) is the starting point for queries. An Explore shows a specified set of fields from its associated view file, and these fields can be selected to construct a query in a sheet.
...
**Note:** The first time you connect to a Looker instance from Google Sheets, you will be required to link your Google and Looker accounts. You can unlink your accounts at any time. For more information about Google Account linking, see the [Manage third-party accounts you've linked with Google](https://support.google.com/accounts/answer/2541991) help article.
  3. In the following dialogs, select the Looker [model](/looker/docs/model-and-view-files#model_files) and [Explore](/looker/docs/creating-and-editing-explores#explores_are_the_starting_point_for_exploration) that you want to use for your query, and select **Connect**.

When Sheets connects to your Looker instance, Sheets will open a new sheet that shows the structure of the selected Explore, including views, dimensions, measures, and any field [descriptions](/looker/docs/reference/param-field-description) that was added by your Looker developer.

![](/static/looker/docs/images/connected_sheets_explore_2220.png)
...
A new, empty pivot table is created, and the **Pivot table editor** window opens. You can add rows, columns, values, and [filters](/looker/docs/filtering-and-limiting) to build your query based on the dimensions and measures in your Looker [Explore](/looker/docs/creating-and-editing-explores#explores_are_the_starting_point_for_exploration).

![](/static/looker/docs/images/connected-sheets-pivot-table-editor-2220.png)
...
The rows, columns, values, and filters in the pivot editor are equivalent to your Looker dimensions and measures in the following ways:
     ...
     * Rows are equivalent to Looker [dimensions](/looker/docs/creating-and-editing-explores#explores_are_the_starting_point_for_exploration).
     * Columns are equivalent to Looker [pivoted dimensions](/looker/docs/creating-and-editing-explores#pivoting_dimensions).
     * Values are equivalent to Looker [measures](/looker/docs/creating-and-editing-explores#explores_are_the_starting_point_for_exploration). You can create a custom measure by selecting a Looker dimension and a Sheets **Summarize by** function.
     * Filters are equivalent to Looker [filters](/looker/docs/filtering-and-limiting).
...
![](/static/looker/docs/images/connected-sheets-table-2220.png)
...
**Tip:** For relative date filtering, see the Filters using Looker filter expressions section later in this document.
...
![](/static/looker/docs/images/connected-sheets-filter-by-value-2314.png)

A sidebar will open, and Sheets will fetch the 500 most frequently occurring values from Looker.

![](/static/looker/docs/images/connected-sheets-filter-by-value-list-2314.png)
...
You can apply relative date filters, as well as many other advanced filters, in Connected Sheets by using [Looker filter expressions](/looker/docs/filter-expressions). For example, you can apply filters like "is in the past 30 minutes" or "is in the past 10 days" in Connected Sheets.
...
![](/static/looker/docs/images/connected-sheets-status-menu-2318.png)

  3. Enter any Looker filter expression into the **Value** field.

![](/static/looker/docs/images/connected-sheets-filter-expression-2318.png)
...
To view all available filter expressions, see the [Looker filter expressions](/looker/docs/filter-expressions) documentation page.

#### Using Looker parameters and filter-only fields

You can now use [parameters](/looker/docs/reference/param-field-parameter) and [filter-only fields](/looker/docs/reference/param-field-filter) in Connected Sheets.
...
**Note:** In order to improve performance, Looker [stores query results in a cache](/looker/docs/caching-and-datagroups#how_looker_uses_cached_queries) as determined by the [caching policy that is defined in the model](/looker/docs/caching-and-datagroups). When you refresh your data from Google Sheets, the data returned may be from the Looker cache. If the Looker cache has not expired since the most recent Google Sheets refresh, the Google Sheets refresh may return the same data.

Both your pivot table and the sheet showing your Look or Explore display the time they were most recently refreshed.

![](/static/looker/docs/images/connected-sheets-refresh-2220.png)
...
A Looker admin can view Connected Sheets usage using the **Query API Client Properties** group of fields in the [System Activity History Explore](/looker/docs/usage-reports-with-system-activity-explores#history). An entry is created in the **History** Explore every time a new query is run.
...
API support for Connected Sheets for Looker is accessible through both [AppsScript](https://developers.google.com/apps-script/guides/sheets/connected-sheets) and [Google Sheets APIs](https://developers.google.com/sheets/api/guides/connected-sheets#looker-data-source).
...
To specify Looker as a data source with the [Google Sheets API](https://developers.google.com/sheets/api/reference/rest), use the `looker` object with the [`DataSourceSpec`](https://developers.google.com/sheets/api/reference/rest/v4/spreadsheets#datasourcespec) REST resource.

## Things to know

  * Case-sensitivity for the data shown in the pivot table is determined by the [case-sensitivity setting for the model](/looker/docs/reference/param-model-case-sensitive). Filters on the pivot table, however, force case-insensitivity in situations where the filter is case-insensitive for native Sheets data.
  * Anyone who has **Viewer** access to the sheet will be able to view the cached data in a pivot table.
  * In addition to **Editor** access to the sheet, any users who need to edit or refresh the pivot table will need the correct [permissions](/looker/docs/access-control-and-permission-management) in Looker to access the data model. This includes [access to the model](/looker/docs/admin-panel-users-roles#model_sets) on which the pivot table is based, [permissions to view the data](/looker/docs/admin-panel-users-roles#permission_sets), and proper access to the data through any [access filters](/looker/docs/reference/param-explore-access-filter) or [access grants](/looker/docs/reference/param-model-access-grant).
...
Queries that are created with Connected Sheets have a maximum limit of 100,000 rows. If the result of the query contains more rows, Sheets will show an error to the user. If the user does not have the [`download_without_limit` permission](/looker/docs/admin-panel-users-roles#download_without_limit), the result will be truncated to a maximum of 5,000 rows regardless of the Sheets limit.
...
This error can occur for multiple reasons:
  ...
  * Sheets cannot connect to the [Looker API](/looker/docs/api-getting-started). This can occur if your Looker instance is not using the default API Host URL. To correct this, leave the [**API Host URL**](/looker/docs/admin-panel-platform-api#api_host_url) field in the [API admin panel](/looker/docs/admin-panel-platform-api) blank.
...
A Looker user with the [Viewer role](/looker/docs/admin-panel-users-roles#default_permission_sets) does not have sufficient permissions to create connections in Sheets, just as they don't have sufficient permissions to use the Explore UI in Looker. If the user wants to use Connected Sheets for Looker, they need to have the [`explore` permission](/looker/docs/admin-panel-users-roles#explore). The [Admin, Developer, and User roles](/looker/docs/admin-panel-users-roles#default_permission_sets) include the `explore` permission. A Looker admin can grant the user the `explore` permission, and that should enable the user to see models on the instance.
...
This error indicates that the Connected Sheets connector has not been enabled. A Looker admin must enable either the **All Looker BI Connectors** or the **Connected Sheets** toggle on the [BI Connectors panel](/looker/docs/bi-connectors) in the **Platform** section of Looker's **Admin** menu.

### Error: 403 Forbidden

If your Looker instance has an [IP allowlist](/looker/docs/admin-panel-server-ip-allowlist) configured, the allowlist can prevent Sheets from connecting to the [Looker API](/looker/docs/api-getting-started). To determine if the allowlist is the cause, try navigating to the Looker instance URL to see if you receive an [IP Address Not on Allowlist](/looker/docs/ip-address-not-on-allowlist) error.