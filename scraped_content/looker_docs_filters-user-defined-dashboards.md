# https://cloud.google.com/looker/docs/filters-user-defined-dashboards

Depth: 3

This page is about standard dashboard filters. To learn about dashboard cross-filters, visit the [Cross-filtering dashboards](/looker/docs/cross-filtering-dashboards) documentation page.
...
When viewing a dashboard, a user can [temporarily update dashboard filter values](/looker/docs/viewing-dashboards#temporarily_changing_filter_values_on_dashboards), which doesn't affect any other users. This differs from filters that are part of a query underlying a dashboard tile; viewers can't change these filters unless they have permission to [edit the tile](/looker/docs/editing-user-defined-dashboards#editing_tiles) or to [**Explore from Here**](/looker/docs/viewing-dashboards#dashboard_\(beta\)_tiles).

To make permanent changes to filters that all users will see, or to add or delete filters, you must have the [**Manage Access, Edit**](/looker/docs/organizing-spaces#folder_access_levels) access level for the dashboard and be in [edit mode](/looker/docs/editing-user-defined-dashboards#entering_edit_mode_in_dashboards_\(beta\)).

**Note:** Dashboard filters override filters applied to a tile's underlying Look or Explore when both types of filters are filtering on the same field. See the [Overriding Look filters in the Explore section with dashboard global filters](https://community.looker.com/dashboards-looks-7/overriding-look-filters-in-the-explore-section-with-dashboard-global-filters-1119) Looker Community post for more details.

### Dashboard filters and LookML

If a model, an Explore, or a field uses the [`label` (model)](/looker/docs/changing-explore-menu-and-field-picker#label_\(model\)), [`label` (Explore)](/looker/docs/changing-explore-menu-and-field-picker#label_\(explore\)), or [`label` (field)](/looker/docs/changing-explore-menu-and-field-picker#label_\(field\)) parameter, respectively, to change its name in the **Explore** menu or field picker, the name given in the `label` parameter is used in the **Add Filter** window.

If you are a LookML developer and are in [**Development Mode**](/looker/docs/dev-mode-prod-mode), you can create a dashboard filter on a field that appears only in your Development Mode version of a model, but the filter won't function as expected when you exit Development Mode or when the filter is used by other users. Similarly, if a filter field is deleted or renamed after the filter is created, the filter won't function as expected.

## Adding dashboard filters

To get started, make sure the dashboard is in [edit mode](/looker/docs/editing-user-defined-dashboards#entering_edit_mode_in_dashboards_\(beta\)) and that you have at least one query tile or Look-linked tile. Select **Filters** in the top toolbar, and then select **Add Filter**.

An **Add Filter** window appears next.

### The Add Filter window

The **Add Filter** window appears pre-populated with [fields](/looker/docs/lookml-terms-and-concepts#dimension_and_measure_fields) (dimensions or measures) from any [Explores](/looker/docs/creating-and-editing-explores) that were used in the creation of this dashboard's tiles.
...
![The Explores pre-populated in the Add Filter window appear after the Filter by search bar.](/static/looker/docs/images/add-filter-explores-models-2206.png)
...
**Tip:** If you have already selected a field in the **Filter by** dialog of the **Add Filter** window, selecting the **Advanced** switch results in no changes to the **Add Filter** window and an advanced filter is not initiated.

Once you've selected the field you want to filter by, select **Add**.

### Advanced filters

Advanced filters let you create dashboard filters using fields that are outside of the [Explores](/looker/docs/creating-and-editing-explores) used to create tiles on the dashboard. In the **Add Filter** window, enable the **Advanced** switch to use advanced filters. This updates the **Add Filter** window with options to select a [model](/looker/docs/lookml-terms-and-concepts#model), an [Explore](/looker/docs/lookml-terms-and-concepts#explore), and a [field](/looker/docs/lookml-terms-and-concepts#dimension_and_measure_fields) with which to create the filter:

![](/static/looker/docs/images/add-advanced-filter-selection-2206.png)

First, select a model name from the drop-down **Model** menu. The drop-down menu contains all LookML models that you [have access to](/looker/docs/admin-panel-users-roles#model_sets).
...
Dashboard filter controls let you customize the appearance of filters for dashboard viewers. The filter control types available in the **Control** drop-down as you create a dashboard filter depend on the LookML data [`type`](/looker/docs/reference/param-dimension-filter-parameter-types) assigned to the field you're filtering on.

> To see the data type for a field you are filtering on, you must have the [`see_lookml`](/looker/docs/admin-panel-users-roles#see_lookml) permission. You can see the field's LookML by selecting [**Go to LookML**](/looker/docs/creating-and-editing-explores#features_for_developers) from the gear menu to the right of the field in the field picker.
...
DT = Works with most [date and time](/looker/docs/reference/param-dimension-filter-parameter-types#time) data types*****
...
---|---|---|---  
Multiple Selection  
Button Group|  This type of filter is useful for giving viewers a curated set of limited options. Up to 30 options can be curated in the **Values** setting. For `string`, `tier`, and `zipcode` data types, if the **Values** setting is left blank, the first 30 values from the database are shown.A button is colored once it's selected.| ![Button group controls appear as a horizontal series of separate buttons representing filter values.](/static/looker/docs/images/button-group-example-2316.png) |  S N T ZC YN DI DU  
Checkboxes|  Like a button group but with a slightly different design, this type of filter is useful for giving viewers a curated set of limited options. Up to 50 options can be curated in the **Values** setting. For `string`, `tier`, and `zipcode` data types, if the **Values** setting is left blank, the first 50 values from the database are shown.|  ![Checkbox controls appear as a horizonal series of checkboxes representing filter values](/static/looker/docs/images/checkboxes-example-2100.png) |  S N T ZC YN DI DU  
Tag List| A combination of drop-down and checkbox filters. The drop-down options can be curated in the **Values** setting. For `string`, `tier`, and `zipcode` data types, all values can be surfaced from the database if the **Values** setting is left blank.Viewers can expand the drop-down by selecting the chevron.Taglist controls can appear only in the popover orientation or behind the **More** button. |  ![A taglist appears as a drop-down menu with checkboxes next to the menu items.](/static/looker/docs/images/taglist-example-2100.png) |  S N T ZC DI DU  
Range Slider| Range slider minimum and maximum can be set in the **Settings** tab of the filter configuration window.Viewers can set the filter value range by adjusting both ends of the slider.|  ![A range slider appears as a horizontal numeric scale with moveable ends on each side to customize a range of values.](/static/looker/docs/images/range-slider-example-2100.png)|  N DI DU  
Single Selection  
Button Toggles| This type of filter is useful for giving viewers a curated set of limited options. Up to 30 options can be curated in the **Values** setting. For `string`, `tier`, and `zipcode` data types, if the **Values** setting is left blank, the first 30 values from the database are shown.A button is colored once it's selected.|  ![Button toggles appear as a horizontal series of connected buttons representing filter values.](/static/looker/docs/images/button-toggle-example-2316.png) |  S N T ZC YN DI DU P  
Radio Buttons| Like a button toggle but with a slightly different design, this type of filter is useful for giving viewers a curated set of limited options. Up to 50 options can be curated in the **Values** setting. For `string`, `tier`, and `zipcode` data types, if the **Values** setting is left blank, the first 50 values from the database are shown.|  ![Radio buttons appear as a horizontal list of values with a circular radio button to the left of each value.](/static/looker/docs/images/radio-example-2406.png) |  S N T ZC YN DI DU P  
Drop-down Menu| The drop-down options can be curated in the **Values** setting. For `string`, `tier`, and `zipcode` data types, all values can be surfaced from the database if the **Values** setting is left blank.Viewers can expand the drop-down by selecting the chevron and then either selecting an option from the drop-down or typing in the box to narrow the drop-down options. Viewers can also select **Any value**.Drop-down menu controls can appear only in the inline orientation or behind the **More** button. |  ![A drop-down menu appears as a single value and a chevron. When selected, the chevron reveals a list of possible filter values.](/static/looker/docs/images/dropdown-example-2100.png) |  S N T ZC YN DI DU P  
Slider| Slider minimum and maximum can be set in the **Settings** tab of the filter configuration window.Viewers can set the filter value by adjusting the right end of the slider.|  ![A slider appears as a horizontal numeric scale with a moveable end on the right side to customize the maximum value.](/static/looker/docs/images/slider-example-2320.png)| N DI DU  
Dates and Times  
Single Day|  Viewers can select the date to expand the calendar and select a new day.Single day menu controls can appear only in the inline orientation or behind the **More** button.| ![A single day control initially appears as a single date, which can be selected to reveal a monthly calendar.](/static/looker/docs/images/single-day-example-2320.png) |  DT***** The single day control can be used with most [timeframes](/looker/docs/reference/param-field-dimension-group#timeframe_options)and [time-based types](/looker/docs/reference/param-dimension-filter-parameter-types#available_time-based_types), but viewers can only select single dates with this typeof control.  
Date Range| Viewers can select the date range to expand the calendar and select a new date range.Date range menu controls can appear only in the inline orientation or behind the **More** button.| ![Date range controls initially appear as a date range, but the date range can be selected to reveal a 2-month calendar. Dates on the calendar can be selected to update the filter value.](/static/looker/docs/images/date-range-example-2100.png)|  DT***** The date range control can be used with most [timeframes](/looker/docs/reference/param-field-dimension-group#timeframe_options) and [time-based types](/looker/docs/reference/param-dimension-filter-parameter-types#available_time-based_types), but viewers can only select date ranges with this type of control.  
Timeframes| Viewers can either select from a set of timeframes built into Looker or create their own custom timeframes by selecting the **Custom** tab. See the [Viewing dashboards](/looker/docs/viewing-dashboards#temporarily_changing_filter_values_on_dashboards) documentation page to see the full range of timeframes options for a Timeframes filter.Timeframes options such as **Last 7 Days** include the current day. For a timeframe that excludes the current day, select **Advanced** from the control drop-down and configure the value using **complete days**.**Year to Date** includes the beginning of the year through to the current second.Viewers can select the timeframe to see the timeframes options.Timeframes menu controls can appear only in the inline orientation or behind the **More** button.| ![Timeframe controls initially appear as a timeframe. The timeframe can be selected to reveal a menu of preset timeframes as well as a custom tab.](/static/looker/docs/images/timeframes-example-2410.png)|  DT***** The timeframe control can be used with most [timeframes](/looker/docs/reference/param-field-dimension-group#timeframe_options) and [time-based types](/looker/docs/reference/param-dimension-filter-parameter-types#available_time-based_types), but viewers cannot select time options with this type of control.  
Other  
Advanced| See the Using advanced controls section on this page to learn more about advanced controls.Advanced controls can appear only in the popover orientation or behind the **More** button. |  ![Advanced controls initially appear as a filter condition; that condition can be selected to reveal a series of drop-down menus and text fields to customize the condition.](/static/looker/docs/images/advanced-example-2100.png)|  S N T ZC YN DT DI DU L P  
***** **Note about date and time data:** Some [timeframes](/looker/docs/reference/param-field-dimension-group#timeframe_options) and [time-based types](/looker/docs/reference/param-dimension-filter-parameter-types#available_time-based_types) are interpreted as different data types when Looker is selecting control options for them. Two examples are the [`yesno` timeframe](/looker/docs/reference/param-field-dimension-group#yesno), which is interpreted as a [`yesno` data type](/looker/docs/reference/param-dimension-filter-parameter-types#yesno) and therefore has the control options available to the `yesno` type; and the [`hour_of_day` timeframe](/looker/docs/reference/param-field-dimension-group#hour_of_day), which is interpreted as a [`number` data type](/looker/docs/reference/param-dimension-filter-parameter-types#number) and therefore has the control options available to the `number` type.  
  
### Using advanced controls

**Note:** Advanced controls are not the same as advanced filters.
...
![To use advanced controls, select Advanced in the Control drop-down. This is not the same as the Advanced toggle to the right of the Filter by field.](/static/looker/docs/images/advanced-filter-string-options-2414.png)

Once you select **Advanced** , the **Configure Default Value** field populates with appropriate filter condition options for the type of data you're filtering. For example, if you created a filter for a field with a [`string`](/looker/docs/reference/param-dimension-filter-parameter-types#string) data type, the filter condition options such as **is** , **contains** , **starts with**. Filters on other types of data, such as `number` or `date` types, will display different condition options.

The **Allow multiple filter values** option lets users select more than one value for the filter. It defaults to enabled. When enabled, a `+` symbol appears to the right of the **Configure Default Value** field. Select the `+` symbol to add an additional filter value. To restrict users so that they can select only one value, disable this option. The **Allow multiple filter values** option is not available for filters based on the [`location`](/looker/docs/reference/param-dimension-filter-parameter-types#location) data type or based on the [`parameter`](/looker/docs/reference/param-field-parameter) parameter.
...
Advanced controls also give you the option to select **matches a user attribute**. This lets you set filters that change dynamically based on dashboard viewers' [user attributes](/looker/docs/admin-panel-users-user-attributes). See the [Filtering and limiting data](/looker/docs/filtering-and-limiting#adapt_filters_to_each_user_with_user_attributes) documentation page for more information.

![User attributes appear in a drop-down to the right of the matches a user attribute condition.](/static/looker/docs/images/matches-user-attribute-filter-2100.png)

#### Matches advanced

The final option in the **Configure Default Value** option drop-down is **matches advanced**. With this option selected, you can enter a [Looker filter expression](/looker/docs/filter-expressions) to customize a filter beyond the options provided in the options drop-down.
...
To set the way a filter appears, first make sure your dashboard is in [edit mode](/looker/docs/editing-user-defined-dashboards#entering_edit_mode_in_dashboards_\(beta\)) and then select the style of display in the **Display** field of the filter configuration window that appears when you select **Edit** from the filter menu. Most dashboard filters can be displayed in the following ways:

  * **Inline** : The filter is displayed directly in the top bar of the dashboard.

![](/static/looker/docs/images/dash-next-inline-filter-2116.png)

  * **Popover** : A summary value appears in the top bar of the dashboard; select the value to see the full filter.

![](/static/looker/docs/images/dash-next-popover-filter-2402.png)
...
![](/static/looker/docs/images/dash-next-overflow-filter-2408.png)
...
To set the way a filter bar appears, follow these steps:
  ...
  1. Make sure your dashboard is in [edit mode](/looker/docs/editing-user-defined-dashboards#entering_edit_mode_in_dashboards_\(beta\)).
  2. Open the dashboard's [**Settings**](/looker/docs/editing-user-defined-dashboards#editing_dashboard_settings) window.
  3. Select the **Filters** tab of the **Settings** window.
  4. You can set the [**Default filters view**](/looker/docs/editing-user-defined-dashboards#default_filters_view) option to **Expanded** or **Collapsed**. (The default is **Expanded**.)
  5. The [**Filters location**](/looker/docs/editing-user-defined-dashboards#filters_location) option allows you to place the filter bar at the top or the right of the dashboard. (The default is at the top.)

## Requiring a filter value

**Important:** To limit dashboard viewers' data access, use the [`access_filter`](/looker/docs/reference/param-explore-access-filter) LookML parameter instead of required filters.
...
To require that a user enter a value in a filter before they can run the dashboard:
  ...
  1. Make sure the dashboard is in [edit mode](/looker/docs/editing-user-defined-dashboards#entering_edit_mode_in_dashboards_\(beta\)).
...
Required filters are helpful in limiting the sizes of queries sent to your database. To limit dashboard viewers' data access, use the [`access_filter`](/looker/docs/reference/param-explore-access-filter) LookML parameter instead.

## Setting up linked filters

**Note:** Linked filters don't affect suggestions on any fields that have a [suggest_dimension](/looker/docs/reference/param-field-suggest-dimension) provided.
...
![](/static/looker/docs/images/dash-beta-linked-filter-tooltip-2316.png)
...
When you are linking filters, any filter can be used as the "parent" filter (the filter that narrows another filter's options), but the "child" filter (the filter to be narrowed) can only use a field of type [`string`](/looker/docs/reference/param-dimension-filter-parameter-types#string), commonly used for words or phrases, or type [`zipcode`](/looker/docs/reference/param-dimension-filter-parameter-types#zipcode), commonly used for zip codes.
...
Once filters are placed on a dashboard, you can move or reposition them by:
  ...
  1. Entering [edit mode](/looker/docs/editing-user-defined-dashboards#entering_edit_mode_in_dashboards_\(beta\)) on the dashboard.
  2. Hovering over the filter to display the filter's six-dot icon. The six-dot icon appears at the top left of the filter. ![](/static/looker/docs/images/filter-six-dot-icon-2110.png)
...
If [cross-filtering](/looker/docs/cross-filtering-dashboards) is enabled on a dashboard, you cannot reposition cross-filters using drag and drop, and entering edit mode clears any cross-filters currently on the dashboard.
...
**Caution:** Hiding dashboard filters through the dashboard URL is not a security feature. Users can still see and modify dashboard filters if they access the dashboard by navigating to it within Looker or by using a URL that does not contain the `hide_filter` parameter.
...
There are a few additional things to understand about hiding dashboard filters:
  ...
  * Any user with [**View** access](/looker/docs/organizing-spaces#folder_access_levels) to a dashboard can hide dashboard filters with the URL. The user does not need to have edit access to the dashboard.
  ...
  * If dashboards are being [scheduled or sent](/looker/docs/scheduling-and-sending-dashboards), dashboard links that are included in the delivery do not include the `hide_filter` parameter.
  * If dashboards contain [alerts](/looker/docs/alerts-overview), dashboard links that are included in the alert delivery do not include the `hide_filter` parameter.

## Applying dashboard filters to alerts

The **Filters** menu also lets you apply filter edits to the alerts present on a dashboard's tiles. See the [**Effect of dashboard edits on alerts**](/looker/docs/effect-of-dashboard-edits-on-alerts#editing_dashboard_filters) documentation page for more information.

## Editing dashboard filters

In [edit mode](/looker/docs/editing-user-defined-dashboards#entering_edit_mode_in_dashboards_\(beta\)), a three-dot **Filter Menu** icon appears next to every filter. To edit a filter, select the icon. This opens the filter menu, which displays options to edit or delete a filter.
...
Once you've edited filters, select the dashboard's [**Update** button](/looker/docs/viewing-dashboards#reload_data_icon) to apply the new filter settings to the tiles.

Select **Save** in the blue toolbar to save your changes and exit edit mode.

## Deleting dashboard filters

In [edit mode](/looker/docs/editing-user-defined-dashboards#entering_edit_mode_in_dashboards_\(beta\)), three-dot **Filter Menu** icon appears next to every filter.
...
To delete a filter:
  ...
  5. Once you've deleted the filters, select the dashboard's [**Update** button](/looker/docs/viewing-dashboards#reload_data_icon) to apply the new filter settings to the tiles.
