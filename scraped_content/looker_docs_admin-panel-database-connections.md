...
Column | Description  
---|---  
Name | The name of the connection, chosen by you, that is used in the [`connection`](/looker/docs/reference/param-model-connection) LookML parameter. When you test the connection, Looker displays the list of status checks under the connection name.  
...
SSL | Whether or not you are using SSL encryption to protect the data traveling between Looker and your database (there are [other secure options](/looker/docs/enabling-secure-db-access) besides SSL).  
Type | The SQL dialect of the database connection.  
Actions | Actions that you can take for a connection: test a connection, edit a connection, view other information about a connection, jump to a list of links to the connection's Explores, or delete a connection. If the ability to create a new database has been enabled for your instance, you can also [add data from an additional data source](/looker/docs/connecting-to-your-db#connecting_to_a_data_source) to any Looker-managed connections.  
...
The **SSH Servers** tab lists the SSH server configurations that you have added, indicates the status of connections to the SSH servers, and lists the [database connections](/looker/docs/connecting-to-your-db#adding-connections) using each SSH server. From the **SSH Servers** tab, you can also test a connection to an SSH server, and add or edit SSH server configurations.
...
Three-dot **Options** menu more_vert | Actions that you can take for an SSH server configuration: test connections to an SSH server, [add a database connection to an SSH server](/looker/docs/connecting-to-your-db#adding_a_database_connection_to_an_ssh_server), edit an SSH server configuration, or delete an SSH server configuration.  
...
Looker displays the [**Connection Settings**](/looker/docs/connecting-to-your-db#existing_database) page, with the SSH server listed in the **SSH Server** field.
...
Show PDT Event Log | This option brings you to a Looker Explore page that lets you create Looker Explore queries based on the derived table activity for this connection. The available fields are described in more detail on our [Persistent derived tables](/looker/docs/admin-panel-database-pdts#event_log) documentation page.