...
In Looker, [persistent derived tables (PDTs)](/looker/docs/derived-tables#persistent_derived_table) are written to the scratch schema of your database. Looker persists and rebuilds a PDT based on its [persistence strategy](/looker/docs/derived-tables#persistence_strategies). When a PDT is triggered to rebuild, by default Looker rebuilds the entire table.
...
![A large table with the three bottom rows highlighted to show a small number of new rows being added to the table.](/static/looker/docs/images/dev-incremental-pdts-2102.png)

If your dialect supports incremental PDTs, you can make the following types of PDTs into incremental PDTs:

  * [Aggregate tables](/looker/docs/aggregate_awareness#adding_aggregate_tables_to_your_project)
  * [LookML-based (native)](/looker/docs/derived-tables#native_derived_tables) PDTs
  * [SQL-based](/looker/docs/derived-tables#sql-based_derived_tables) PDTs
...
Note the following for incremental PDTs:
  ...
  * Incremental PDTs are supported only for PDTs that use a trigger-based persistence strategy ([`datagroup_trigger`](/looker/docs/derived-tables#datagroup_trigger), [`sql_trigger_value`](/looker/docs/derived-tables#sql_trigger_value), or [`interval_trigger`](/looker/docs/derived-tables#interval_trigger)). Incremental PDTs are not supported for PDTs that use the [`persist_for`](/looker/docs/derived-tables#persist_for) persistence strategy.
  * For SQL-based PDTs, the table query must be defined using the [`sql`](/looker/docs/reference/param-view-sql-for-derived-table) parameter to be used as an incremental PDT. SQL-based PDTs that are defined with the [`sql_create`](/looker/docs/reference/param-view-sql-create) parameter or the [`create_process`](/looker/docs/reference/param-view-create-process) parameter cannot be incrementally built. As you can see in Example 1 on this page, Looker uses an INSERT or a MERGE command to create the increments for an incremental PDT. The derived table cannot be defined using custom Data Definition Language (DDL) statements, since Looker wouldn't be able to determine which DDL statements would be required to create an accurate increment.
  * The incremental PDT's source table must be optimized for time-based queries. Specifically, the time-based column that is used for the increment key must have an optimization strategy, such as [partitioning](/looker/docs/reference/param-view-partition-keys), [sortkeys](/looker/docs/reference/param-view-sortkeys), [indexes](/looker/docs/reference/param-view-indexes), or whatever optimization strategy is supported for your dialect. Source table optimization is strongly recommended because each time the incremental table is updated, Looker queries the source table to determine the latest values of the time-based column that is used for the increment key. If the source table is not optimized for these queries, Looker's query for the latest values may be slow and expensive.

## Defining an incremental PDT

You can use the following parameters to make a PDT into an incremental PDT:

  * [`increment_key`](/looker/docs/reference/param-view-increment-key) (required to make the PDT an incremental PDT): Defines the time period for which new records should be queried.
  * [`{% incrementcondition %}`](/looker/docs/reference/param-view-increment-key#example_psdt) Liquid filter (required to make a [SQL-based PDT](/looker/docs/derived-tables#sql-based_derived_tables) an incremental PDT; not applicable to [LookML-based PDTs](/looker/docs/derived-tables#native_derived_tables)): Connects the increment key to the database time column that the increment key is based on. See the [`increment_key`](/looker/docs/reference/param-view-increment-key#example_psdt) documentation page for more information.
  * [`increment_offset`](/looker/docs/reference/param-view-increment-offset) (optional): An integer that defines the number of previous time periods (at the increment key's granularity) that are rebuilt for each incremental build. The `increment_offset` parameter is useful in the case of late-arriving data, where previous time periods may have new data that wasn't included when the corresponding increment was originally built and appended to the PDT.

See the [`increment_key`](/looker/docs/reference/param-view-increment-key) parameter documentation page for examples that show how to create incremental PDTs from [persistent native derived tables](/looker/docs/reference/param-view-increment-key#example_pndt), [persistent SQL-based derived tables](/looker/docs/reference/param-view-increment-key#example_psdt), and [aggregate tables](/looker/docs/reference/param-view-increment-key#example_aggtable).
...
The increment key is based on the `departure_date` dimension, which is actually the `date` [timeframe](/looker/docs/reference/param-field-dimension-group#timeframes) from the `departure` dimension group. (See the [`dimension_group`](/looker/docs/reference/param-field-dimension-group) parameter documentation page for an overview of how dimension groups work.) The dimension group and timeframe are both defined in the `flights` view, which is the `explore_source` for this PDT. Here is how the `departure` dimension group is defined in the `flights` view file:
...
A PDT's `increment_key` and `increment_offset` settings are independent of the PDT's [persistence strategy](/looker/docs/derived-tables#persistence_strategies):

  * The incremental PDT's persistence strategy determines only when the PDT increments. The PDT builder doesn't modify the incremental PDT unless the table's persistence strategy is triggered, or unless the PDT is manually triggered with the [**Rebuild Derived Tables & Run**](/looker/docs/derived-tables#manually_rebuilding_persistent_tables_for_a_query) option in an Explore.
...
Before deploying a new incremental PDT to your production environment, you can test the PDT to be sure it builds and increments. To test an incremental PDT in Development Mode:
  ...
  1. Create an Explore for the PDT:
     ...
     * In an associated model file, use the [`include`](/looker/docs/reference/param-model-include#including_views_and_dashboards_in_a_model) parameter to include the PDT's view file in the model file.
     * In the same model file, use the [`explore`](/looker/docs/reference/param-explore-explore) parameter to create an Explore for the incremental PDT's view.
...
**Tip:** Once you've included the view in your model file and created an Explore, you can navigate directly to the Explore from the view file or model file in your LookML project. To do this, select the **See file actions** ⏷ menu at the top of the LookML file.

![](/static/looker/docs/images/dev-file-actions-menu-2104.png)
  ...
  2. You can verify that the initial PDT was built in the following ways:
     ...
     * If you have the [`see_logs`](/looker/docs/admin-panel-users-roles#see_logs) permission, you can verify that the table was built by looking in the [PDT Event Log](/looker/docs/admin-panel-database-pdts#pdt_event_log_model). If you don't see the PDT create events in the PDT Event Log, check the status information at the top of the PDT Event Log Explore. If it says "from cache," you can select **Clear Cache & Refresh** to get more recent information.
  ...
  3. Once you create the initial build of the PDT, prompt an incremental build of the PDT by using the [**Rebuild Derived Tables & Run**](/looker/docs/derived-tables#manually_rebuilding_persistent_tables_for_a_query) option from the Explore.

  4. You can use the same methods as before to verify that the PDT builds incrementally:

     * If you have the [`see_logs`](/looker/docs/admin-panel-users-roles#see_logs) permission, you can use the [PDT Event Log](/looker/docs/admin-panel-database-pdts#pdt_event_log_model) to see `create increment complete` events for the incremental PDT. If you don't see this event in the PDT Event Log and the query status says "from cache," select **Clear Cache & Refresh** to get more recent information.
...
After the PDT is built in Development Mode, the same table will be used for production once you deploy your changes, unless you make further changes to the table's definition. See the [Persisted tables in Development Mode](/looker/docs/derived-tables#persisted_tables_in_development_mode) section of the **Derived tables in Looker** documentation page for more information.
...
If your schema changes and you want to rebuild your incremental PDT from scratch, use the API call [`start_pdt_build`](/looker/docs/reference/looker-api/latest/methods/DerivedTable/start_pdt_build), and include the `full_force_incremental` parameter.

## Supported database dialects for incremental PDTs

For Looker to support incremental PDTs in your Looker project, your database dialect must support [Data Definition Language (DDL)](https://en.wikipedia.org/wiki/Data_definition_language) commands that enable deleting and inserting rows.

The following table shows which dialects support incremental PDTs in the latest release of Looker (for [Databricks](/looker/docs/db-config-databricks), Incremental PDTs are supported only on Databricks version 12.1 and higher):
