# https://cloud.google.com/looker/docs/admin-panel-authentication-google

Depth: 3

**Note:** Setting up Google authentication in [Looker (Google Cloud core)](/looker/docs/looker-core-overview) is covered on the [Create OAuth authorization credentials for a Looker (Google Cloud core) instance](/looker/docs/looker-core-create-oauth) and the [Use Google OAuth for Looker (Google Cloud core) user authentication](/looker/docs/looker-core-oauth-authentication) documentation pages.
...
Looker can perform authentication using [Google OAuth](https://developers.google.com/identity/protocols/OAuth2) for users that have accounts registered with Google Google Workspace.
...
Using Google OAuth requires the following:
  ...
  * A [Google Google Workspace](https://workspace.google.com/) account for the organization.
  ...
  * Each user must have a [managed user account](https://support.google.com/a/answer/11112794?ref_topic=7042002) in Google Google Workspace. To find and migrate any users with unmanaged user accounts, use the [Transfer tool for unmanaged users](https://support.google.com/a/answer/6178640).
...
The steps for enabling Google OAuth on the Google side are described in this section. The generic description of these steps is on the Google support page on [setting up OAuth 2.0](https://support.google.com/cloud/answer/6158849). You can also refer to [Google Cloud console Help](https://developers.google.com/console/help/) documentation.
...
Google displays the **Create OAuth client ID** page.
  ...
  7. Google requires that you configure an [OAuth consent screen](https://support.google.com/cloud/answer/6158849#userconsent), which lets your users choose how to grant access to their private data and provides a link to your organization's terms of service and privacy policy. Click **Configure consent screen**. (If you have configured OAuth consent for a previous project, you will not see this option, and you can skip to step 13.)
...
**Note:** For information about configuring the Google **OAuth consent screen** , see the [Setting up OAuth 2.0](https://support.google.com/cloud/answer/6158849#userconsent) Google support page.
...
## Troubleshooting
  ...
  * If a user's attempt to sign in fails, and Looker displays an error such as `User not in the authorized domain`, check the `hd` field of the JSON response. If the `hd` field contains a domain, make sure that the domain is registered to your Google Google Workspace account. If the `hd` field is empty, use the [Transfer tool for unmanaged users](https://support.google.com/a/answer/6178640) to invite the user to convert their account to a managed account within your domain.
...
**Note:** If a user has authenticated into the Looker instance using only Google, you can enable alternate login only by using the Looker API. To learn how to enable alternate login using the Looker API, see the [Enabling the alternate login option](/looker/docs/alternate-login) documentation page.
