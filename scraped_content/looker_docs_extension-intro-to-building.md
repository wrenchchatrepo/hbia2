# https://cloud.google.com/looker/docs/extension-intro-to-building
...
Looker extensions each require a LookML project that includes a model file and a manifest file to run.
  ...
  1. [Create a blank project](/looker/docs/generating-a-model#blank-project) for your extension.

  2. In your new project, [create a model file](/looker/docs/ide-folders#creating_files) by using the `+` at the top of the file browser in the Looker IDE.

  3. The new model file is generated with a [`connection`](/looker/docs/reference/param-model-connection) parameter and an [`include`](/looker/docs/reference/param-model-include) parameter:

![](/static/looker/docs/images/new-model-file-ext-2104.png)

Replace `connection_name` in the [`connection`](/looker/docs/reference/param-model-connection) parameter with the name of a valid [database connection](/looker/docs/admin-panel-database-connections) on your instance. The extension requires a valid model file because Looker permissions data is accessed through the LookML model. To manage access to your extension, you must associate the extension with a valid LookML model and grant users permissions to access that model. When you are done editing the model file, click **Save Changes**.
...
Since your extension does not require a view file, either delete the `include: "/views/*.view.lkml"` parameter or add a `#` character at the beginning of the line to make the line a comment. Leaving the `include: "/views/*.view.lkml"` parameter as is will cause the LookML validator to generate errors.
  ...
  4. [Create a project manifest file](/looker/docs/ide-folders#creating_files) by using the `+` at the top of the file browser in the Looker IDE.

The project manifest file will initally be empty, but later in this procedure you will copy generated contents into your project manifest file that will include the [`application`](/looker/docs/reference/param-manifest-application) parameter. The `application` parameter gives the extension a label, tells Looker where to find the extension JavaScript, and provides a list of [entitlements](/looker/docs/reference/param-manifest-application#entitlements) for the extension. Entitlements define the Looker resources that the extension can access. The extension will not be able to access a Looker resource unless that resource is listed in the entitlements.
...
You can leave the manifest file as is for now, though, since you will copy the rest of the file's contents into it later in this process.
  ...
  5. [Configure a Git connection](/looker/docs/setting-up-git-connection) for your new project.

You can access the **Configure Git** page either by clicking the **Configure Git** button in the top right corner of the Looker IDE, or by using the **Project Configuration** page as described in the [Setting up and testing a Git connection](/looker/docs/setting-up-git-connection) documentation page.
...
Both processes require [Yarn](https://yarnpkg.com/), so ensure you have Yarn installed.
...
![](/static/looker/docs/images/yarn-create-looker-extension-output-2108.png)
...
![](/static/looker/docs/images/yarn-create-looker-extension-ls-2104.png)
...
![](/static/looker/docs/images/ext-manifest-contents-2108.png)

Click **Save Changes** to save your project manifest file.

**Note:** The contents of the generated `manifest.lkml` file include the [`project_name` LookML parameter](/looker/docs/reference/param-manifest-project-name). The `project_name` parameter is used by the Local Project Import feature. See the [Importing files from other projects](/looker/docs/importing-projects#importing_local_projects) documentation page for details on importing projects.
...
This procedure requires [Yarn](https://yarnpkg.com/), so ensure that you have Yarn installed.
...
![](/static/looker/docs/images/ext-manifest-contents-2104.png)
...
The extension is now running and serving the JavaScript on a local development server specified in the [`url`](/looker/docs/reference/param-manifest-application#url) parameter of the manifest file: `http://localhost:8080/bundle.js`.

  8. Reload Looker using your browser's reload function.

In the **Applications** section of the Looker main menu, you will see the name of your new extension, which is determined by the [`label` parameter](/looker/docs/reference/param-manifest-application) in your project manifest file.
...
> Keep in mind that as you add functionality, you may need to modify the [entitlements](/looker/docs/reference/param-manifest-application#entitlements) granted to your extension in the [`application`](/looker/docs/reference/param-manifest-application) parameter in your project manifest file.