# https://cloud.google.com/looker/docs/editing-user-defined-dashboards

Depth: 3

To edit a dashboard, you must have the following prerequisites:

  * The [**Manage Access, Edit**](/looker/docs/organizing-spaces#folder_access_levels) access level for the folder in which the dashboard resides
  * The appropriate [Looker permissions](/looker/docs/admin-panel-users-roles#save_content) to edit the dashboard

Additionally, to edit a specific dashboard tile, you must have [access to the model](/looker/docs/admin-panel-users-roles#model_sets) that the tile is based on.

If you don't have access to all the models that a particular dashboard's tiles are based on, you can still edit other parts of the dashboard, such as the dashboard title, dashboard settings, [dashboard filters](/looker/docs/filters-user-defined-dashboards#editing_dashboard_filters) (although you will not see filter suggestions for filters based on fields in a model you do not have access to), and tiles for which you do have model access.

When editing a dashboard, keep in mind best practices for dashboard construction. Find information and recommendations about building performant dashboards in the [Considerations when building performant Looker dashboards](/looker/docs/best-practices/considerations-when-building-performant-dashboards) Best Practices page.

## Entering edit mode in dashboards

To enter edit mode, select **Edit dashboard** from the three-dot **Dashboard actions** menu.

In edit mode, you can add, [move](/looker/docs/filters-user-defined-dashboards#moving_and_repositioning_dashboard_filters), edit, or delete tiles or filters, or edit the dashboard title.
...
![](/static/looker/docs/images/dboards-editing-2400.png)

After you make at least one edit, the **Save** button becomes available. Click **Save** to save your edits and exit edit mode.

To discard your edits, click **Cancel**. In the **Discard changes?** dialog box, click **Discard** to confirm that you want to cancel and exit edit mode. The **Discard changes?** dialog box also appears if you attempt to navigate away from the dashboard by clicking on the [navigation breadcrumb](/looker/docs/viewing-dashboards#navigation_breadcrumb) or the forward or back arrows on your browser.
...
When you add a visualization using [grid layout](/looker/docs/column-options#grid_layout) to a dashboard, the grid arrangement can become responsive to the size of the dashboard tile.
...
To ensure that the grid arrangement is responsive, do not enter a number into the visualization's [**Number of Charts per Row**](/looker/docs/column-options#grid_layout) setting. If you enter a value into **Number of Charts per Row** , the dashboard tile respects that setting regardless of the size or shape of the dashboard tile.

## Adding tiles

In edit mode, you can add tiles or text to your dashboard. See the [Creating user-defined dashboards](/looker/docs/creating-user-defined-dashboards#adding_tiles_and_text_to_a_dashboard) documentation page for information about how to add text and tiles.

## Editing tiles

In edit mode, the three-dot **Tile actions** menu at the top right of each tile shows the available edit options.

These options differ slightly depending on whether or not you are viewing a [query tile](/looker/docs/creating-user-defined-dashboards#query_tiles) or a [Look-linked tile](/looker/docs/creating-user-defined-dashboards#look-linked_tiles).
...
In edit mode, you can edit a [query tile's](/looker/docs/creating-user-defined-dashboards#query_tiles) title by clicking on the title and making your edits. Click **Save** at the upper right of the dashboard to save your changes.

![](/static/looker/docs/images/dboard-beta-edit-tile-title-720.png)

You can also edit tile titles for both query tiles and [Look-linked](/looker/docs/creating-user-defined-dashboards#look-linked_tiles) tiles when editing a tile's underlying query or Look.
...
![](/static/looker/docs/images/dboard-beta-tile-query-edit-720.png)

> When you're editing a dashboard tile that contains a funnel chart or a timeline chart, the charts look different on the dashboard tile than they do in the edit window. Additionally, map charts' [tooltips](/looker/docs/viewing-dashboards#drilling_into_data_points) function differently when viewed on dashboard tiles. For more information, visit the [funnel chart](/looker/docs/funnel-options#stepped_funnel_and_funnel_styles), [timeline chart](/looker/docs/timeline-options#viewing_timeline_charts_in_the_new_dashboard_experience), and [map chart](/looker/docs/map-options#viewing_map_charts_in_the_new_dashboard_experience) documentation pages.
...
The hide/show option does not appear in the three-dot **Tile actions** menu of [text tiles](/looker/docs/creating-user-defined-dashboards#text_tiles) or tiles that use [single value visualizations](/looker/docs/single-value-options). To hide or show the title of a single value visualization, [click **Edit**](/looker/docs/editing-user-defined-dashboards#editing_a_tiles_query_or_look) in the tile's three-dot **Tile actions** menu, toggle the [**Show Title**](/looker/docs/single-value-options#show_title) switch in the style menu, and save. To hide or show a title of a text tile, [click **Edit**](/looker/docs/editing-user-defined-dashboards#editing_a_tiles_query_or_look) in the tile's three-dot **Tile actions** menu, remove or reinstate the tile title, and save.
...
Enter the note text and configure the following display options:
  ...
  1. In the **Note Text** field, enter the note text to show on the tile. You can use plain text or [HTML](/looker/docs/html-sanitization).
  2. In the **Display Location** field, select the location of the note. The options are above the visualization, below the visualization, or as text that appears when users hold their pointers over a circled _i_ icon to the right of the tile title. For tiles containing [single value visualizations](/looker/docs/single-value-options) that are sized to be 150 pixels tall or shorter, title notes render as hover text regardless of the location selected in this field.
...
![](/static/looker/docs/images/dboard-beta-collapse-expand-note-76-v2.png)
...
In edit mode, you can duplicate [query tiles](/looker/docs/creating-user-defined-dashboards#query_tiles) by clicking **Duplicate tile** from the tile's three-dot **Tile actions** menu:
...
You cannot duplicate a [Look-linked tile](/looker/docs/creating-user-defined-dashboards#look-linked_tiles) and will not see **Duplicate tile** in the three-dot **Tile actions** menu of Look-linked tiles.
...
There are two types of text tiles, each of which offers a different editing experience but results in the same kind of tile:
  ...
  * **Text** : A dashboard text tile that provides a [visual editing experience](/looker/docs/creating-user-defined-dashboards#adding_text_tiles).
  * **Markdown** : A dashboard text tile that [can be edited](/looker/docs/creating-user-defined-dashboards#adding_markdown_tiles) using [some HTML](/looker/docs/html-sanitization) and [a subset](/looker/docs/using-markdown-in-text-tiles) of the Markdown markup language to format the tile.
...
![](/static/looker/docs/images/dboard-beta-settings-2218.png)

### Time zone

This option is only available if your Looker admin has enabled the [**User Specific Time Zone**](/looker/docs/admin-panel-general-settings#user_specific_time_zones) setting.

Select the time zone in which your dashboard will be run. Dashboard viewers will be able to [temporarily change the time zone setting](/looker/docs/viewing-dashboards#temporarily_changing_the_dashboard_time_zone) when viewing the dashboard.
...
If **Run on load** is disabled, the dashboard does not display any data until the [reload data](/looker/docs/viewing-dashboards#reload_data) icon is clicked.

### Allow full-screen mode for visualizations

If your Looker admin has enabled the **Full Screen Visualizations** [Labs feature](/looker/docs/admin-panel-general-labs), an option for **Allow full-screen mode for visualizations** appears in the dashboard settings. This option defaults to enabled.

Disabling this option on a dashboard prevents viewers of that dashboard from seeing the [**View**](/looker/docs/viewing-dashboards#view_options) option in each dashboard tile's three-dot **Dashboard actions** menu, and viewers will not be able to see dashboard tiles in full-screen or expanded formats.
...
The [**Clear cache and refresh**](/looker/docs/viewing-dashboards#updating_data_on_dashboards_\(beta\)) and [**reload data**](/looker/docs/viewing-dashboards#reload_data) options are independent of the autorefresh interval, meaning that manually refreshing the dashboard does not restart the autorefresh interval. For example, if a dashboard's autorefresh interval is 1 hour, you can use the **Clear cache and refresh** option 40 minutes after an autorefresh and the dashboard will still autorefresh 20 minutes later (1 hour after it was last autorefreshed).

Similarly, autorefresh works regardless of whether **Run on load** is enabled or disabled. If **Run on load** is disabled and you open a dashboard with autorefresh settings, the refresh interval begins upon opening regardless of whether you click the [reload data](/looker/docs/viewing-dashboards#reload_data) icon to load the data initially.
...
Find more information and recommendations about building performant dashboards in the [Considerations when building performant Looker dashboards](/looker/docs/best-practices/considerations-when-building-performant-dashboards) Best Practices page.
...
Dashboard viewers can temporarily change the filter bar back and forth between collapsed and expanded by clicking the [filters icon](/looker/docs/viewing-dashboards#filters_icon). However, only the **Default filters view** setting permanently changes the default state of the filter bar.
...
**Caution:** Using this feature resets the order of the tiles to the order in which they were originally added to the dashboard. Use caution as this can drastically change the layout of your dashboard.
...
Tile sizes are responsive to the size of the dashboard. Quick layout affects all tiles including text tiles, visualization tiles, Markdown tiles, and [buttons](/looker/docs/creating-user-defined-dashboards#adding_buttons).

## Editing dashboard details

In edit mode, select **Show dashboard details** from the [three-dot **Dashboard actions** menu](/looker/docs/viewing-dashboards#three-dot_dashboard_menu). The **Description** text box then becomes editable.

Once you are finished editing the description, click **Save** to save the change.

## Deleting dashboards

If you have the [**Manage Access, Edit**](/looker/docs/organizing-spaces#folder_access_levels) access level for a dashboard, you can delete it in one of two ways:

  * You can delete one or more dashboards at a time from folders, as described on the [Organizing and managing access to content](/looker/docs/organizing-spaces#deleting_looks_and_dashboards) documentation page.
  * You can delete a single dashboard by selecting [**Move to trash**](/looker/docs/viewing-dashboards#anatomy_of_a_dashboard_\(beta\)) from the dashboard's three-dot **Dashboard actions** menu.

Once you delete a dashboard, [only a Looker admin can retrieve it from the trash](/looker/docs/admin-spaces).

## Keyboard shortcuts when dashboards are in edit mode

To see the keyboard shortcuts that you can use when editing dashboards, visit the [Keyboard shortcuts in Looker](/looker/docs/keyboard-shortcuts#shortcuts_when_editing_dashboards) documentation page.