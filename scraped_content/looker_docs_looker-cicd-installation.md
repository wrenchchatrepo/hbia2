# https://cloud.google.com/looker/docs/looker-cicd-installation

Depth: 3

This page explains how to install and configure the necessary components to implement a [CI/CD workflow in Looker](/looker/docs/looker-cicd).
...
This process uses tools called [Gazer](https://github.com/looker-open-source/gzr) and [Spectacles](https://www.spectacles.dev/) that are designed to work with Unix-like operating systems. Each LookML developer must have access to the command line in a Linux environment or macOS where you plan to run your CI/CD workflow.

If you are using Windows, Gazer and Spectacles can be used inside of Microsoft's [Windows Subsystem For Linux](https://learn.microsoft.com/en-us/windows/wsl/about) (WSL). WSL lets you run a variety of different Linux flavors. If you don't have a preferred Linux operating system, the latest version of Ubuntu Linux is a good choice due to its broad support.
...
[Database connections](/looker/docs/admin-panel-database-connections) should have the same name within each Looker instance, regardless of what tier they represent. For example, a `sales` connection should have that name in all instances, rather than `sales_dev` or `sales_qa`.
...
For GitHub, follow the instructions at [Adding a new SSH key to your GitHub account](https://docs.github.com/en/authentication/connecting-to-github-with-ssh/adding-a-new-ssh-key-to-your-github-account?platform=linux).

### Creating and configuring a Git server repository

For the CI/CD workflow to function, LookML must be stored in a git repository and connected to a [Looker project](/looker/docs/manage-projects). In the project settings, **Git Production Branch Name** must be set to `main` and **Enable Advanced Deploy Mode** must be enabled.
...
### Create New Repo
  ...
  6. In Looker, [create a new project](/looker/docs/generating-a-model).
...
### Use Existing Repo
  ...
  2. Press the green button labeled **< > Code** and copy the SSH URL. It will look similar to: `git@github.com:org_name/REPOSITORY_NAME.git`.
  3. In Looker, [create a new project](/looker/docs/generating-a-model).
  4. Enter development mode and choose the project settings item from the left side bar, then **Configure Git**.
  5. Paste in the repository URL (`git@github.com:org_name/REPOSITORY_NAME.git` in this example) and select **Continue**.
  6. Copy the deploy key and go back to the GitHub UI for this repository.
  7. Choose **Settings** and then **Deploy keys**.
  8. Click the **Add deploy key** button and paste the deploy key into the **Key** field.
  9. Add a title such as `Looker-REPOSITORY_NAME`, check the **Allow write access** checkbox, and push the **Add key** button.
  10. Return to Looker and choose **Test and Finalize Setup**.
  11. Choose project settings again from the left side bar. Change the **Git Production Branch Name** to `main`.
  12. Choose **Enable Advanced Deploy Mode** and select **Save Project Configuration**.
...
[LAMS](https://github.com/looker-open-source/look-at-me-sideways) is an open source linter that inspects your LookML for errors and bad practices. Add a file named `lams.yml` to the `.github/workflows` directory with these contents:
...
[Release Please](https://github.com/google-github-actions/release-please-action) is an open source tool that automatically tags releases with the proper version numbers.
...
This GitHub action will ensure that a pull request is opened with a title that adheres to the [conventional commit](/looker/docs/looker-cicd-usage#best_practices_for_numbering_releases_and_naming_commits) standard.
...
![GitHub UI for adding branch protection.](/static/looker/docs/images/cicd-branch-protection.png)
...
![GitHub UI for setting branch protection options.](/static/looker/docs/images/cicd-branch-protection-options.png)
...
In Looker you can require that pull requests are used and make Looker open PRs on behalf of the developer. This should only be configured for the development instance. The QA and production instance will use [Advanced Deploy Mode](/looker/docs/advanced-deploy-mode) to get their updates.

Enable this by going to the [**Project Configuration** page](/looker/docs/git-options#configuration) for each project, and then select **Pull Requests Required** under the heading **GitHub Integration**.

![](/static/looker/docs/images/cicd-project-settings.png)
...
![GitHub UI for configuring webhooks.](/static/looker/docs/images/cicd-github-webhook-settings.png)

Ensure that **Pull Requests** and **Pushes** are selected:

![GitHub checkboxes for pull requests and pushes.](/static/looker/docs/images/cicd-github-webhook-options.png)
...
These commands will also work on Debian Linux, Linux Mint, and several other Linux flavors that use the Aptitude package manager. You may need to search for commands that work on other Linux flavors, or for commands to install on macOS. Check [Installing Ruby](https://www.ruby-lang.org/en/documentation/installation/) for more information.

### Installing Gazer

[Gazer](https://github.com/looker-open-source/gzr) is an open source project created by Google employees to navigate and manage Spaces, Looks, and dashboards using a command line tool.
...
[Spectacles](https://www.spectacles.dev/) is a non-Google tool used to test LookML. Spectacles offer both a [paid version](https://www.spectacles.dev/pricing) and an [open source version](https://github.com/spectacles-ci/spectacles), and details for installing it can be found on their [Getting Started page](https://docs.spectacles.dev/cli/tutorials/getting-started/).
...
These commands will also work on Debian Linux, Linux Mint, and several other Linux flavors that use the Aptitude package manager. You may need to search for commands that work on other Linux flavors. Instructions for Fedora and macOS, for example, can be found at [Getting Started - Installing Git](https://git-scm.com/book/en/v2/Getting-Started-Installing-Git).
...
In place of KEY_NAME, use the name of the private key file that you generated with the [Adding a new SSH key to your GitHub account](https://docs.github.com/en/authentication/connecting-to-github-with-ssh/adding-a-new-ssh-key-to-your-github-account?platform=linux) instructions. The private key file has the same name as the public key file, but without the `.pub` extension. For example, if you used the public key found in the file `id_ed25519.pub`, the private key would be named `id_ed25519`.
...
Test that this works by running a simple Gazer command against each server, such as the following:
