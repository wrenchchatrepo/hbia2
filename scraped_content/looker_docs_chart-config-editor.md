...
You can use the Chart Config Editor to customize formatting options on Looker visualizations that use the [HighCharts API](https://api.highcharts.com/highcharts/chart). This includes most [Cartesian charts](/looker/docs/visualization-types#cartesian_charts), such as the [column chart](/looker/docs/column-options), [bar chart](/looker/docs/bar-options), and [line chart](/looker/docs/line-options), among others.

If Gemini in Looker features are enabled, you can use the [**Visualization Assistant**](/looker/docs/looker-core-visualization-gemini-assistance) to generate JSON formatting options from text-based prompts to accelerate the customization of Looker visualizations.
...
To customize a visualization with the Chart Config Editor, follow these steps:
  ...
  4. Select the **Chart Config (Override)** section and enter some valid HighCharts JSON. The new values will override any values in the **Chart Config (Source)** section.
     ...
     * Looker accepts any [valid JSON values](https://www.w3schools.com/js/js_json_datatypes.asp). Looker does not accept functions, dates, or undefined values.
...
![](/static/looker/docs/images/avc-basic-252.png)
...
The `style` attribute can be used to apply styles that HighCharts supports. For example, you can color series values using `style.color`, color series borders using `style.borderColor`, and set series border width using `style.borderWidth`. For a more complete list of style options, see the [Highcharts options for `series.column.data`](https://api.highcharts.com/highcharts/series.column.data).

For [line visualizations](/looker/docs/line-options), use `style.marker.fillColor` and `style.marker.lineColor` instead of `style.color`. For a more complete list of line style options, see the [Highcharts options for `series.line.data.marker`](https://api.highcharts.com/highcharts/series.line.data.marker).
...
The following sections provide examples of some common use cases for the Chart Config Editor. For a complete list of the attributes that you can edit, see the [HighCharts API documentation](https://api.highcharts.com/highcharts/chart).
...
To change the background color of a visualization, use the [`chart.backgroundColor`](https://api.highcharts.com/highcharts/chart.backgroundColor) attribute.

Similarly, to change the text color of the axes in a visualization, use the following attributes:

  * [`xAxis.labels.style.color`](https://api.highcharts.com/highcharts/xAxis.labels.style.color)
  * [`xAxis.title.style.color`](https://api.highcharts.com/highcharts/xAxis.title.style.color)
  * [`yAxis.labels.style.color`](https://api.highcharts.com/highcharts/yAxis.labels.style.color)
  * [`yAxis.title.style.color`](https://api.highcharts.com/highcharts/yAxis.title.style.color)
...
![](/static/looker/docs/images/avc-background-240.png)

### Customize tooltip color

To customize the color of the tooltip, use the following attributes:

  * [`tooltip.backgroundColor`](https://api.highcharts.com/highcharts/tooltip.backgroundColor)
  * [`tooltip.style.color`](https://api.highcharts.com/highcharts/tooltip.style.color)
...
![](/static/looker/docs/images/avc-tooltip-240.png)

### Customize tooltip content and styles

To customize the content of the tooltip, use the following attributes:

  * [`tooltip.format`](https://api.highcharts.com/highcharts/tooltip.format)
...
This example uses the following HighCharts functions and variables:
  ...
  * `{y:.2f}` is a variable that returns the y-axis value of the selected point, rounded to two decimal places. 
    ...
    * `{variable:.2f}` rounds `variable` to two decimal places. See the [Highcharts templating documentation](https://www.highcharts.com/docs/chart-concepts/templating) for more examples of value formatting.
...
![](/static/looker/docs/images/avc-tooltip-format-242.png)

### Add chart annotations and captions

To add an annotation, use the [`annotations`](https://api.highcharts.com/highcharts/annotations) attribute. To add a caption to the chart, use the [`caption`](https://api.highcharts.com/highcharts/caption) attribute.
...
![](/static/looker/docs/images/avc-annotations-240.png)

### Add vertical reference bands

To add a vertical reference band, use the [`xAxis.plotBands`](https://www.highcharts.com/docs/chart-concepts/plot-bands-and-plot-lines) attribute.
...
![](/static/looker/docs/images/avc-band-240.png)

### Add dotted and dashed lines

To change solid lines to dotted or dashed lines, use the [`series.dashStyle`](https://www.highcharts.com/docs/chart-concepts/series#dash-style) attribute.
...
![](/static/looker/docs/images/avc-dashstyle-2416.png)

### Color the maximum, minimum, and percentile values

See the [Getting the most out of Looker visualizations cookbook: Conditional formatting customization in Cartesian charts](/looker/docs/best-practices/cookbook-visualizations-conditional-formatting-customization-in-cartesian-charts) page for an in-depth example about coloring the maximum, minimum, and percentile values of a Cartesian visualization.
...
![](/static/looker/docs/images/avc-conditional-series-2420.png)
...
![](/static/looker/docs/images/avc-field-meta-data-252.png)
...
![](/static/looker/docs/images/avc-compare-mean-2420.png)
...
You can use the Chart Config Editor to create visualization types that aren't included in Looker's [default visualization types](/looker/docs/visualization-types). The following articles provide examples of some of the visualizations that you can design with the Chart Config Editor:

  * [Creating a bullet chart with the Chart Config Editor](/looker/docs/bullet-chart)
  * [Creating a solid gauge chart with the Chart Config Editor](/looker/docs/solid-gauge)
  * [Creating a streamgraph chart with the Chart Config Editor](/looker/docs/streamgraph)
  * [Creating a treemap chart with the Chart Config Editor](/looker/docs/treemap)
  * [Creating a Sankey chart with the Chart Config Editor](/looker/docs/sankey)
  * [Creating a dependency wheel chart with the Chart Config Editor](/looker/docs/dependency-wheel)
  * [Creating a Venn diagram with the Chart Config Editor](/looker/docs/venn)
  * [Creating a sunburst chart with the Chart Config Editor](/looker/docs/sunburst)
  * [Creating an item chart with the Chart Config Editor](/looker/docs/item)